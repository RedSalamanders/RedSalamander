...
# ASan triplets set VCPKG_LINKER_FLAGS="/INCREMENTAL:NO", but vcpkg-cmake-get-vars
# also captures CMake's MSVC default `${flag-prefix}INCREMENTAL` for debug builds
# (CMAKE_*_LINKER_FLAGS_DEBUG_INIT = "/debug /INCREMENTAL") and merges it into
# VCPKG_COMBINED_*_LINKER_FLAGS_DEBUG, producing "...-INCREMENTAL:NO ... -INCREMENTAL".
# Both flags on the link line cause LNK4300 -> LNK1158 with /fsanitize=address.
# Strip standalone INCREMENTAL tokens (`-INCREMENTAL` or `/INCREMENTAL` not followed
# by `:`) so only our triplet-supplied `-INCREMENTAL:NO` remains. vcpkg's combined
# vars use hyphenated flags but we accept either prefix defensively.
foreach(_rs_flag_var
    VCPKG_COMBINED_SHARED_LINKER_FLAGS_DEBUG
    VCPKG_COMBINED_EXE_LINKER_FLAGS_DEBUG
    VCPKG_COMBINED_MODULE_LINKER_FLAGS_DEBUG)
    if(DEFINED ${_rs_flag_var})
        string(REGEX REPLACE "([ \t]|^)[/-]INCREMENTAL([ \t]|$)" "\\1\\2"
            ${_rs_flag_var} "${${_rs_flag_var}}")
        # Run twice to handle adjacent occurrences after a single replace.
        string(REGEX REPLACE "([ \t]|^)[/-]INCREMENTAL([ \t]|$)" "\\1\\2"
            ${_rs_flag_var} "${${_rs_flag_var}}")
    endif()
endforeach()
