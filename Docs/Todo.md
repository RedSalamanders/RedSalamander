# Planned / TODO features

RedSalamander includes menu items and shortcut bindings for features that are planned but not implemented yet. Today these typically show a “Not implemented” message and do nothing.

This page is a user-facing summary of what is planned.

## Navigation

- [ ] Filter UI

## View / layout

- [ ] Pane view options (hidden/system files, extensions, thumbnails, preview pane, filter bar)
- [ ] Navigation bar visibility toggles per pane

## Files and editors

- [ ] “Edit width”
- [ ] Alternate View
- [ ] View With / Edit With menus
- [ ] Edit / Edit new file workflow
- [ ] Editors preferences page (currently a placeholder)

## Selection tools

- [ ] Select/Unselect dialogs
- [ ] Invert / Restore selection
- [ ] Save/Load selection
- [ ] Select same extension/name
- [ ] Hide selected/unselected names and related navigation helpers

## Commands

- [ ] Find files and directories
- [ ] Make file list
- [ ] Go to shortcut/link target
- [ ] List of opened files
- [ ] Shared directories
- [ ] Quick search / command-line input
- [ ] “Bring current directory/filename to command line”
- [ ] Reread associations
- [ ] User menu

## Packing

- [ ] Pack / Unpack commands (beyond browsing archives as virtual file systems)
