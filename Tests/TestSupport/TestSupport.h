...
[[nodiscard]] inline std::filesystem::path ResolveTestSandboxBase(std::error_code& ec) noexcept
{
    ...
    try
    {
        ...
        if (! configuredRoot.empty())
        {
            ...
            if (ec || ! Common::Testing::IsAuthorizedTestSandboxPath(requested, {}, false, ec))
            {
                if (! ec)
                {
        ...
        // An explicit marked root is valid for a relocated package harness with no checkout.
        // Repository discovery is needed only to choose the default drive.
