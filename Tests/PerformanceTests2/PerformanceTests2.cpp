...
TEST_CLASS(PerformanceTests2){
    public : TEST_METHOD(SplashScreenCloseGuardTriggersWhenCloseEventWasSignaled){HANDLE closeEvent = CreateEventW(nullptr, TRUE, TRUE, nullptr);
Assert::IsNotNull(closeEvent, L"failed to create close event for splash guard test");

std::stop_source stopSource;
Assert::IsTrue(SplashScreen::Detail::ShouldAbortPendingOpen(stopSource.get_token(), closeEvent),
