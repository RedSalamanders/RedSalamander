...
[[nodiscard]] bool TestImapMessageLeafNamePreferredFormat()
{
    ...
    ok = Require(FileSystemCurlInternal::BuildImapMessageLeafName(L"Quarterly report", L"boss@example.test", 12345u) == L"Quarterly report [12345].eml",
                 L"IMAP message leaf name should use '<subject> [uid].eml'.") &&
         ok;
    ok = Require(FileSystemCurlInternal::BuildImapMessageLeafName(L"", L"", 7u) == L"(no subject) [7].eml",
                 L"Empty IMAP subjects should use a stable message fallback.") &&
         ok;
    ok = Require(FileSystemCurlInternal::BuildImapMessageLeafName(L"Q4: plan/report", L"boss@example.test", 12u) == L"Q4_ plan_report [12].eml",
...
[[nodiscard]] bool TestImapUidParserAcceptsPreferredAndDirectNames()
{
    ...
    ok = Require(FileSystemCurlInternal::TryParseImapUidFromLeafName(L"Quarterly report [12345].eml", uid) && uid == 12345u,
                 L"IMAP UID parser should read the bracketed id from the preferred filename.") &&
         ok;
...
void RunPerfProbe()
{
    ...
    for (uint64_t i = 0; i < iterations; ++i)
    {
        const std::wstring built = FileSystemCurlInternal::BuildImapMessageLeafName(L"Quarterly report with detailed appendices", L"boss@example.test", i + 1u);
        checksum += built.size();
    }
