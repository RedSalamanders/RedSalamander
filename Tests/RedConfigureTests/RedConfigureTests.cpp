...
[[nodiscard]] bool TestLocalizationClipboardPasteUsesVisibleIdentitiesAtomically()
{
    ...
    ok = Require(rowA != nullptr && rowB != nullptr && rowC != nullptr && rowD != nullptr, L"Clipboard identity fixture should expose all four review rows.") &&
         ok;
    ...
    const RedConfigure::Workflow::LocalizationClipboardPasteTarget target =
        RedConfigure::Workflow::BuildLocalizationClipboardPasteTarget(rows, visibleRows, rowCIndex, visibleCultures, L"ja-JP");

    ok = Require(target.destinationRows.size() == 2u && target.destinationRows[0].id == L"IDS_ROW_C" && target.destinationRows[1].id == L"IDS_ROW_A",
    ...
    ok                           = Require(readCell(L"IDS_ROW_C", L"ja-JP") == L"C-ja pasted {0}" && readCell(L"IDS_ROW_C", L"fr-FR") == L"C-fr pasted {0}" &&
                                   ...
                                   ok;
    ok = Require(readCell(L"IDS_ROW_B", L"ja-JP") == L"B {0}" && readCell(L"IDS_ROW_D", L"ja-JP") == L"D {0}" && readCell(L"IDS_ROW_C", L"de-DE") == L"C {0}" &&
                     readCell(L"IDS_ROW_A", L"de-DE") == L"A {0}",
    ...
    ok = Require(! session.ApplyClipboardMatrix(
                     hiddenCultureTarget.destinationRows, hiddenCultureTarget.destinationCultures, L"C-ja hidden {0}\tC-fr hidden {0}\tC-de hidden {0}") &&
                     ! session.CanUndo() && session.CanRedo(),
...
[[nodiscard]] bool TestLocalizationReviewGridModelShowsOwnersEnglishAndVisibleCultures()
{
    ...
    {
        foundWarningRow = foundWarningRow || model.GetRowStyle(rowIndex).tone == DxUi::GridRowTone::Warning;
    }
...
[[nodiscard]] bool TestLocalizationReviewSurfacesMissingTranslations()
{
    ...
    ok = Require(missingViewRow.has_value() && model.GetRowStyle(missingViewRow.value()).tone == DxUi::GridRowTone::Info,
                 L"Missing-translation rows should render the distinct Missing row tone.") &&
         ok;
    ok = Require(warningViewRow.has_value() && model.GetRowStyle(warningViewRow.value()).tone == DxUi::GridRowTone::Warning,
                 L"Placeholder problems should keep warning-tone precedence over the Missing tone.") &&
         ok;
    ok = Require(cleanViewRow.has_value() && model.GetRowStyle(cleanViewRow.value()).tone == DxUi::GridRowTone::None,
...
{
    std::error_code ec;
    const auto modulePath = Common::Testing::GetCurrentModulePath(ec);
    if (! Require(! ec, L"RedConfigure UI smoke should locate its executable."))
    {
        return false;
    }
    // Load the actual product satellite: English defaults would hide an omitted adapter call.
    const auto resourcePath = modulePath.parent_path() / L"Lang" / L"RedConfigure-ja-JP.dll";
    wil::unique_hmodule resources(LoadLibraryExW(resourcePath.c_str(), nullptr, LOAD_LIBRARY_AS_DATAFILE | LOAD_LIBRARY_AS_IMAGE_RESOURCE));
    if (! Require(resources != nullptr, L"RedConfigure UI smoke requires the Japanese product satellite."))
    {
        return false;
    }
    RedConfigure::RedConfigureSession session;
    RedConfigure::Ui::RedConfigureRootCreateResult result =
        RedConfigure::Ui::CreateRedConfigureRoot(resources.get(), session, std::filesystem::path(L"C:\\RedConfigureUiSmoke"));
    ...
    size_t comboCount = 0u;
    std::vector<DxUi::Control*> pending{result.control.get()};
    while (! pending.empty())
    {
        DxUi::Control* control = pending.back();
        pending.pop_back();
        if (const auto* combo = dynamic_cast<const DxUi::ComboBox*>(control))
        {
            ++comboCount;
            ok = Require(combo->GetNoMatchesText() == L"一致する項目がありません",
                         L"Every product combo box should use its owned Japanese no-match resource.") &&
                 ok;
        }
    ...
    return Require(comboCount == 12u, L"The smoke should cover all twelve combo boxes, including both TagPicker inputs.") && ok;
}
