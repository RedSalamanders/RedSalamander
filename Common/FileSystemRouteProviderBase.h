...
public:
                                             ...
                                             FileSystemRouteFacts* facts) noexcept override
    {
        ...
        const uint64_t requiredBytes64 = WideBytes(descriptor.acceptedSeparators) + WideBytes(descriptor.providerId) +
            WideBytes(descriptor.pathProfileId) + WideBytes(descriptor.rootId);
    ...
    HRESULT STDMETHODCALLTYPE IsTransferPeerAllowed(const wchar_t* path,
                                                    FileSystemOperation operation,
                                                    ...
                                                    BOOL* allowed) noexcept override
    {
        ...
        *allowed = FALSE;
        if (! IsValidRequest(path, operation) || (operation != FILESYSTEM_COPY && operation != FILESYSTEM_MOVE) ||
            ...
            peerPluginId == nullptr || peerPluginId[0] == L'\0')
        {
            return E_INVALIDARG;
        }
        ...
        const bool permits = role == FILESYSTEM_TRANSFER_PEER_EXPORT
            ? (isCopy ? descriptor.exportCopyAll : descriptor.exportMoveAll)
                                                       ...
                                                       unsigned long* requiredArenaBytes) noexcept override
    {
        ...
        *requiredArenaBytes = 0u;
        if (arena == nullptr ||
            (arena->buffer != nullptr && reinterpret_cast<uintptr_t>(arena->buffer) % alignof(wchar_t) != 0u) ||
        ...
        if (descriptor.componentComparison == FILESYSTEM_ROUTE_COMPONENT_ORDINAL_IGNORE_CASE && ! collisionKey.empty())
        {
            ...
            const int written = LCMapStringEx(
                LOCALE_NAME_INVARIANT, LCMAP_LOWERCASE, collisionKey.data(), length, folded.data(), required, nullptr, nullptr, 0);
            if (written != required)
            {
                return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
            }
...
protected:
    ...
    virtual HRESULT BuildFileSystemRouteDescriptor(const wchar_t* path,
                                                   FileSystemOperation operation,
                                                   FileSystemRouteDescriptor& descriptor) noexcept = 0;
...
private:
    ...
    {
        const bool availabilityValid = descriptor.availability == FILESYSTEM_ROUTE_UNSUPPORTED ||
        ...
        const bool namespaceValid = descriptor.namespaceKind >= FILESYSTEM_NAMESPACE_REAL_CONTAINER &&
            descriptor.namespaceKind <= FILESYSTEM_NAMESPACE_FIXED_OBJECT_SET;
        ...
        const bool caseRenameValid = descriptor.caseOnlyRename >= FILESYSTEM_ROUTE_CASE_ONLY_SUPPORTED &&
        ...
        return ! descriptor.providerId.empty() && ! descriptor.pathProfileId.empty() && ! descriptor.rootId.empty() &&
            ! descriptor.acceptedSeparators.empty() &&
            descriptor.acceptedSeparators.find(descriptor.preferredSeparator) != std::wstring::npos &&
            descriptor.maxComponentUtf16 != 0u && descriptor.copyMoveMaxConcurrency != 0u && descriptor.deleteMaxConcurrency != 0u &&
            descriptor.deleteRecycleBinMaxConcurrency != 0u && availabilityValid && routeValid && namespaceValid && comparisonValid &&
            normalizationValid && caseRenameValid && (descriptor.proofFlags & ~knownProofFlags) == 0u && cancellationValid &&
    ...
    [[nodiscard]] static HRESULT ValidateNameShape(const FileSystemRouteDescriptor& descriptor, std::wstring_view name) noexcept
    {
        if (name.empty() || name == L"." || name == L".." ||
            name.find_first_of(descriptor.acceptedSeparators) != std::wstring_view::npos)
        {
                                                  ...
                                                  unsigned long* requiredArenaBytes) noexcept
    {
        ...
        *requiredArenaBytes = static_cast<unsigned long>(bytes64);
        if (arena->buffer == nullptr || arena->usedBytes > arena->capacityBytes ||
            bytes64 > static_cast<uint64_t>(arena->capacityBytes - arena->usedBytes))
        {
            return HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER);
        }
