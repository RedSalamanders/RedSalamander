...
namespace Common::PaneVisualState
{
...
// Overlaying the pane background with this alpha scales every rendered
// foreground difference toward that background by the standard text opacity.
[[nodiscard]] constexpr float ResolveInactiveContentOverlayAlpha(bool paneFocused, bool highContrast) noexcept
{
    return (! paneFocused && ! highContrast) ? 1.0f - kUnfocusedPaneTextOpacity : 0.0f;
}
} // namespace Common::PaneVisualState
