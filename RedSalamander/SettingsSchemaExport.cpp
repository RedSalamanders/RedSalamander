...
[[nodiscard]] HRESULT ApplyAppSpecificSettingsSchemaFilter(std::wstring_view appId, yyjson_mut_val* root) noexcept
{
    ...
    yyjson_mut_val* properties = yyjson_mut_obj_get(root, "properties");
    if (! properties || ! yyjson_mut_is_obj(properties))
    {
        return E_FAIL;
    }

    if (OrdinalString::EqualsNoCase(appId, L"RedSalamander"))
    {
        static_cast<void>(yyjson_mut_obj_remove_key(properties, "monitor"));
    }
...
HRESULT BuildAggregatedSettingsSchemaJson(std::wstring_view appId, std::span<const PluginConfigurationSchemaSource> pluginSchemas, std::string& out) noexcept
{
    ...
    if (const HRESULT hr = ApplyAppSpecificSettingsSchemaFilter(appId, root); FAILED(hr))
    {
        return hr;
    }
