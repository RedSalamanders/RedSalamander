...
static FileOperationsTheme MakeFileOperationsTheme(const NavigationViewTheme& navigationTheme, const MenuTheme& menuTheme, bool highContrast) noexcept
{
    ...
    theme.progressVerify =
        highContrast ? ColorFromCOLORREF(menuTheme.text) : (menuTheme.darkBase ? D2D1::ColorF(1.0f, 0.80f, 0.35f) : D2D1::ColorF(0.65f, 0.38f, 0.0f));
    theme.successText =
...
std::optional<uint32_t> FindAppThemeColorArgb(const AppTheme& theme, std::wstring_view key) noexcept
{
    ...
    if (key == L"fileOps.progressVerify")
        return ArgbFromColor(theme.fileOperations.progressVerify);
    ...
    if (key == L"fileOps.graphVerify")
        return ArgbFromColor(theme.fileOperations.graphVerify);
