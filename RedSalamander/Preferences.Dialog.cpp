...
std::mutex g_preferencesSettingsFileOpenDebugMutex;
PreferencesSettingsFileOpenDebugState g_preferencesSettingsFileOpenDebug;

[[nodiscard]] bool TryCapturePreferencesSettingsFileOpenForTest(const std::filesystem::path& path, const HRESULT hr) noexcept
{
    std::lock_guard lock(g_preferencesSettingsFileOpenDebugMutex);
    if (! g_preferencesSettingsFileOpenDebug.captureNext)
    {
        return false;
    }

    g_preferencesSettingsFileOpenDebug.captureNext = false;
    g_preferencesSettingsFileOpenDebug.lastPath    = path;
    g_preferencesSettingsFileOpenDebug.lastHr      = hr;
    return true;
}
#endif
...
constexpr std::array<CategoryInfo, 14> kCategories = {{
    ...
    {PrefCategory::Monitor, IDS_PREFS_CAT_MONITOR, IDS_PREFS_CAT_MONITOR_DESC},
...
struct PreferencesPageHostSurfaceControl final : RedSalamander::DxUi::Control
{
    ...
    void Paint(WindowHost& host) const override
    {
        ...
        const COLORREF borderColor  = UiMetrics::BlendColorRefWeightedTruncate(surfaceColor, _state->theme.menu.text, _state->theme.dark ? 40 : 30, 255);
...
[[nodiscard]] const wchar_t* GetPrefCategoryDebugName(const PrefCategory category) noexcept
{
    switch (category)
    {
        ...
        case PrefCategory::Monitor: return L"Monitor";
...
[[nodiscard]] bool EnsurePreferencesPageInitialized(HWND pageHostWindow, PreferencesDialogState& state, const PrefCategory category) noexcept
{
    ...
    switch (category)
    {
        ...
        case PrefCategory::Monitor: return ensurePage(hostState._monitorPane);
...
[[nodiscard]] COLORREF GetDisabledTextColor(const PreferencesDialogState& state, COLORREF background) noexcept
{
    ...
    auto blended = UiMetrics::BlendColorRefWeightedTruncate(background, normal, state.theme.dark ? 140 : 90, 255);
    if (std::abs(ColorLuma(blended) - ColorLuma(background)) < minBgDiff)
    {
        blended = UiMetrics::BlendColorRefWeightedTruncate(background, normal, state.theme.dark ? 170 : 120, 255);
    }
...
void PaintPageHostBackgroundAndCards(HDC hdc, HWND host, const PreferencesDialogState& state) noexcept
{
    ...
    const COLORREF border  = UiMetrics::BlendColorRefWeightedTruncate(surface, state.theme.menu.text, state.theme.dark ? 40 : 30, 255);
...
LRESULT CALLBACK PreferencesDxCategoryHostWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) noexcept
{
    ...
    if (msg == WM_LBUTTONDOWN && GetFocus() != hwnd)
    {
...
{
    const ThemePalette palette = MakeAppThemeDxPalette(theme);
...
void AttachPreferencesPageHostDxSurface(PreferencesDialogHost& hostState) noexcept
{
    ...
    hostState._pageHostHost.SetTheme(MakeAppThemeDxPalette(hostState.theme));
...
[[nodiscard]] bool AreEquivalentMonitorSettings(const Common::Settings::Settings& a, const Common::Settings::Settings& b) noexcept
{
    const auto& left  = GetMonitorSettingsOrDefault(a);
    const auto& right = GetMonitorSettingsOrDefault(b);
    return left.menu.toolbarVisible == right.menu.toolbarVisible && left.menu.lineNumbersVisible == right.menu.lineNumbersVisible &&
           left.menu.alwaysOnTop == right.menu.alwaysOnTop && left.menu.showIds == right.menu.showIds && left.menu.autoScroll == right.menu.autoScroll &&
           left.filter.mask == right.filter.mask && left.filter.preset == right.filter.preset;
}
...
{
    if (a.formatVersion != b.formatVersion || a.id != b.id || a.name != b.name || a.baseThemeId != b.baseThemeId || a.palette != b.palette)
    {
        return false;
    }
...
void ReloadPreferencesDialogFromDisk(HWND dlg, PreferencesDialogState& state) noexcept
{
    ...
    state.baselineSettings.monitor.reset();
    state.workingSettings = *state.settings;
    state.workingSettings.monitor.reset();
    Common::Settings::Settings monitorSettings{};
    static_cast<void>(Common::Settings::LoadSettings(kPreferencesMonitorAppId, monitorSettings));
    state.monitorBaselineSettings = monitorSettings;
    state.workingMonitorSettings  = monitorSettings;
...
void ResetAllPreferencesToDefaults(HWND dlg, PreferencesDialogState& state) noexcept
{
    ...
    state.workingMonitorSettings = Common::Settings::Settings{};
...
#ifdef ENABLE_TESTS
    const bool forceDiscardForAutoAccept = HostGetAutoAcceptPrompts() && HostGetTestPromptResultOverride() == HOST_PROMPT_RESULT_NONE;
...
[[nodiscard]] bool RequestPreferencesDialogCancelClose(HWND dlg, PreferencesDialogState& state) noexcept
{
    ...
    switch (PromptSaveDirtyPreferencesBeforeClose(dlg, state))
    {
        case HOST_PROMPT_RESULT_YES:
            ...
            return RequestPreferencesDialogClose(dlg);
        case HOST_PROMPT_RESULT_NO: RestorePreviewAppliedPreferencesOnCancel(state); return RequestPreferencesDialogClose(dlg);
...
void RefreshPreferencesDialogThemeImpl(HWND dlg, PreferencesDialogState& state) noexcept
{
    ...
    {
        hostState._categoryTreeHost.SetTheme(MakeAppThemeDxPalette(state.theme));
    }
    ...
    {
        hostState._pageHostHost.SetTheme(MakeAppThemeDxPalette(state.theme));
    }
...
void LayoutPreferencesPageHost(HWND host, PreferencesDialogState& state) noexcept
{
    ...
    hostState._monitorPane.OnVisibilityChanged(showMonitor);
    ...
    if (showMonitor)
    {
        hostState._monitorPane.LayoutPage(host, state, x, y, width, margin, gapY, pageTypography);
        FinalizePreferencesPageHostLayout(host, state, margin, width);
        return;
    }
...
void RefreshMonitorPage(HWND host, PreferencesDialogState& state) noexcept
{
    static_cast<PreferencesDialogHost&>(state)._monitorPane.Refresh(host, state);
}
...
void RefreshActivePreferencesPage(HWND host, PreferencesDialogState& state) noexcept
{
    ...
    switch (state.currentCategory)
    {
        ...
        case PrefCategory::Monitor: RefreshMonitorPage(host, state); break;
...
void UpdatePageText(HWND dlg, PreferencesDialogState& state, PrefCategory category) noexcept
{
    ...
    // Repaint only the surfaces whose page content changed. Redrawing every
    // child also repaints the category tree after its input/selection paint,
    // creating an unrelated fourth frame for a single category transition.
    RedrawWindow(dlg, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW);
    if (hostState._shellHostHwnd && IsWindow(hostState._shellHostHwnd) != FALSE)
    {
        RedrawWindow(hostState._shellHostHwnd, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW);
    }
    if (state.pageHostWindow && IsWindow(state.pageHostWindow) != FALSE)
    {
        RedrawWindow(state.pageHostWindow, nullptr, nullptr, RDW_INVALIDATE | RDW_ALLCHILDREN | RDW_UPDATENOW);
    }

    if (restoreCategoryTreeFocus && state.categoryTreeWindow && IsWindow(state.categoryTreeWindow) != FALSE)
    {
        static_cast<void>(SetFocus(state.categoryTreeWindow));
        if (PostMessageW(dlg, WndMsg::kPreferencesRestoreCategoryTreeFocus, 0, 0) == FALSE)
        {
            Debug::Warning(L"Preferences: failed to queue deferred category-tree focus restoration");
        }
    }
}
...
LRESULT CALLBACK PreferencesPageHostWindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) noexcept
{
    ...
    switch (msg)
    {
        ...
        case WndMsg::kPreferencesDeferredPaneAction:
            ...
            return FALSE;
        case WndMsg::kPreferencesApplyPageHostScroll: PrefsPageHost::FlushPendingScroll(hwnd, *state); return 0;
        case WM_COMMAND:
        {
...
INT_PTR OnInitDialog(HWND dlg, PreferencesDialogState* state)
{
    ...
    if (GetWindowRect(dlg, &initial))
    {
        ...
        state->restoreMinSizePx  = state->minTrackSizePx;
    ...
    // original Win32 statics. Grow the dialog once to accommodate and update min size.
    {
        ...
        if (hostInit._shellHostHwnd)
        {
            ...
            if (GetWindowRect(dlg, &wr))
            {
                ...
                state->restoreMinSizePx.cy += chromeExtraPx;
            }
        }
    }
...
[[nodiscard]] std::wstring_view GetPreferencesSettingsFileAppId(const PreferencesDialogState& state, const PreferencesSettingsFileTarget target) noexcept
{
    return target == PreferencesSettingsFileTarget::Monitor ? kPreferencesMonitorAppId : std::wstring_view{state.appId};
}

[[nodiscard]] const Common::Settings::Settings& GetPreferencesSettingsFileSourceSettings(const PreferencesDialogState& state,
                                                                                         const PreferencesSettingsFileTarget target) noexcept
{
    return target == PreferencesSettingsFileTarget::Monitor ? state.monitorBaselineSettings : state.baselineSettings;
}

[[nodiscard]] HRESULT OpenPreferencesSettingsFile(HWND owner,
                                                  const PreferencesDialogState& state,
                                                  const PreferencesSettingsFileTarget target,
                                                  std::filesystem::path& outPath) noexcept
{
    outPath.clear();

    const std::wstring_view appId = GetPreferencesSettingsFileAppId(state, target);
    if (appId.empty())
    {
        return E_INVALIDARG;
    }
...
{
    Debug::Error(L"Preferences: failed to open settings file '{}': 0x{:08X}", path.wstring(), static_cast<unsigned int>(hr));

    const std::wstring message = FormatStringResource(nullptr, IDS_FMT_PREFS_OPEN_SETTINGS_FILE_FAILED, path.wstring(), static_cast<unsigned long>(hr));
    const std::wstring caption = LoadStringResource(nullptr, IDS_CAPTION_ERROR);
    static_cast<void>(MessageBoxW(dlg, message.c_str(), caption.c_str(), MB_OK | MB_ICONERROR));
}
...
INT_PTR OnCommand(HWND dlg, PreferencesDialogState* state, UINT commandId, [[maybe_unused]] UINT notifyCode, [[maybe_unused]] HWND hwndCtl)
{
    ...
    switch (commandId)
    {
        ...
        case IDC_PREFS_ADV_OPEN_SETTINGS_FILE:
        {
            std::filesystem::path path;
            const HRESULT hr = OpenPreferencesSettingsFile(dlg, *state, PreferencesSettingsFileTarget::Main, path);
            if (FAILED(hr))
            {
                ShowPreferencesSettingsFileOpenFailure(dlg, path, hr);
            }
            return TRUE;
        }
        case IDC_PREFS_MONITOR_OPEN_SETTINGS_FILE:
        {
            std::filesystem::path path;
            const HRESULT hr = OpenPreferencesSettingsFile(dlg, *state, PreferencesSettingsFileTarget::Monitor, path);
            if (FAILED(hr))
            {
                ShowPreferencesSettingsFileOpenFailure(dlg, path, hr);
            }
...
INT_PTR CALLBACK PreferencesDialogProc(HWND dlg, UINT msg, WPARAM wp, LPARAM lp) noexcept
{
    ...
    switch (msg)
    {
        ...
        case WndMsg::kPreferencesDeferredPaneAction:
            ...
            return FALSE;
        case WndMsg::kPreferencesRestoreCategoryTreeFocus:
            if (state && GetActiveWindow() == dlg && state->categoryTreeWindow && IsWindow(state->categoryTreeWindow) != FALSE)
            {
                const HWND focus = GetFocus();
                const bool pageHostOwnsFocus =
                    state->pageHostWindow && (focus == state->pageHostWindow || (focus && IsChild(state->pageHostWindow, focus) != FALSE));
                if (! focus || pageHostOwnsFocus)
                {
                    static_cast<void>(SetFocus(state->categoryTreeWindow));
                }
            }
    ...
    HWND owner, std::wstring_view appId, Common::Settings::Settings& settings, const AppTheme& theme, PrefCategory initialCategory) noexcept
{
    ...
    const std::optional<Common::Settings::MonitorSettings> legacyMainMonitorSettings = settings.monitor;
    ...
    state->baselineSettings.monitor.reset();
    state->workingSettings = state->baselineSettings;
    settings.monitor.reset();

    Common::Settings::Settings monitorSettings{};
    static_cast<void>(Common::Settings::LoadSettings(kPreferencesMonitorAppId, monitorSettings));
    if (! monitorSettings.monitor.has_value() && legacyMainMonitorSettings.has_value())
    {
        monitorSettings.monitor = legacyMainMonitorSettings.value();
    }
    state->monitorBaselineSettings = monitorSettings;
    state->workingMonitorSettings  = monitorSettings;
...
bool PreferencesDialog::DebugGetSnapshot(::PreferencesDebugSnapshot& out) noexcept
{
    ...
    if (const auto* scrollPanel = dynamic_cast<const ScrollPanel*>(static_cast<const PreferencesDialogHost*>(state)->_pageHostContentRootControl))
    {
        ...
        const auto dipToPx = [dpi](const float value) noexcept { return static_cast<LONG>(std::lround((value * static_cast<float>(dpi)) / 96.0f)); };
        if (state->pageHostWindow)
        {
            RECT pageClient{};
            if (GetClientRect(state->pageHostWindow, &pageClient))
            {
                ...
                const int thicknessPx            = static_cast<int>(std::lround((scrollPanel->GetScrollbarThickness() * static_cast<float>(dpi)) / 96.0f));
    ...
    if (foregroundWindow)
    {
        ...
        out.foregroundProcessId = foregroundProcessId;
    }
    if (nativeFocus)
    {
        std::array<wchar_t, 128> className{};
        if (GetClassNameW(nativeFocus, className.data(), static_cast<int>(className.size())) > 0)
        {
            out.nativeFocusClassName.assign(className.data());
        }
    }
    ...
    if (auto& hostState = static_cast<PreferencesDialogHost&>(*state); hostState._shellHostHwnd)
    {
        ...
        {
            return rect.right > rect.left && rect.bottom > rect.top && rect.left >= 0 && rect.top >= 0 && rect.right <= out.shellHostClientWidthPx &&
                   rect.bottom <= out.shellHostClientHeightPx;
        };
        out.shellFooterButtonsInsideHost =
            rectInsideHost(out.shellOkButtonBoundsPx) && rectInsideHost(out.shellCancelButtonBoundsPx) && rectInsideHost(out.shellApplyButtonBoundsPx);
        ...
        if (hostState._shellHost.DebugCaptureBitmap(capture))
        {
            const auto samplePixel = [&](const int x, const int y, uint32_t& outPixel) noexcept
            {
                ...
                outPixel = static_cast<uint32_t>(capture.bgraPixels[base + 0u]) | (static_cast<uint32_t>(capture.bgraPixels[base + 1u]) << 8u) |
            ...
            const auto colorComponent = [](const float value) noexcept { return static_cast<uint8_t>(std::clamp(std::lround(value * 255.0f), 0l, 255l)); };
            const ThemePalette expectedPalette   = MakeAppThemeDxPalette(state->theme);
            ...
            {
                const std::array buttonRects{out.shellOkButtonBoundsPx, out.shellCancelButtonBoundsPx, out.shellApplyButtonBoundsPx};
                ...
                if (insideAnyButton(sampleX, sampleY))
                {
                    ...
                    for (int candidateX = kFooterBackgroundSamplePadding; candidateX < out.shellHostClientWidthPx - kFooterBackgroundSamplePadding;
                         ++candidateX)
                    {
    ...
    if (state->categoryTreeUsesDxUi)
    {
        ...
        out.categoryTreeDxHostInvalidateCount   = hostState._categoryTreeHost.DebugGetInvalidateCount();
        out.categoryTreeDxHostHasResizeFailures = GetDxHostDebugResizeFailureCount(hostState._categoryTreeHost) != 0u;
        out.categoryTreeDxHostFocusControlActive =
            hostState._categoryTreeControl && hostState._categoryTreeHost.GetFocusControl() == hostState._categoryTreeControl;
        if (hostState._categoryTreeControl)
        {
        ...
        out.monitorSettingsFileCardLast                 = hostState._monitorPane.DebugSettingsFileCardIsLast();
        ...
        out.compareDirectoriesVisibleSectionHeaderCount = hostState._compareDirectoriesPane.DebugVisibleSectionHeaderCount();
...
bool PreferencesDialog::DebugSelectCategory(const PrefCategory category) noexcept
{
    ...
    if (state->categoryTreeUsesDxUi)
    {
        ...
        for (size_t visibleIndex = 0u; visibleIndex < visibleItemCount; ++visibleIndex)
        {
            ...
            return DebugGetSnapshot(snapshot) && snapshot.currentCategory == category && ! snapshot.pluginItemSelected;
        }
    ...
    PreferencesDebugSnapshot snapshot{};
    if (DebugGetSnapshot(snapshot) && snapshot.currentCategory == category)
    {
        return true;
    }
...
bool PreferencesDialog::DebugFocusCategoryTree() noexcept
{
    const HWND dlg = GetHandle();
    if (! dlg || IsWindow(dlg) == FALSE)
    {
    ...
    auto* state = GetState(dlg);
    if (! state || ! state->categoryTreeUsesDxUi || ! state->categoryTreeWindow || IsWindow(state->categoryTreeWindow) == FALSE)
    {
        return false;
    }
    ...
    hostState._categoryTreeHost.ResetInteractionState();
    hostState._categoryTreeHost.SetFocusControl(hostState._categoryTreeControl);
    return GetFocus() == state->categoryTreeWindow && hostState._categoryTreeHost.GetFocusControl() == hostState._categoryTreeControl;
}

bool PreferencesDialog::DebugSendCategoryTreeKey(const UINT virtualKey) noexcept
{
    const HWND dlg = GetHandle();
    if (! dlg || IsWindow(dlg) == FALSE)
    {
    ...
    auto& hostState = static_cast<PreferencesDialogHost&>(*state);
    if (! hostState._categoryTreeControl || ! DebugFocusCategoryTree())
    {
        return false;
    }
    ...
    {
        keyDownLParam |= static_cast<LPARAM>(1ull << 24u);
    }
    const LPARAM keyUpLParam = keyDownLParam | static_cast<LPARAM>(1ull << 30u) | static_cast<LPARAM>(1ull << 31u);
    static_cast<void>(SendMessageW(state->categoryTreeWindow, WM_KEYDOWN, virtualKey, keyDownLParam));
    static_cast<void>(SendMessageW(state->categoryTreeWindow, WM_KEYUP, virtualKey, keyUpLParam));
    return true;
}
...
bool PreferencesDialog::DebugFocusMonitorToolbarToggle() noexcept
{
    ...
    auto& hostState = static_cast<PreferencesDialogHost&>(*state);
    return hostState._monitorPane.DebugFocusToolbarToggle();
}

bool PreferencesDialog::DebugSelectMonitorFilterPreset(std::wstring_view displayText) noexcept
{
    const HWND dlg = GetHandle();
    if (! dlg)
    {
    ...
    auto& hostState = static_cast<PreferencesDialogHost&>(*state);
    return hostState._monitorPane.DebugSelectFilterPresetByText(displayText);
}

void PreferencesDialog::DebugSetSettingsFileOpenCapture(const bool capture) noexcept
{
    std::lock_guard lock(g_preferencesSettingsFileOpenDebugMutex);
    g_preferencesSettingsFileOpenDebug.captureNext = capture;
}

void PreferencesDialog::DebugClearLastSettingsFileOpen() noexcept
{
    std::lock_guard lock(g_preferencesSettingsFileOpenDebugMutex);
    g_preferencesSettingsFileOpenDebug.lastPath.clear();
    g_preferencesSettingsFileOpenDebug.lastHr = S_FALSE;
}

bool PreferencesDialog::DebugGetLastSettingsFileOpen(std::filesystem::path& outPath, HRESULT& outHr) noexcept
{
    std::lock_guard lock(g_preferencesSettingsFileOpenDebugMutex);
    if (g_preferencesSettingsFileOpenDebug.lastPath.empty() && g_preferencesSettingsFileOpenDebug.lastHr == S_FALSE)
    {
        return false;
    }

    outPath = g_preferencesSettingsFileOpenDebug.lastPath;
    outHr   = g_preferencesSettingsFileOpenDebug.lastHr;
    return true;
}
