...
template <typename CreateOrUpdate> [[nodiscard]] uint64_t ResolveChangeCaseTaskUpdate(ChangeCaseTaskPayload& payload, CreateOrUpdate&& createOrUpdate) noexcept
{
    if (payload.receipt)
    {
    ...
    if (payload.receipt->taskId.compare_exchange_strong(expectedTaskId, resolvedTaskId, std::memory_order_acq_rel, std::memory_order_acquire))
    {
        return resolvedTaskId;
    }
