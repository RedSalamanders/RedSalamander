...
[[nodiscard]] inline bool CommandRuntimeIdentityMatches(
    const CommandRuntimeState& captured, const CommandRuntimeState& current) noexcept
{
    return captured.terminalIdentityPresent == current.terminalIdentityPresent &&
        (! captured.terminalIdentityPresent ||
         (captured.terminalSessionGeneration == current.terminalSessionGeneration &&
          std::memcmp(captured.terminalInstanceId.bytes,
                      current.terminalInstanceId.bytes,
                      sizeof(captured.terminalInstanceId.bytes)) == 0));
}
...
[[nodiscard]] bool ResolveCommandRuntimeStateFromWindow(HWND ownerWindow,
