...
{
    return query.size() <= text.size() && CompareStringOrdinal(text.data(), static_cast<int>(query.size()),
...
public:
    ...
    void SetRows(std::vector<PaletteRow> rows) { _rows = std::move(rows); }
    [[nodiscard]] size_t GetRowCount() const noexcept override { return _rows.size(); }
    [[nodiscard]] size_t GetColumnCount() const noexcept override { return _columns.size(); }
    [[nodiscard]] GridColumnDesc GetColumn(size_t index) const override { return _columns.at(index); }
    void GetCellData(size_t rowIndex, size_t columnIndex, GridCellData& cell) const override
    {
        cell = {};
        if (rowIndex >= _rows.size()) return;
    ...
    {
        return index < _rows.size() ? RedSalamander::DxUi::GridRowStyle{.rainbowSeed = _rows[index].command.commandId}
                                    : RedSalamander::DxUi::GridRowStyle{};
    }
    [[nodiscard]] uint64_t GetStableRowId(size_t index) const noexcept override { return index + 1u; }
    ...
    [[nodiscard]] const PaletteRow* GetRow(size_t index) const noexcept { return index < _rows.size() ? &_rows[index] : nullptr; }
...
public:
    ...
    [[nodiscard]] HWND Create(HWND owner,
    ...
    void OnGridRowActivated(Grid&, size_t index) override { Activate(index); }
    void OnGridRowActivated(size_t index) override { Activate(index); }
#ifdef ENABLE_TESTS
...
HWND CommandPaletteWindow::Create(HWND owner,
                                  const Common::Settings::ShortcutsSettings& shortcuts,
                                  bool terminalContext,
                                  const AppTheme& theme) noexcept
{
    ...
    if (! RegisterClass(instance)) return nullptr;
    ...
    if (! _window) _window.reset(hwnd);
    ShowWindow(hwnd, SW_SHOWNORMAL);
    SetForegroundWindow(hwnd);
    Debug::Perf::EmitDurationUs(L"app.command_palette.open_to_visible_us",
                                Debug::Perf::ElapsedUs(openStartedAt), _catalog.size(), 0u);
    return hwnd;
}
...
LRESULT CALLBACK CommandPaletteWindow::WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) noexcept
{
    if (message == WM_NCCREATE)
    {
        ...
        if (! self->_window) self->_window.reset(hwnd);
    }
...
LRESULT CommandPaletteWindow::HandleMessage(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) noexcept
{
    ...
    if (message == WM_KEYDOWN && wParam == VK_RETURN && _grid && _model)
    {
        ...
        if (selected.has_value()) Activate(selected.value());
        return 0;
    }
    ...
    switch (message)
    {
        ...
        case WM_NCDESTROY:
            _host.Detach();
            if (_window.get() == hwnd) _window.release();
...
void CommandPaletteWindow::RebuildRows()
{
    ...
    for (const auto& command : _catalog)
    {
        ...
        {
            if (! shortcuts.empty()) shortcuts.append(L"  ");
            shortcuts.append(L"‹");
        ...
        if (! _search.empty() && ! titleMatch && ! shortcutMatch && ! ContainsNoCase(command.searchText, _search)) continue;
        PaletteRow row;
        row.command = command;
        ...
        if (! command.description.empty()) { row.commandText.append(L"\n"); row.commandText.append(command.description); }
    ...
    if (_emptyLabel != nullptr) _emptyLabel->SetVisible(_model->GetRowCount() == 0u);
    if (_model->GetRowCount() != 0u) static_cast<void>(_grid->RequestSelectRow(0u, 0u));
    _host.Invalidate();
    Debug::Perf::EmitDurationUs(L"app.command_palette.filter_batch_us",
                                Debug::Perf::ElapsedUs(filterStartedAt), _catalog.size(), _model->GetRowCount());
}
...
void CommandPaletteWindow::Layout() noexcept
{
    ...
    {
        _emptyLabel->SetBounds(D2D1::RectF(margin + 18.0f,
                                           margin + searchHeight + 34.0f,
                                           bounds.right - margin - 18.0f,
                                           margin + searchHeight + 74.0f));
    }
}
...
void CommandPaletteWindow::ApplyTheme() noexcept
{
    ...
    if (_window) ApplyWindowBackdropTheme(_window.get(), _theme, WindowBackdropTarget::Tool);
}
...
void CommandPaletteWindow::Activate(size_t index) noexcept
{
    ...
    _window.reset();
    if (owner && IsWindow(owner) != FALSE) static_cast<void>(DispatchShortcutCommandFromWindow(owner, commandId));
}
...
{
    out = {};
    if (! _window || ! _model || ! _grid) return false;
                              ...
                              const AppTheme& theme) noexcept
{
    ...
    if (! g_palette->Create(owner, shortcuts, terminalContext, theme)) g_palette.reset();
}

void UpdateCommandPaletteWindowTheme(const AppTheme& theme) noexcept
{
    if (g_palette && g_palette->GetHwnd()) g_palette->UpdateTheme(theme);
}
