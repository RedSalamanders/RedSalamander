...
public:
    ...
    {
        _columns = {
            {L"command", LoadStringResource(nullptr, IDS_SHORTCUTS_COL_COMMAND), 530.0f, 260.0f, DxUi::GridColumnKind::Text, false, false},
            {L"shortcut", LoadStringResource(nullptr, IDS_SHORTCUTS_COL_KEY), 250.0f, 120.0f, DxUi::GridColumnKind::Text, false, false},
        };
    }
    ...
    void GetCellData(size_t rowIndex, size_t columnIndex, GridCellData& cell) const override
    {
        ...
        if (columnIndex == 0u)
        {
            cell.kind        = DxUi::GridCellKind::IconText;
    ...
    [[nodiscard]] DxUi::GridRowStyle GetRowStyle(size_t index) const override
    {
        return index < _rows.size() ? DxUi::GridRowStyle{.rainbowSeed = _rows[index].command.commandId} : DxUi::GridRowStyle{};
    }
...
LRESULT CommandPaletteWindow::HandleMessage(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) noexcept
{
    ...
    switch (message)
    {
        ...
        case WM_NCDESTROY:
            _host.Detach();
            // Detach destroys the control tree the host owned. This object outlives its window
            // (the singleton is only replaced on the next Show), so every pointer into that tree
            // goes with it; the debug hooks and RebuildRows guard on these being null.
            _root        = nullptr;
...
{
    // Refuse once the window is gone rather than write into a destroyed search field; a palette
    // that lost activation mid-case used to take the Commands suite down here.
    if (! _window || ! _searchField)
        return false;
