...
[[nodiscard]] bool TestSelectionSelectAllKeepsNavigationShellStable(HWND mainWindow, CaseState& state) noexcept
{
    ...
    const auto waitForStableBaseline = [&]() noexcept
    {
        ...
        while (std::chrono::steady_clock::now() < deadline)
        {
            ...
            const bool ready = g_folderWindow.DebugGetNavigationViewSnapshot(FolderWindow::Pane::Left, snapshot) &&
                               snapshot.focusTarget == NavigationViewDebugFocusTarget::None && ! snapshot.editMode && ! snapshot.historyDropdownVisible &&
    ...
    state.Require(waitForStableBaseline(), L"Failed to capture the baseline navigation-view state before select-all shell-stability validation.");
    if (! state.failure.empty())
    {
        return false;
    }
    ...
    const auto requireStableNavigationShell = [&](size_t expectedSelectedCount, std::wstring_view context) noexcept
    {
        ...
        if (g_folderWindow.GetFocusedFolderViewHwnd() != folderView)
        {
            ...
            {
                static_cast<void>(
                    state.Skip(std::format(L"{} focus retention requires foreground ownership; another process retained the desktop foreground.", context)));
                return false;
            }
...
[[nodiscard]] bool TestSelectionSelectNextKeepsNavigationShellStable(HWND mainWindow, CaseState& state) noexcept
{
    ...
    state.Require(waitForStableBaseline(), L"Failed to capture the baseline navigation-view state before select-next shell-stability validation.");
    if (! state.failure.empty())
    {
        return false;
    }
...
[[nodiscard]] bool TestSelectionSelectCalculateDirectorySizeNextKeepsNavigationShellStable(HWND mainWindow, CaseState& state) noexcept
{
    ...
    const FolderWindow::DebugSelectionSizeSnapshot folderSizeBefore = g_folderWindow.DebugGetSelectionSizeSnapshot(FolderWindow::Pane::Left);
    SendMessageW(mainWindow, WM_COMMAND, MAKEWPARAM(IDM_PANE_SELECT_CALC_DIR_SIZE_NEXT, 0), 0);
    ...
    state.Require(computedFolderSize.requestCount >= folderSizeBefore.requestCount + 1u && computedFolderSize.lastRequestedSelectedPaths.size() == 1u &&
                      ...
                      computedFolderSize.lastRequestedFolderPaths.front().filename() == L"size_folder" && computedFolderSize.folderBytesValid &&
                      computedFolderSize.folderBytes == 7u,
    ...
    const std::optional<POINT> selectedFolderPoint = g_folderWindow.DebugGetPaneItemCenterClientPointForSelfTest(FolderWindow::Pane::Left, L"size_folder");
    ...
    FolderWindow::DebugSelectionSizeSnapshot canceledFolderSize = g_folderWindow.DebugGetSelectionSizeSnapshot(FolderWindow::Pane::Left);
    state.Require(canceledFolderSize.generation > completedFolderGeneration && ! canceledFolderSize.folderBytesPending &&
                      ! canceledFolderSize.folderBytesValid && canceledFolderSize.folderBytes == 0u && canceledFolderSize.lastRequestedSelectedPaths.empty(),
                  L"A second Space did not deselect the folder and cancel/exclude its size contribution.");
    state.Require(g_folderWindow.DebugPostSelectionSizeCompletionForSelfTest(FolderWindow::Pane::Left, completedFolderGeneration, 999u, S_OK),
...
[[nodiscard]] bool TestOpenDriveMenuKeepsNavigationShellStable(HWND mainWindow, CaseState& state) noexcept
{
    ...
    state.Require(baselineSnapshot.menuFallbackGlyph == FluentIcons::kBulletedList || baselineSnapshot.menuFallbackGlyph == FluentIcons::kFallbackBulletedList,
                ...
                if (popup != nullptr || inMenuMode)
                {
                    ...
                    if (popup != nullptr)
                    {
                        ...
                        for (size_t itemIndex = 0u; itemIndex < 32u; ++itemIndex)
                        {
                            ...
                            if (! DxUi::DebugGetContextMenuPopupItemText(popup, itemIndex, itemText))
                            {
                                break;
                            }
                            ...
                            DxUi::ContextMenuPopupItemLayoutDebugState itemLayout{};
                            const bool hasLayout = DxUi::DebugGetContextMenuPopupItemLayout(popup, itemIndex, itemLayout);
                        ...
                        {
                            DxUi::ContextMenuPopupItemLayoutDebugState goToLayout{};
                            menuResult.goToEntryHasSubmenu = DxUi::DebugGetContextMenuPopupItemLayout(popup, goToIndex.value(), goToLayout) &&
    ...
    try
    {
        std::jthread closer([&](std::stop_token stopToken) noexcept
        {
            const auto inspectCommonFoldersSubmenu = [&](HWND submenu) noexcept -> bool
            {
                ...
                {
                    std::wstring itemText;
                    if (! DxUi::DebugGetContextMenuPopupItemText(submenu, itemIndex, itemText))
                    {
                        break;
                    }
                    ...
                    for (; nextExpectedLabelIndex < commonFolderLabels.size(); ++nextExpectedLabelIndex)
                    {
                        ...
                        if (itemText == commonFolderLabels[nextExpectedLabelIndex])
                        {
                            ...
                            DxUi::ContextMenuPopupItemLayoutDebugState itemLayout{};
                            if (DxUi::DebugGetContextMenuPopupItemLayout(submenu, itemIndex, itemLayout) && itemLayout.hasBitmapIcon)
                            {
                                ++bitmapRows;
                            }
                            break;
            ...
            {
                DxUi::ContextMenuPopupDebugState popupState{};
                if (! DxUi::DebugGetContextMenuPopupState(popup, popupState) || ! popupState.keyboardIndex.has_value())
                {
                    return false;
                }
                ...
                if (popup != nullptr || inMenuMode)
                {
                    ...
                    if (popup != nullptr)
                    {
                        ...
                        if (commonFoldersIndex.has_value())
                        {
                            ...
                            DxUi::ContextMenuPopupItemLayoutDebugState itemLayout{};
                            menuResult.commonFoldersHasSubmenu = DxUi::DebugGetContextMenuPopupItemLayout(popup, commonFoldersIndex.value(), itemLayout) &&
...
{
    return WaitForClipboardQuietForSelfTest(ownerWindow) && DxUi::DebugWriteClipboardUnicodeText(ownerWindow, text) &&
           WaitForClipboardQuietForSelfTest(ownerWindow);
}
...
[[nodiscard]] bool TestChangeDirectoryEditClipboardAccelerators(HWND mainWindow, CaseState& state) noexcept
{
    ...
    NavigationViewDebugSnapshot copySnapshot{};
    const bool capturedCopySnapshot = g_folderWindow.DebugGetNavigationViewSnapshot(FolderWindow::Pane::Left, copySnapshot);
    state.Require(copiedText == selectedText,
                  std::format(L"Ctrl+C/Copy command should copy the selected address-bar edit text while edit mode is active. "
                              L"expectedLength={} actualLength={} snapshot={} editMode={} focusTarget={} host={} input={} focus={} "
                              L"selection={}:{} selected={} clipboardOpen={} clipboardOwner={} unicode={} fileDrop={}.",
                              selectedText.size(),
                              copiedText.size(),
                              capturedCopySnapshot,
                              copySnapshot.editMode,
                              static_cast<int>(copySnapshot.focusTarget),
                              reinterpret_cast<uintptr_t>(copySnapshot.currentEditHostHwnd),
                              reinterpret_cast<uintptr_t>(copySnapshot.currentEditInputHwnd),
                              reinterpret_cast<uintptr_t>(GetFocus()),
                              copySnapshot.currentEditSelectionStart,
                              copySnapshot.currentEditSelectionEnd,
                              copySnapshot.currentEditHasSelection,
                              reinterpret_cast<uintptr_t>(GetOpenClipboardWindow()),
                              reinterpret_cast<uintptr_t>(GetClipboardOwner()),
    ...
    {
        selectAllEditText();
        state.Require(waitForSelectedEditText(selectionBeforePaste), L"Address-bar edit selection should remain selected immediately before paste validation.");
    }
    if (! state.failure.empty())
    {
        return false;
    ...
    const bool pasteApplied                 = WaitForNavigationViewSnapshot(FolderWindow::Pane::Left, [&](const NavigationViewDebugSnapshot& value) noexcept {
        return value.editMode && value.currentEditHostHwnd == editHost && value.currentEditText == pastedText && ! value.currentEditHasSelection;
    }, SelfTest::Scale(3000ms), &pastedSnapshot);
...
[[nodiscard]] bool TestRefreshKeepsNavigationShellStable(HWND mainWindow, CaseState& state) noexcept
{
    ...
    // A first path match can precede queued provider/focus notifications. Observe
    // ten unchanged 20 ms samples before taking the counters protected by Refresh.
    uint64_t previousRefreshCount = 0u;
    size_t previousItemCount      = 0u;
    size_t previousSelectedCount  = 0u;
    size_t previousHistoryCount   = 0u;
    size_t stableSamples          = 0u;
    ...
    state.Require(WaitForNavigationViewSnapshot(FolderWindow::Pane::Left,
                                                [&](const NavigationViewDebugSnapshot& value) noexcept
    {
        const uint64_t refreshCount = g_folderWindow.DebugGetForceRefreshCount(FolderWindow::Pane::Left);
        const size_t itemCount      = g_folderWindow.DebugGetItemCount(FolderWindow::Pane::Left);
        const size_t selectedCount  = g_folderWindow.DebugGetSelectedCount(FolderWindow::Pane::Left);
        const bool ready            = value.focusTarget == NavigationViewDebugFocusTarget::None && ! value.editMode && ! value.historyDropdownVisible &&
                                      ! value.editSuggestPopupVisible && ! value.fullPathPopupVisible && ! value.fullPathPopupEditMode &&
                                      value.visibleChildWindowCount == 0u && value.currentPathText == root.wstring() && GetFocus() == folderView &&
                                      g_folderWindow.GetFocusedFolderViewHwnd() == folderView &&
                                      g_folderWindow.DebugGetFocusedItemDisplayName(FolderWindow::Pane::Left) == L"a.txt" &&
                                      ! g_folderWindow.DebugIsNameFilterActive(FolderWindow::Pane::Left);
        const bool unchanged        = ready && havePreviousSample && refreshCount == previousRefreshCount && itemCount == previousItemCount &&
                                      selectedCount == previousSelectedCount && value.historyCount == previousHistoryCount;
        stableSamples               = unchanged ? stableSamples + 1u : 0u;
        previousRefreshCount        = refreshCount;
        previousItemCount           = itemCount;
        previousSelectedCount       = selectedCount;
        previousHistoryCount        = value.historyCount;
        havePreviousSample          = ready;
        return stableSamples >= 10u;
    },
                                                SelfTest::Scale(3000ms),
                                                &baselineSnapshot),
    ...
    const uint64_t baselineRefreshCount = previousRefreshCount;
    const size_t baselineItemCount      = previousItemCount;
    const size_t baselineSelectedCount  = previousSelectedCount;
    ...
    state.Require(shellStable,
                  std::format(L"Navigation shell did not stay quiet during Refresh; focusTarget={}, editMode={}, historyVisible={}, suggestVisible={}, "
                              L"popupVisible={}, childWindows={}, currentPath='{}', historyCount={}, refreshCount={}, itemCount={}, selectedCount={}, "
                              L"nameFilterActive={}, focusedItem='{}', baselinePath='{}', baselineHistoryCount={}, baselineRefreshCount={}, "
                              L"baselineItemCount={}, baselineSelectedCount={}, focusedFolderViewMatch={}.",
...
[[nodiscard]] bool TestDeleteRehomesCurrentItemToNextOrPrevious(CaseState& state) noexcept
{
    ...
    g_folderWindow.SetFolderPath(FolderWindow::Pane::Left, root);
    state.Require(WaitForPanePath(FolderWindow::Pane::Left, root, SelfTest::Scale(3000ms)), L"Failed to set pane path for delete-focus test.");
    state.Require(WaitForAtomicAtLeast(enumerationCount, 1u, SelfTest::Scale(3000ms)), L"Initial delete-focus enumeration did not complete.");
    ...
    const auto waitForRemovalFocus =
        [&](uint32_t expectedCompletionCount, const std::filesystem::path& removedPath, size_t expectedItemCount, std::wstring_view expectedFocus) noexcept
    {
        ...
        while (std::chrono::steady_clock::now() < deadline)
        {
            ...
            if (completedDeletes.load(std::memory_order_acquire) >= expectedCompletionCount && SUCCEEDED(lastDeleteResult.load(std::memory_order_acquire)) &&
                removed && g_folderWindow.DebugGetItemCount(FolderWindow::Pane::Left) == expectedItemCount &&
                g_folderWindow.DebugGetFocusedItemDisplayName(FolderWindow::Pane::Left) == expectedFocus &&
                g_folderWindow.DebugGetSelectedCount(FolderWindow::Pane::Left) == 0u)
            {
                return true;
...
[[nodiscard]] bool TestStatusBarKeepsNavigationShellStable(HWND mainWindow, CaseState& state) noexcept
{
    ...
    IDWriteFactory* statusMeasureFactory = DxUi::Typography::GetSharedMeasurementFactory();
    wil::com_ptr<IDWriteTextFormat> statusTextFormat;
    if (statusMeasureFactory)
    {
        static_cast<void>(DxUi::Typography::CreateTextFormat(
            statusMeasureFactory, DxUi::Typography::MakeUiTextSpec(statusAfterDown.textSizeDip), statusTextFormat.put(), L""));
    }
    ...
    const UINT statusDpi      = DxUi::Typography::GetEffectiveDpi(statusBarHwnd);
    ...
    const int lineHeightPx =
        DxUi::Typography::MeasureSingleLineTextMetrics(statusMeasureFactory, statusTextFormat.get(), statusDpi, statusAfterDown.selectionText).lineHeightPx;
...
{
    using namespace std::chrono_literals;
    const std::filesystem::path root = SelfTest::GetTempRoot(SelfTest::SelfTestSuite::Commands) / L"work" / (L"terminal command state " + NewGuidText());
    ...
    state.Require(snapshot.lifecycle == TerminalLifecycleState::Running, L"The Terminal command-state fixture must reach Running before state queries.");
    if (! state.failure.empty())
    {
        return false;
    }
    ...
    state.Require(g_folderWindow.QueryTerminalHostCommandState(snapshot.childHwnd, L"cmd/terminal/tab/select/1", folderTabState) && folderTabState.enabled,
                  L"Embedded tab identity 1 must expose the available Folder target.");
    state.Require(g_folderWindow.QueryTerminalHostCommandState(snapshot.childHwnd, L"cmd/terminal/tab/select/2", previewTabState) && ! previewTabState.enabled,
                  L"Embedded tab identity 2 must be disabled when Preview is unavailable.");
    state.Require(g_folderWindow.QueryTerminalHostCommandState(snapshot.childHwnd, L"cmd/terminal/tab/select/3", terminalTabState) && terminalTabState.enabled,
                  L"Embedded tab identity 3 must be enabled while its Terminal is live.");
    state.Require(g_folderWindow.QueryTerminalHostCommandState(snapshot.childHwnd, L"cmd/terminal/copy", copyState) && ! copyState.enabled,
    ...
    state.Require(g_folderWindow.ExecuteTerminalHostCommand(L"cmd/terminal/tab/select/1"), L"Direct dispatch must handle available tab identity 1.");
    state.Require(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot) && snapshot.open && ! snapshot.selected,
    ...
    state.Require(FocusWindowAndWait(snapshot.childHwnd, SelfTest::Scale(1000ms)), L"The restored Terminal must accept focus before shortcut validation.");
    state.Require(! DebugDispatchShortcutCommand(mainWindow, L"cmd/terminal/tab/select/2"), L"Shortcut dispatch must not consume unavailable tab identity 2.");
    state.Require(DebugDispatchShortcutCommand(mainWindow, L"cmd/terminal/tab/select/3"), L"Shortcut dispatch must handle available tab identity 3.");
    ...
    PumpPendingMessages();
    // The palette closes itself when the app window loses activation. Name that condition here so a
    // run that lost foreground activation reports it, rather than failing on the first query.
    state.Require(GetCommandPaletteWindowHandle() != nullptr && IsWindow(GetCommandPaletteWindowHandle()) != FALSE,
                  L"The command palette must still be open before its queries run; it closes when the app window loses activation.");
    state.Require(DebugSetCommandPaletteSearch(L"cmd/terminal/tab/select/2"), L"The palette must accept the unavailable numbered-tab query.");
    PumpPendingMessages();
    ...
    state.Require(DebugGetCommandPaletteSnapshot(paletteSnapshot) && paletteSnapshot.rowCount == 1u &&
                      paletteSnapshot.selectedCommandId == L"cmd/terminal/tab/select/2" && ! paletteSnapshot.selectedEnabled,
    ...
    state.Require(DebugSetCommandPaletteSearch(L"cmd/terminal/close"), L"The palette must accept the host-owned Terminal Close query.");
    PumpPendingMessages();
    state.Require(DebugGetCommandPaletteSnapshot(paletteSnapshot) && paletteSnapshot.rowCount == 1u &&
...
[[nodiscard]] bool TestTerminalSessionMenuFloatingDispatch(HWND mainWindow, CaseState& state) noexcept
{
    ...
    const std::filesystem::path root = SelfTest::GetTempRoot(SelfTest::SelfTestSuite::Commands) / L"work" / (L"terminal session menu " + NewGuidText());
    ...
    g_folderWindow.SetFolderPath(FolderWindow::Pane::Left, root);
    state.Require(WaitForPanePath(FolderWindow::Pane::Left, root, SelfTest::Scale(3000ms)), L"Failed to navigate to the Terminal session-menu fixture folder.");
    g_folderWindow.SetActivePane(FolderWindow::Pane::Left);
    const HWND folder = g_folderWindow.GetFolderViewHwnd(FolderWindow::Pane::Left);
    state.Require(FocusWindowAndWait(folder, SelfTest::Scale(1000ms)), L"The first floating Terminal must be launchable from a normal file pane.");
    CommandRuntimeState initialState{};
    state.Require(
        ResolveCommandRuntimeStateFromWindow(mainWindow, folder, L"cmd/terminal/openFloatingWindow", CommandStateSource::ApplicationHost, initialState) &&
    ...
    {
        PumpPendingMessages();
        if (g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, embedded) && embedded.open && embedded.childHwnd != nullptr &&
            embedded.lifecycle == TerminalLifecycleState::Running)
        {
            break;
        }
        Sleep(10u);
    ...
    state.Require(GetFloatingTerminalWindowHandle() == nullptr, L"Terminal session-menu dispatch fixture requires no pre-existing floating Terminal.");

    CommandRuntimeState floatingState{};
    state.Require(g_folderWindow.QueryTerminalHostCommandState(embedded.childHwnd, L"cmd/terminal/openFloatingWindow", floatingState) && floatingState.enabled,
    ...
    state.Require(snapshot.root != nullptr && snapshot.tabCount == 1u, L"The Terminal session-menu floating action must create one floating Terminal tab.");
    if (snapshot.tabCount > 0u)
    {
        state.Require(DebugCloseFloatingTerminalTab(snapshot.tabCount - 1u), L"Failed to close the session-menu floating Terminal fixture tab.");
    }
    ...
    state.Require(GetFloatingTerminalWindowHandle() == nullptr, L"Session-menu floating Terminal fixture did not release its singleton root.");
    return state.failure.empty();
}
...
[[nodiscard]] bool TestEmbeddedTerminalPluginLifecycle(HWND mainWindow, CaseState& state) noexcept
{
    ...
    g_folderWindow.SetFolderPath(FolderWindow::Pane::Left, root);
    state.Require(WaitForPanePath(FolderWindow::Pane::Left, root, SelfTest::Scale(3000ms)), L"Failed to navigate to embedded-terminal test folder.");
    ...
    do
    {
        PumpPendingMessages();
        static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
        if (snapshot.lifecycle == TerminalLifecycleState::Running && snapshot.activityTrust == TerminalActivityTrust::Trusted && snapshot.idleAtPrimaryPrompt)
        {
            break;
        }
        Sleep(10u);
    ...
    state.Require(snapshot.childHwnd != nullptr && IsWindow(snapshot.childHwnd) != FALSE, L"Embedded terminal must expose a live child HWND.");
    state.Require(WindowExposesUiaProvider(snapshot.childHwnd), L"Embedded terminal child HWND must expose its plugin-owned UI Automation provider.");
    ...
    state.Require(
        snapshot.activityTrust == TerminalActivityTrust::Trusted,
        std::format(L"The contained PowerShell root should authenticate its prompt state through the private PID-bound pipe; status: {}.", snapshot.status));
    ...
    state.Require(promptVisible,
                  std::format(L"A Running shell must produce initial terminal text (read=0x{:08X}, characters={}).",
                              static_cast<uint32_t>(promptRead),
                              promptText.size()));
    ...
    if (! state.Require(snapshot.open && snapshot.childHwnd == terminalChild && IsWindow(terminalChild) != FALSE &&
                            snapshot.lifecycle == TerminalLifecycleState::Running,
                        std::format(L"The live terminal must still be Running 1s after open, before SwapPanes (open={}, hwnd={}, lifecycle={}, "
                                    L"exitCodePresent={}, exitCode={}, client={}x{}).",
                                    snapshot.open,
    ...
    state.Require(
        leftAfterSwap.open && leftAfterSwap.childHwnd == terminalChild && IsWindow(terminalChild) != FALSE,
    ...
    state.Require(! rightAfterSwap.open, L"The original host pane must not keep the terminal after SwapPanes.");
    RECT frameAfterSwap{};
    RECT termAfterSwap{};
    state.Require(GetWindowRect(g_folderWindow.GetHwnd(), &frameAfterSwap) != FALSE && GetWindowRect(terminalChild, &termAfterSwap) != FALSE &&
    ...
    do
    {
        ...
        Sleep(10u);
        static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Left, leftAfterSwap));
        if (! leftAfterSwap.open || leftAfterSwap.childHwnd != terminalChild || leftAfterSwap.lifecycle != TerminalLifecycleState::Running)
        {
            break;
        }
    } while (std::chrono::steady_clock::now() < afterFirstSwapKeepAliveUntil);
    if (! state.Require(leftAfterSwap.open && leftAfterSwap.childHwnd == terminalChild && IsWindow(terminalChild) != FALSE &&
    ...
    if (! state.Require(snapshot.open && snapshot.childHwnd == terminalChild && IsWindow(terminalChild) != FALSE,
                        std::format(L"A second SwapPanes must restore the live terminal to its original host pane (beforePumpOpen={}, beforePumpHwnd={}, "
                                    L"beforePumpLifecycle={}, open={}, hwnd={}, expected={}, lifecycle={}, leftOpen={}).",
    ...
    if (! state.Require(GetWindowRect(g_folderWindow.GetHwnd(), &frameAfterReturn) != FALSE && GetWindowRect(terminalChild, &termAfterReturn) != FALSE &&
    ...
    state.Require(secondTerminal.open && secondTerminal.childHwnd != nullptr && IsWindow(secondTerminal.childHwnd) != FALSE && rightWhileOpeningSecond.open &&
                      rightWhileOpeningSecond.childHwnd == terminalChild,
                  std::format(L"A second opposite-pane terminal must open on the left while the right terminal stays live (leftOpen={}, leftHwnd={}, "
                              L"rightOpen={}, rightHwnd={}, expectedRight={}).",
                              secondTerminal.open,
    ...
    state.Require(leftTabAfterDualSwap <= 2u && rightTabAfterDualSwap <= 2u && (leftTabAfterDualSwap != 1u || ! leftExclusive.selected) &&
                      (rightTabAfterDualSwap != 1u || ! rightExclusive.selected) && (leftTabAfterDualSwap != 2u || leftExclusive.selected) &&
                      (rightTabAfterDualSwap != 2u || rightExclusive.selected),
                  std::format(L"SwapPanes must keep exactly one of Folder/Preview/Terminal selected per pane (leftTab={}, rightTab={}, "
                              L"leftTerminalSelected={}, rightTerminalSelected={}).",
    ...
    state.Require(
        SUCCEEDED(g_folderWindow.RouteTerminalShortcut(snapshot.childHwnd, L"cmd/app/preferences", routedMessage, ShortcutManager::kModCtrl, route)) &&
    ...
    state.Require(
        SUCCEEDED(g_folderWindow.RouteTerminalShortcut(snapshot.childHwnd, L"cmd/terminal/font/increase", routedMessage, ShortcutManager::kModCtrl, route)) &&
    ...
    route                = TerminalShortcutRoute::Handled;
    state.Require(SUCCEEDED(g_folderWindow.RouteTerminalShortcut(snapshot.childHwnd, L"cmd/terminal/copySelectionOrPassthrough", routedMessage, 0u, route)) &&
                      route == TerminalShortcutRoute::PassThrough,
    ...
    state.Require(RedrawWindow(reusedChild, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW) != FALSE, L"Embedded terminal focused repaint should complete.");
    ...
    PumpPendingMessages();
    static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
    state.Require(snapshot.open && ! snapshot.selected, L"Selecting the Folder tab must keep the terminal open without displaying it.");
    state.Require(IsWindowVisible(reusedChild) == FALSE, L"Selecting the Folder tab must hide the open terminal child.");
    ...
    PumpPendingMessages();
    state.Require(RedrawWindow(reusedChild, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW) != FALSE, L"Embedded terminal inactive repaint should complete.");
    if (! state.failure.empty())
    {
        return false;
    }
    ...
    PumpPendingMessages();
    static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
    state.Require(snapshot.childHwnd == reusedChild, L"Open Command Shell should reuse the live opposite-pane terminal instead of replacing its process tree.");
    ...
    state.Require(SUCCEEDED(g_folderWindow.DebugSetTerminalConfiguration(FolderWindow::Pane::Right, R"json({"followPathWhenIdle":true})json")),
    ...
    do
    {
        PumpPendingMessages();
        static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
        if (snapshot.followState == TerminalFollowState::Applied && snapshot.activityTrust == TerminalActivityTrust::Trusted && snapshot.idleAtPrimaryPrompt)
        {
            break;
        }
        Sleep(10u);
    ...
    PumpPendingMessages();
    static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
    state.Require(snapshot.childHwnd == reusedChild && snapshot.selected, L"Bring Current Directory must insert through the same live opposite-pane terminal.");
    ...
    PumpPendingMessages();
    static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
    state.Require(snapshot.sourceGeneration > localSourceGeneration, L"A local-to-plugin source transition must advance the terminal source generation.");
    ...
    state.Require(g_folderWindow.DebugGetPaneAlertSnapshot(FolderWindow::Pane::Left, unsupportedAlert) && unsupportedAlert.visible,
    ...
    const std::filesystem::path archivePath    = SelfTest::TryFindRepoRoot() / L"Plugins" / L"FileSystem7z" / L"Tests" / L"Tests.zip";
    state.Require(SelfTest::PathExists(archivePath), std::format(L"The plugin-backed terminal fixture archive is missing: '{}'.", archivePath.wstring()));
    if (! state.failure.empty())
    {
        return false;
    }
    const std::filesystem::path pluginBackingDirectory = archivePath.parent_path();
    const std::filesystem::path pluginBackedLocation   = NavigationLocation::FormatHistoryPath(L"7z", archivePath.wstring(), L"/");
    ...
    {
        PumpPendingMessages();
        if (OrdinalString::EqualsNoCase(g_folderWindow.GetFileSystemInstanceContext(FolderWindow::Pane::Left), archivePath.wstring()))
        {
            break;
        }
        Sleep(10u);
    ...
    static_cast<void>(g_folderWindow.DebugGetTerminalPaneSnapshot(FolderWindow::Pane::Right, snapshot));
    state.Require(snapshot.sourceGeneration > unsupportedSourceGeneration && snapshot.sourceLocationKind == TerminalLocationKind::PluginBacked &&
                      OrdinalString::EqualsNoCase(snapshot.sourcePluginShortId, L"7z") && snapshot.sourcePath == pluginBackingDirectory,
...
{
    using namespace std::chrono_literals;
    const std::filesystem::path root = SelfTest::GetTempRoot(SelfTest::SelfTestSuite::Commands) / L"work" / (L"embedded terminal exit " + NewGuidText());
    ...
    g_folderWindow.SetFolderPath(FolderWindow::Pane::Left, root);
    state.Require(WaitForPanePath(FolderWindow::Pane::Left, root, SelfTest::Scale(3000ms)), L"Failed to navigate to the embedded-terminal exit test folder.");
    g_folderWindow.CommandOpenCommandShell(FolderWindow::Pane::Left);
    ...
    state.Require(snapshot.open && snapshot.selected && snapshot.childHwnd != nullptr && snapshot.activityTrust == TerminalActivityTrust::Trusted &&
...
[[nodiscard]] bool TestEmbeddedTerminalTabVisibilityAndKeyboardTarget(HWND /*mainWindow*/, CaseState& state) noexcept
{
    ...
    g_folderWindow.SetFolderPath(FolderWindow::Pane::Left, root);
    state.Require(WaitForPanePath(FolderWindow::Pane::Left, root, SelfTest::Scale(3000ms)), L"Failed to navigate to the embedded-terminal chrome test folder.");
    if (! state.failure.empty())
    {
        return false;
    }
