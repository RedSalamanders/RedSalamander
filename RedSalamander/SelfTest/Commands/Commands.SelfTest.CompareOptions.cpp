...
[[nodiscard]] bool FocusCompareDirectoriesOptionsTargetAndWait(HWND compare,
...
[[nodiscard]] bool TestCompareDirectoriesOptionsThemeCycleKeepsSurfaceLegible(HWND mainWindow, CaseState& state) noexcept
{
    ...
    state.Require(
        FocusCompareDirectoriesOptionsTargetAndWait(compare, CompareDirectoriesOptionsDebugFocusTarget::IgnoreFilesEdit, SelfTest::Scale(3000ms), snapshot) &&
...
[[nodiscard]] bool TestCompareDirectoriesOptionsEnterAndEscapeRouteDefaultCancel(HWND mainWindow, CaseState& state) noexcept
{
    ...
    const auto runPass = [&](const bool accept, std::wstring_view label) noexcept
    {
        ...
        state.Require(FocusCompareDirectoriesOptionsTargetAndWait(
                          compare, CompareDirectoriesOptionsDebugFocusTarget::CompareSubdirectoriesToggle, SelfTest::Scale(3000ms), snapshot) &&
                          snapshot.visibleLegacyStaticCount == 0u && snapshot.visibleLegacyFooterButtonCount == 0u && snapshot.visibleLegacyToggleCount == 0u &&
                          snapshot.visibleLegacyEditCount == 0u && snapshot.visibleBodyRenderedDxHostCount == 1u && snapshot.bodyDxHostResizeFailureCount == 0u,
