                                    ...
                                    std::wstring_view failureContext) noexcept
    {
                                 ...
                                 std::wstring* observedInitialValue) noexcept
        {
            ...
            std::jthread worker([&](std::stop_token) noexcept
            {
                ...
                // An early UIA failure must release the modal SendMessage on the owner thread
                // before the test can join this worker and report its failed assertion.
                const auto dismissOnFailure    = wil::scope_exit([&]() noexcept
                {
                    if (! workerResult.closedAfterAction && IsWindow(workerResult.prompt) != FALSE && IsOwnedBy(workerResult.prompt, mainWindow))
                    {
                        static_cast<void>(PostMessageW(workerResult.prompt, WM_CLOSE, 0, 0));
                    }
                });
                workerResult.ownedByMainWindow = IsOwnedBy(workerResult.prompt, mainWindow);
