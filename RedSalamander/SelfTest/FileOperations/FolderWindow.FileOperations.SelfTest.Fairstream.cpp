...
case SelfTestState::Step::Fairstream_StorageKindProbed:
{
    ...
    if (state.stepState == 0)
    {
        ...
        // UNKNOWN means the seek-penalty and bus-type queries were unavailable, which is the normal
        // answer on hosted-runner and other virtual disks. The clamp under test needs a resolved
        // medium, so the case is skipped there instead of failing the volume's provenance.
        if (characteristics.storageKind == FILESYSTEM_STORAGE_UNKNOWN)
        {
            clearAutoConcurrencyOverride();
            cancelStorageClampTask();
            Debug::Perf::Emit(L"FileOps.SelfTest.FairstreamStorageClamp", L"skip=storage-kind-unknown", 0u, 0u, 0u, S_FALSE);
            RecordCurrentPhase(
                state, SelfTest::SelfTestCaseResult::Status::skipped, L"Storage-kind probe returned UNKNOWN for this volume (virtual or unreported medium).");
