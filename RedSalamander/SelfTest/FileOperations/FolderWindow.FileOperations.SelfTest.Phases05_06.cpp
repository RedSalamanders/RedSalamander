...
case SelfTestState::Step::FileOps_ProviderCapabilityMatrix:
{
    ...
    if (state.stepState == 0)
    {
        ...
        Debug::Perf::Emit(L"FileOps.SelfTest.R0d.ReceiptClassificationCount",
                          ...
                          0u,
                          r0dPerfValid ? S_OK : E_FAIL);
        // Match the adjacent R0e diagnostic profile: Release qualifies performance;
        // Debug and ASan Debug retain correctness checks with a bounded diagnostic ceiling.
#if defined(_DEBUG)
        constexpr uint64_t kR0dSelfTestCeilingUs = 5'000'000u;
#else
        constexpr uint64_t kR0dSelfTestCeilingUs = 2'000'000u;
#endif
        if (! require(r0dPerfValid && r0dPerfElapsedUs < kR0dSelfTestCeilingUs,
                      std::format(L"R0d capability/admission/receipt classification must remain valid and within its build-specific ceiling "
                                  L"(valid={}, elapsed={}us, ceiling={}us).",
                                  r0dPerfValid,
                                  r0dPerfElapsedUs,
                                  kR0dSelfTestCeilingUs)))
        {
            return true;
        }
...
case SelfTestState::Step::R4A19_DiscoveryProviderControls:
{
    ...
    if (state.stepState == 1u)
    {
        ...
        // The Dummy holds its whole subtree before it mutates, so on this direct route it closes on
        // the exact total before its first byte moves: nothing completes while discovery is open and
        // nothing may arrive after the closure. The overlap facts (first mutation before closure,
        // open-discovery bytes and mutations) belong to the fake-MTP control below, whose route the
        // host bridge traverses.
        constexpr unsigned long kExpectedDummyFiles       = kDummyDirectoryCount * kDummyFilesPerDirectory;
        constexpr unsigned long kExpectedDummyDirectories = kDummyDirectoryCount + 1u;
        if (FAILED(info.hr) || ! bytesMatch || ! info.discoveryClosed || info.firstMutationBeforeDiscoveryClosed ||
            info.discoveredTotalBytes != kExpectedDummyBytes || info.discoveredFiles != kExpectedDummyFiles ||
            info.discoveredDirectories != kExpectedDummyDirectories || info.discoveryBytesCompletedWhileOpen != 0u ||
            info.discoveryMutationsCompletedWhileOpen != 0u || info.discoveryGrowthAfterCloseCount != 0u || info.discoveryStarvationCount != 0u ||
            info.discoveryMaxQueueDepth > 256u || durationUs >= 30'000'000u)
        {
            Fail(std::format(L"R4-A19 Dummy control failed: hr=0x{:08X} bytesMatch={} closed={} beforeClose={} "
                             L"discoveredBytes={} expectedPayloadBytes={} files={} expectedPayloadFiles={} directories={} expectedDirectories={} "
                             L"bytesWhileOpen={} mutationsWhileOpen={} growthAfterClose={} starvation={} queue={} durationUs={}.",
                             ...
                             kExpectedDummyFiles,
                             info.discoveredDirectories,
                             kExpectedDummyDirectories,
        ...
        Debug::Perf::Emit(L"FileOps.SelfTest.R4A19.ProviderControl",
                          L"provider=dummy;latencyMs=5;streamChunkLatencyMs=2;virtualSpeedLimit=1048576;mode=single-root-per-item;closed=before-first-byte",
                          durationUs,
...
case SelfTestState::Step::R4A19_DiscoveryIndependentVolumes:
{
    ...
    PruneEmptyAlternateVolumeSandboxParents(alternateRoot);
    NextStep(state, SelfTestState::Step::DiscoveryScope_LeafCopy);
    return false;
}

// ---------------------------------------------------------------------------------------------
// I21 discovery scope. Two rules are proved here. A selected root that is a known regular file
// publishes its exact byte total and closes its own discovery scope as soon as the source binds,
// before any writer, stage or payload work, so the card holds a fixed denominator for the whole
// transfer. And a traversal owner's nested provider calls (exact source cleanup, a refused Native
// directory rename) can never close a root they do not own, so totals never grow after the task
// has presented them as final. A successful outcome cannot prove truthful progress, so the leaf
// cases observe the live task and the real popup projection while the task is still unfinished.
// ---------------------------------------------------------------------------------------------
case SelfTestState::Step::DiscoveryScope_LeafCopy:
case SelfTestState::Step::DiscoveryScope_LeafMove:
{
    const bool isMove             = state.step == SelfTestState::Step::DiscoveryScope_LeafMove;
    const std::wstring_view label = isMove ? L"DiscoveryScope_LeafMove" : L"DiscoveryScope_LeafCopy";
    const ULONGLONG nowTick       = GetTickCount64();

    const auto disarm = []() noexcept
    {
        ReleaseFileOpsSelectedLeafDiscoveryPublishedPauseForSelfTest();
        SetFileOpsSelectedLeafDiscoveryPublishedPauseForSelfTest(false);
    };
    const auto fail = [&](std::wstring_view message) noexcept
    {
        disarm();
        Fail(message);
    };
    ...
    if (! state.fileOps || ! state.fsDummy)
    {
        fail(std::format(L"{} requires the file-operation state and the Dummy provider.", label));
        return true;
    }

    // A deliberately non-round size: a denominator that tracked transferred bytes instead of the
    // bound source size cannot match this by accident at the held checkpoint.
    constexpr uint64_t kLeafBytes          = 3ull * 1024ull * 1024ull + 777ull;
    const std::wstring dummyDestination    = isMove ? L"/discovery-scope-leaf-move" : L"/discovery-scope-leaf-copy";
    const std::filesystem::path sourceRoot = state.tempRoot / (isMove ? L"discovery-scope-leaf-move-src" : L"discovery-scope-leaf-copy-src");
    const std::filesystem::path sourceFile = sourceRoot / L"payload.bin";

    if (state.stepState == 0u)
    {
        disarm();
        if (! RecreateEmptyDirectory(sourceRoot) || ! WriteTestFile(sourceFile, static_cast<size_t>(kLeafBytes)))
        {
            fail(std::format(L"{}: could not seed the selected leaf.", label));
            return true;
        }

        // The configured Dummy instance survives same-process repeats, so clear this case's
        // destination or a stale object would raise an unrelated overwrite prompt.
        const HRESULT resetHr = state.fsDummy->DeleteItem(dummyDestination.c_str(),
                                                          static_cast<FileSystemFlags>(FILESYSTEM_FLAG_RECURSIVE | FILESYSTEM_FLAG_ALLOW_REPLACE_READONLY),
                                                          nullptr,
                                                          nullptr,
                                                          nullptr);
        if (FAILED(resetHr) && resetHr != HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND) && resetHr != HRESULT_FROM_WIN32(ERROR_PATH_NOT_FOUND))
        {
            fail(std::format(L"{}: could not reset the Dummy destination (hr=0x{:08X}).", label, static_cast<unsigned long>(resetHr)));
            return true;
        }
        if (! EnsureDummyFolderExists(state.fsDummy.get(), dummyDestination))
        {
            fail(std::format(L"{}: could not create the Dummy destination.", label));
            return true;
        }

        SetFileOpsSelectedLeafDiscoveryPublishedPauseForSelfTest(true);
        state.taskA = StartFileOperationAndGetId(state.fileOps,
                                                 ...
                                                 state.fsLocal,
                                                 {sourceFile},
                                                 std::filesystem::path(dummyDestination),
                                                 FILESYSTEM_FLAG_NONE,
                                                 false,
                                                 0,
                                                 FolderWindow::FileOperationState::ExecutionMode::PerItem,
        ...
        if (! state.taskA.has_value())
        {
            fail(std::format(L"{}: the cross-provider leaf transfer did not start.", label));
            return true;
        }
        state.stepState = 1u;
        return false;
    ...
    if (state.stepState == 1u)
    {
        if (state.taskA.has_value() && state.completedTasks.contains(state.taskA.value()))
        {
            fail(std::format(L"{}: the task finished before the held leaf checkpoint could be observed.", label));
            return true;
        }
        if (! HasFileOpsSelectedLeafDiscoveryPublishedPauseEnteredForSelfTest())
        {
            return false;
        }
        ...
        if (task == nullptr)
        {
            fail(std::format(L"{}: the live task disappeared at the held leaf checkpoint.", label));
            return true;
        }

        // Producer truth: the known leaf's exact size is published and this root's scope is closed
        // while no payload has moved.
        const bool liveDiscoveryClosed     = task->_discoveryClosed.load(std::memory_order_acquire);
        const uint64_t liveDiscoveredBytes = task->_discoveredTotalBytes.load(std::memory_order_acquire);
        const unsigned long liveFiles      = task->_discoveredFileCount.load(std::memory_order_acquire);
        const unsigned long liveDirs       = task->_discoveredDirectoryCount.load(std::memory_order_acquire);
        if (! liveDiscoveryClosed || liveDiscoveredBytes != kLeafBytes || liveFiles != 1ul || liveDirs != 0ul)
        {
            fail(std::format(L"{}: the held known leaf must report exactly one file of {} bytes with discovery closed "
                             L"(closed={} bytes={} files={} dirs={}).",
                             label,
                             kLeafBytes,
                             liveDiscoveryClosed ? 1 : 0,
                             liveDiscoveredBytes,
                             liveFiles,
                             liveDirs));
            return true;
        ...
        // Consumer truth: the popup's own projection of the live task, not a synthetic snapshot.
        FileOperationsPopupInternal::TaskSnapshot snapshot{};
        if (! TryGetPopupTaskSnapshot(state.fileOps, state.taskA.value(), snapshot))
        {
            if (state.markerTick == 0u)
            {
                state.markerTick = nowTick;
            }
            if (nowTick >= state.markerTick && (nowTick - state.markerTick) > SelfTest::ScaleTimeout(8'000ull))
            {
                fail(std::format(L"{}: could not read the live popup snapshot at the held leaf checkpoint.", label));
                return true;
            }
            return false;
        }

        if (snapshot.finished || ! snapshot.discoveryClosed || snapshot.discoveredTotalBytes != kLeafBytes || snapshot.discoveredFileCount != 1ul ||
            snapshot.totalBytes != kLeafBytes || snapshot.totalItems != 1ul)
        {
            fail(std::format(L"{}: the live card must show a fixed full-file denominator on an unfinished task "
                             L"(finished={} closed={} discoveredBytes={} discoveredFiles={} totalBytes={} totalItems={}).",
                             label,
                             snapshot.finished ? 1 : 0,
                             snapshot.discoveryClosed ? 1 : 0,
                             snapshot.discoveredTotalBytes,
                             snapshot.discoveredFileCount,
                             snapshot.totalBytes,
                             snapshot.totalItems));
            return true;
        }

        FileOperationsPopupInternal::TaskProgressPresentationDebugSnapshot presentation{};
        DebugResolveFileOperationsWholeTaskProgress(snapshot, presentation);
        if (! presentation.determinate || presentation.provisional || presentation.showDiscoveryActivity || presentation.showTransferMarquee)
        {
            fail(std::format(L"{}: a known leaf mid-transfer must render determinate progress with no discovery activity "
                             L"(determinate={} provisional={} discoveryActivity={} marquee={}).",
                             label,
                             presentation.determinate ? 1 : 0,
                             presentation.provisional ? 1 : 0,
                             presentation.showDiscoveryActivity ? 1 : 0,
                             presentation.showTransferMarquee ? 1 : 0));
            return true;
        }

        // The item header must take the known-total branch. Comparing it with the same snapshot
        // carrying an open scope proves the branch without depending on localized wording.
        const std::wstring knownTotalHeader                 = DebugBuildFileOperationsTaskHeaderText(snapshot, nowTick);
        FileOperationsPopupInternal::TaskSnapshot openScope = snapshot;
        openScope.discoveryClosed                           = false;
        const std::wstring unknownTotalHeader               = DebugBuildFileOperationsTaskHeaderText(openScope, nowTick);
        if (knownTotalHeader.empty() || knownTotalHeader == unknownTotalHeader)
        {
            fail(std::format(L"{}: the item header still withholds the denominator for a closed known leaf (header='{}').", label, knownTotalHeader));
            return true;
        }

        // The compact card and the hosted accessibility value follow the same known-progress cohort.
        if (! DebugFileOperationsTaskHasKnownCompactProgress(snapshot))
        {
            fail(std::format(L"{}: the compact card must expose known progress once the leaf total is closed.", label));
            return true;
        }
        const float completeFraction = DebugComputeFileOperationsTaskCompleteFraction(snapshot);
        if (completeFraction < 0.0f || completeFraction > 1.0f)
        {
            fail(std::format(L"{}: the hosted progress value must stay in range (fraction={}).", label, completeFraction));
            return true;
        }
    ...
    const CompletedTaskInfo& info = completed->second;
    if (FAILED(info.hr) || ! info.discoveryClosed || info.discoveredTotalBytes != kLeafBytes || info.discoveredFiles != 1ul ||
        info.discoveryGrowthAfterCloseCount != 0ull || info.progressCompletedBytes != kLeafBytes)
    {
        fail(std::format(L"{}: the completed leaf transfer must keep its exact totals "
                         L"(hr=0x{:08X} closed={} bytes={} files={} growthAfterClose={} completedBytes={}).",
                         label,
                         static_cast<unsigned long>(info.hr),
                         info.discoveryClosed ? 1 : 0,
                         info.discoveredTotalBytes,
                         info.discoveredFiles,
                         info.discoveryGrowthAfterCloseCount,
                         info.progressCompletedBytes));
        return true;
    }

    // Closing discovery early is neither transfer completion nor permission to remove a source.
    const bool sourceStillPresent = std::filesystem::exists(sourceFile);
    if (sourceStillPresent == isMove)
    {
        fail(std::format(
            L"{}: source retention is wrong after the transfer (present={} expectedPresent={}).", label, sourceStillPresent ? 1 : 0, isMove ? 0 : 1));
        return true;
    }

    Debug::Perf::Emit(L"FileOps.SelfTest.DiscoveryScope.Leaf",
                      isMove ? L"op=move;route=cross-provider-bridge" : L"op=copy;route=cross-provider-bridge",
                      info.discoveryDurationUs,
                      info.discoveredTotalBytes,
                      info.discoveryGrowthAfterCloseCount,
                      info.hr);
    disarm();
    NextStep(state, isMove ? SelfTestState::Step::DiscoveryScope_ManagedDirectory : SelfTestState::Step::DiscoveryScope_LeafMove);
    return false;
}

case SelfTestState::Step::DiscoveryScope_ManagedDirectory:
{
    // A managed directory Move: every child is published to the destination provider and then the
    // exact bound source object is deleted. That nested cleanup reports its own closed record, and
    // it must not close the walker's root while later siblings are still being discovered.
    constexpr std::wstring_view label = L"DiscoveryScope_ManagedDirectory";
    const ULONGLONG nowTick           = GetTickCount64();
    if (HasTimedOut(state, nowTick, 180'000ull))
    {
        Fail(std::format(L"{} timed out.", label));
    ...
    constexpr unsigned int kChildCount     = 6u;
    constexpr uint64_t kChildBytes         = 128ull * 1024ull + 13ull;
    constexpr uint64_t kExpectedBytes      = static_cast<uint64_t>(kChildCount) * kChildBytes;
    const std::wstring dummyDestination    = L"/discovery-scope-managed-directory";
    const std::filesystem::path sourceRoot = state.tempRoot / L"discovery-scope-managed-src";
    const std::filesystem::path sourceTree = sourceRoot / L"tree";

    if (state.stepState == 0u)
    {
        if (! RecreateEmptyDirectory(sourceTree))
        {
            Fail(std::format(L"{}: could not reset the managed-directory fixture.", label));
            return true;
        }
        for (unsigned int index = 0u; index < kChildCount; ++index)
        {
            if (! WriteTestFile(sourceTree / std::format(L"child-{:02}.bin", index), static_cast<size_t>(kChildBytes)))
            {
                Fail(std::format(L"{}: could not seed managed-directory child {}.", label, index));
                return true;
            }
        }
        ...
        if (! state.taskA.has_value())
        {
            Fail(std::format(L"{}: the managed directory Move did not start.", label));
            return true;
        }
        state.stepState = 1u;
        return false;
    ...
    const CompletedTaskInfo& info    = completed->second;
    const size_t remainingSourceFiles = CountFilesRecursive(sourceTree);
    if (FAILED(info.hr) || ! info.discoveryClosed || info.discoveryGrowthAfterCloseCount != 0ull || info.discoveredFiles != kChildCount ||
        info.discoveredTotalBytes != kExpectedBytes || remainingSourceFiles != 0u)
    {
        Fail(std::format(L"{}: nested exact source cleanup must not close the walker's root "
                         L"(hr=0x{:08X} closed={} growthAfterClose={} files={}/{} bytes={}/{} remainingSourceFiles={}).",
                         label,
                         static_cast<unsigned long>(info.hr),
                         info.discoveryClosed ? 1 : 0,
                         info.discoveryGrowthAfterCloseCount,
    ...
    Debug::Perf::Emit(L"FileOps.SelfTest.DiscoveryScope.ManagedDirectory",
                      L"route=cross-provider-managed-move",
                      info.discoveryDurationUs,
                      info.discoveredTotalBytes,
                      info.discoveryGrowthAfterCloseCount,
                      info.hr);
    NextStep(state, SelfTestState::Step::DiscoveryScope_NativeRenameMerge);
    return false;
}

case SelfTestState::Step::DiscoveryScope_NativeRenameMerge:
{
    // A same-endpoint directory Move is one Native rename. When a regular directory already
    // occupies the destination the provider refuses to commit and the same item continues as a
    // host rename merge that keeps discovering, so the refused attempt's own closed report must
    // not latch this root closed while the merge walk is still producing work.
    constexpr std::wstring_view label = L"DiscoveryScope_NativeRenameMerge";
    const ULONGLONG nowTick           = GetTickCount64();
    if (HasTimedOut(state, nowTick, 180'000ull))
    {
        Fail(std::format(L"{} timed out.", label));
    ...
    if (! state.fileOps)
    {
        Fail(std::format(L"{} requires the file-operation state.", label));
        return true;
    }

    constexpr unsigned int kChildCount          = 6u;
    constexpr uint64_t kChildBytes              = 96ull * 1024ull + 5ull;
    const std::filesystem::path base            = state.tempRoot / L"discovery-scope-merge";
    const std::filesystem::path sourceTree      = base / L"src" / L"tree";
    const std::filesystem::path destinationRoot = base / L"dst";
    const std::filesystem::path occupiedTree    = destinationRoot / L"tree";

    if (state.stepState == 0u)
    {
        if (! RecreateEmptyDirectory(sourceTree) || ! RecreateEmptyDirectory(occupiedTree))
        {
            Fail(std::format(L"{}: could not reset the rename-merge fixture.", label));
            return true;
        }
        for (unsigned int index = 0u; index < kChildCount; ++index)
        {
            if (! WriteTestFile(sourceTree / std::format(L"child-{:02}.bin", index), static_cast<size_t>(kChildBytes)))
            {
                Fail(std::format(L"{}: could not seed rename-merge child {}.", label, index));
                return true;
            }
        }
        // The destination directory exists and holds an unrelated child, so the one-mutation
        // rename cannot commit and the item continues as a merge walk.
        if (! WriteTestFile(occupiedTree / L"occupant.bin", 64u))
        {
            Fail(std::format(L"{}: could not seed the destination occupant.", label));
            return true;
        }
        ...
        if (! state.taskA.has_value())
        {
            Fail(std::format(L"{}: the same-endpoint directory Move did not start.", label));
            return true;
        }
        state.stepState = 1u;
        return false;
    ...
    const CompletedTaskInfo& info      = completed->second;
    const size_t mergedFiles           = CountFilesRecursive(occupiedTree);
    const size_t remainingSourceFiles  = CountFilesRecursive(sourceTree);
    const size_t expectedMergedFiles   = static_cast<size_t>(kChildCount) + 1u;
    if (FAILED(info.hr) || ! info.discoveryClosed || info.discoveryGrowthAfterCloseCount != 0ull || mergedFiles != expectedMergedFiles ||
        remainingSourceFiles != 0u)
    {
        Fail(std::format(L"{}: a refused Native directory rename must hand discovery to the merge walk "
                         L"(hr=0x{:08X} closed={} growthAfterClose={} mergedFiles={}/{} remainingSourceFiles={}).",
                         label,
                         static_cast<unsigned long>(info.hr),
                         info.discoveryClosed ? 1 : 0,
                         info.discoveryGrowthAfterCloseCount,
    ...
    Debug::Perf::Emit(L"FileOps.SelfTest.DiscoveryScope.NativeRenameMerge",
                      L"route=same-endpoint-native-rename-merge",
                      info.discoveryDurationUs,
                      info.discoveredTotalBytes,
                      info.discoveryGrowthAfterCloseCount,
                      info.hr);
    NextStep(state, SelfTestState::Step::DiscoveryScope_MixedRoots);
    return false;
}

case SelfTestState::Step::DiscoveryScope_MixedRoots:
{
    // Several selected roots. A leaf root closes its own scope as soon as its size is known, but
    // the task's aggregate stays open until every root has resolved, and the closed aggregate must
    // hold the exact sum in either selection order.
    constexpr std::wstring_view label = L"DiscoveryScope_MixedRoots";
    const ULONGLONG nowTick           = GetTickCount64();
    if (HasTimedOut(state, nowTick, 180'000ull))
    {
        Fail(std::format(L"{} timed out.", label));
    ...
    constexpr unsigned int kTreeChildCount = 5u;
    constexpr uint64_t kTreeChildBytes     = 64ull * 1024ull + 3ull;
    constexpr uint64_t kLeafBytes          = 512ull * 1024ull + 11ull;
    constexpr uint64_t kExpectedBytes      = kLeafBytes + static_cast<uint64_t>(kTreeChildCount) * kTreeChildBytes;
    constexpr unsigned long kExpectedFiles = 1ul + kTreeChildCount;
    const bool directoryFirst              = state.stepState >= 2u;
    const std::wstring dummyDestination    = directoryFirst ? L"/discovery-scope-mixed-b" : L"/discovery-scope-mixed-a";
    const std::filesystem::path sourceRoot = state.tempRoot / (directoryFirst ? L"discovery-scope-mixed-b-src" : L"discovery-scope-mixed-a-src");
    const std::filesystem::path leafFile   = sourceRoot / L"leaf.bin";
    const std::filesystem::path treeRoot   = sourceRoot / L"tree";

    if (state.stepState == 0u || state.stepState == 2u)
    {
        if (! RecreateEmptyDirectory(treeRoot) || ! WriteTestFile(leafFile, static_cast<size_t>(kLeafBytes)))
        {
            Fail(std::format(L"{}: could not reset the mixed-roots fixture.", label));
            return true;
        }
        for (unsigned int index = 0u; index < kTreeChildCount; ++index)
        {
            if (! WriteTestFile(treeRoot / std::format(L"child-{:02}.bin", index), static_cast<size_t>(kTreeChildBytes)))
            {
                Fail(std::format(L"{}: could not seed mixed-roots child {}.", label, index));
                return true;
            }
        }
        ...
        std::vector<std::filesystem::path> selection;
        if (directoryFirst)
        {
            selection = {treeRoot, leafFile};
        }
        else
        {
            selection = {leafFile, treeRoot};
        }
        state.taskA = StartFileOperationAndGetId(state.fileOps,
                                                 FILESYSTEM_COPY,
                                                 FolderWindow::Pane::Left,
        ...
        if (! state.taskA.has_value())
        {
            Fail(std::format(L"{}: the mixed-roots Copy did not start.", label));
            return true;
        }
        state.stepState = directoryFirst ? 3u : 1u;
        return false;
    }
    ...
    const CompletedTaskInfo& info = completed->second;
    if (FAILED(info.hr) || ! info.discoveryClosed || info.discoveryGrowthAfterCloseCount != 0ull || info.discoveredFiles != kExpectedFiles ||
        info.discoveredTotalBytes != kExpectedBytes)
    {
        Fail(std::format(L"{}: the aggregate must close once on exact summed totals ({} order) "
                         L"(hr=0x{:08X} closed={} growthAfterClose={} files={}/{} bytes={}/{}).",
                         label,
                         directoryFirst ? L"directory-first" : L"leaf-first",
                         static_cast<unsigned long>(info.hr),
                         info.discoveryClosed ? 1 : 0,
                         info.discoveryGrowthAfterCloseCount,
                         info.discoveredFiles,
    ...
    Debug::Perf::Emit(L"FileOps.SelfTest.DiscoveryScope.MixedRoots",
                      directoryFirst ? L"order=directory-first" : L"order=leaf-first",
                      info.discoveryDurationUs,
                      info.discoveredTotalBytes,
                      info.discoveryGrowthAfterCloseCount,
                      info.hr);
    ...
    NextStep(state, SelfTestState::Step::DiscoveryScope_ZeroByteAndVerifiedLeaf);
    return false;
}

case SelfTestState::Step::DiscoveryScope_ZeroByteAndVerifiedLeaf:
{
    // Two leaf shapes the ordinary cases cannot reach. A known empty file is a known total of zero
    // bytes, not an unknown total, and its card must stay determinate without dividing by it. And a
    // verification-enabled same-endpoint Copy reaches the bridge through a different admission than
    // the cross-provider route, so the leaf rule is proved there too.
    constexpr std::wstring_view label = L"DiscoveryScope_ZeroByteAndVerifiedLeaf";
    const ULONGLONG nowTick           = GetTickCount64();
    ...
    // Phase A is the empty cross-provider leaf; phase B is the verified same-endpoint leaf.
    const bool verifiedPhase               = state.stepState >= 3u;
    constexpr uint64_t kVerifiedLeafBytes  = 1024ull * 1024ull + 99ull;
    const uint64_t expectedBytes           = verifiedPhase ? kVerifiedLeafBytes : 0ull;
    const std::wstring dummyDestination    = L"/discovery-scope-zero-byte";
    const std::filesystem::path sourceRoot = state.tempRoot / (verifiedPhase ? L"discovery-scope-verified-src" : L"discovery-scope-zero-src");
    const std::filesystem::path sourceFile = sourceRoot / L"payload.bin";
    const std::filesystem::path localDestination = state.tempRoot / L"discovery-scope-verified-dst";

    if (state.stepState == 0u || state.stepState == 3u)
    {
        disarm();
        if (! RecreateEmptyDirectory(sourceRoot) || ! WriteTestFile(sourceFile, static_cast<size_t>(expectedBytes)))
        {
            fail(std::format(L"{}: could not seed the selected leaf.", label));
            return true;
        }
        ...
        if (verifiedPhase)
        {
            if (! RecreateEmptyDirectory(localDestination))
            {
                fail(std::format(L"{}: could not reset the verified destination.", label));
                return true;
            }
            // Verification admits the bridge for a same-endpoint Copy, which the cross-provider
        ...
        state.taskA = StartFileOperationAndGetId(state.fileOps,
                                                 ...
                                                 state.fsLocal,
                                                 {sourceFile},
                                                 verifiedPhase ? localDestination : std::filesystem::path(dummyDestination),
                                                 FILESYSTEM_FLAG_NONE,
                                                 false,
                                                 0,
                                                 FolderWindow::FileOperationState::ExecutionMode::PerItem,
                                                 false,
                                                 verifiedPhase ? nullptr : state.fsDummy);
        if (! state.taskA.has_value())
        {
            fail(std::format(L"{}: the leaf Copy did not start ({} phase).", label, verifiedPhase ? L"verified" : L"zero-byte"));
            return true;
        }
        state.stepState = verifiedPhase ? 4u : 1u;
        return false;
    }

    if (state.stepState == 1u || state.stepState == 4u)
    {
        if (state.taskA.has_value() && state.completedTasks.contains(state.taskA.value()))
        {
            fail(std::format(L"{}: the task finished before the held leaf checkpoint ({} phase).", label, verifiedPhase ? L"verified" : L"zero-byte"));
            return true;
        }
        if (! HasFileOpsSelectedLeafDiscoveryPublishedPauseEnteredForSelfTest())
        {
        ...
        // A known empty file is a known total of zero bytes, never an unknown total.
        if (snapshot.finished || ! snapshot.discoveryClosed || snapshot.discoveredTotalBytes != expectedBytes || snapshot.discoveredFileCount != 1ul ||
            snapshot.totalItems != 1ul)
        {
            fail(std::format(L"{}: the held leaf must publish an exact known total ({} phase: finished={} closed={} bytes={}/{} files={} items={}).",
                             label,
                             verifiedPhase ? L"verified" : L"zero-byte",
                             snapshot.finished ? 1 : 0,
                             snapshot.discoveryClosed ? 1 : 0,
                             snapshot.discoveredTotalBytes,
                             expectedBytes,
        ...
        DebugResolveFileOperationsWholeTaskProgress(snapshot, presentation);
        const float completeFraction = DebugComputeFileOperationsTaskCompleteFraction(snapshot);
        const bool fractionSane      = presentation.fraction >= 0.0 && presentation.fraction <= 1.0 && completeFraction >= 0.0f &&
                                  completeFraction <= 1.0f && std::isfinite(completeFraction);
        if (! presentation.determinate || presentation.provisional || presentation.showDiscoveryActivity || ! fractionSane)
        {
            fail(std::format(L"{}: a closed leaf total must render determinate finite progress with no discovery activity "
                             L"({} phase: determinate={} provisional={} discoveryActivity={} fraction={} hosted={}).",
                             label,
                             verifiedPhase ? L"verified" : L"zero-byte",
                             presentation.determinate ? 1 : 0,
                             presentation.provisional ? 1 : 0,
        ...
        FileOperationsPopupInternal::TaskSnapshot openScope = snapshot;
        openScope.discoveryClosed                           = false;
        if (knownTotalHeader.empty() || knownTotalHeader == DebugBuildFileOperationsTaskHeaderText(openScope, nowTick))
        {
            fail(std::format(L"{}: the item header still withholds the denominator ({} phase, header='{}').",
                             label,
                             verifiedPhase ? L"verified" : L"zero-byte",
                             knownTotalHeader));
            return true;
        ...
        disarm();
        state.markerTick = 0u;
        state.stepState  = verifiedPhase ? 5u : 2u;
        return false;
    }
    ...
    const CompletedTaskInfo& info = completed->second;
    if (FAILED(info.hr) || ! info.discoveryClosed || info.discoveredTotalBytes != expectedBytes || info.discoveredFiles != 1ul ||
        info.discoveryGrowthAfterCloseCount != 0ull)
    {
        fail(std::format(L"{}: the completed leaf must keep its exact totals ({} phase: hr=0x{:08X} closed={} bytes={}/{} files={} growthAfterClose={}).",
                         label,
                         verifiedPhase ? L"verified" : L"zero-byte",
                         static_cast<unsigned long>(info.hr),
                         info.discoveryClosed ? 1 : 0,
    ...
    Debug::Perf::Emit(L"FileOps.SelfTest.DiscoveryScope.ZeroByteAndVerifiedLeaf",
                      verifiedPhase ? L"phase=verified-same-endpoint" : L"phase=zero-byte-cross-provider",
                      info.discoveryDurationUs,
                      info.discoveredTotalBytes,
                      info.discoveryGrowthAfterCloseCount,
                      info.hr);
    ...
    restoreVerifySetting();
    disarm();
    NextStep(state, SelfTestState::Step::DiscoveryScope_LocalDirectApi);
    return false;
}

case SelfTestState::Step::DiscoveryScope_LocalDirectApi:
{
    // The provider's own API surface, exercised without the host. One governed call is one
    // discovery scope: the caller gets one cumulative stream that never decreases and exactly one
    // closure, emitted after the last selected root. Before this repair bulk Copy reported nothing
    // at all, bulk Move closed the call scope on its first root, and direct Delete never closed.
    constexpr std::wstring_view label = L"DiscoveryScope_LocalDirectApi";
    const ULONGLONG nowTick           = GetTickCount64();
    if (HasTimedOut(state, nowTick, 120'000ull))
    {
        Fail(std::format(L"{} timed out.", label));
    ...
    if (! state.fsLocal)
    {
        Fail(std::format(L"{} requires the Local provider.", label));
        return true;
    }

    // Every call runs even when an earlier one is already wrong, so one run witnesses all three
    // defects instead of stopping at the first.
    std::vector<std::wstring> scopeDefects;
    const auto appendDefect = [&scopeDefects](std::wstring defect)
    {
        if (! defect.empty())
        {
            scopeDefects.push_back(std::move(defect));
        }
    };

    constexpr uint64_t kLeafBytesA    = 16u * 1024u + 7u;
    constexpr uint64_t kLeafBytesB    = 32u * 1024u + 11u;
    constexpr uint64_t kLeafBytesC    = 48u * 1024u + 13u;
    constexpr uint64_t kTreeChildBytes = 8u * 1024u + 3u;
    constexpr unsigned int kTreeChildCount = 4u;
    constexpr uint64_t kLeafBytes     = kLeafBytesA + kLeafBytesB + kLeafBytesC;
    constexpr uint64_t kTreeBytes     = static_cast<uint64_t>(kTreeChildCount) * kTreeChildBytes;
    // Copy walks the selected directory, so its totals include the tree. Move is one rename per
    // root: it relocates the whole subtree without enumerating it, so the directory root
    // contributes one directory and no descendant files or bytes. Delete walks what it removes.
    constexpr uint64_t kExpectedCopyBytes   = kLeafBytes + kTreeBytes;
    constexpr uint64_t kExpectedCopyFiles   = 3u + kTreeChildCount;
    constexpr uint64_t kExpectedMoveBytes   = kLeafBytes;
    constexpr uint64_t kExpectedMoveFiles   = 3u;
    constexpr uint64_t kExpectedDeleteBytes = kTreeBytes;
    constexpr uint64_t kExpectedDeleteFiles = kTreeChildCount;

    const std::filesystem::path base        = state.tempRoot / L"discovery-scope-direct-api";
    const std::filesystem::path sourceRoot  = base / L"src";
    const std::filesystem::path copyDest    = base / L"copy-dst";
    const std::filesystem::path moveDest    = base / L"move-dst";
    const std::filesystem::path treeRoot    = sourceRoot / L"tree";
    const FileSystemFlags flags             = static_cast<FileSystemFlags>(FILESYSTEM_FLAG_RECURSIVE);

    const auto seedSelection = [&]() noexcept -> bool
    {
        if (! RecreateEmptyDirectory(sourceRoot) || ! RecreateEmptyDirectory(treeRoot))
        {
            return false;
        }
        if (! WriteTestFile(sourceRoot / L"leaf-a.bin", static_cast<size_t>(kLeafBytesA)) ||
            ! WriteTestFile(sourceRoot / L"leaf-b.bin", static_cast<size_t>(kLeafBytesB)) ||
            ! WriteTestFile(sourceRoot / L"leaf-c.bin", static_cast<size_t>(kLeafBytesC)))
        {
            return false;
        }
        for (unsigned int index = 0u; index < kTreeChildCount; ++index)
    ...
    const std::wstring leafA = (sourceRoot / L"leaf-a.bin").wstring();
    const std::wstring leafB = (sourceRoot / L"leaf-b.bin").wstring();
    const std::wstring leafC = (sourceRoot / L"leaf-c.bin").wstring();
    const std::wstring tree  = treeRoot.wstring();
    const std::array<const wchar_t*, 4> selection{leafA.c_str(), leafB.c_str(), leafC.c_str(), tree.c_str()};

    const auto makeOptions = [](DirectApiDiscoveryRecorder& recorder) noexcept
    {
    ...
    // Bulk Copy: four selected roots, three of them plain files and one a recursive directory.
    if (! seedSelection() || ! RecreateEmptyDirectory(copyDest))
    {
        Fail(std::format(L"{}: could not seed the bulk Copy fixture.", label));
        return true;
    }
    {
        DirectApiDiscoveryRecorder recorder{};
        FileSystemOptions options = makeOptions(recorder);
        const std::wstring destination = copyDest.wstring();
        const HRESULT hr = state.fsLocal->CopyItems(
            selection.data(), static_cast<unsigned long>(selection.size()), destination.c_str(), flags, &options, nullptr, nullptr);
        if (FAILED(hr))
        {
            Fail(std::format(L"{}: direct CopyItems failed (hr=0x{:08X}).", label, static_cast<unsigned long>(hr)));
            return true;
        }
        appendDefect(DescribeDiscoveryScopeDefect(L"CopyItems", recorder, {kExpectedCopyBytes, kExpectedCopyFiles}));
    }

    // Bulk Move: the same selection shape, where the first root used to close the whole call.
    if (! seedSelection() || ! RecreateEmptyDirectory(moveDest))
    {
        Fail(std::format(L"{}: could not seed the bulk Move fixture.", label));
        return true;
    }
    {
        DirectApiDiscoveryRecorder recorder{};
        FileSystemOptions options = makeOptions(recorder);
        const std::wstring destination = moveDest.wstring();
        const HRESULT hr = state.fsLocal->MoveItems(
            selection.data(), static_cast<unsigned long>(selection.size()), destination.c_str(), flags, &options, nullptr, nullptr);
        if (FAILED(hr))
        {
            Fail(std::format(L"{}: direct MoveItems failed (hr=0x{:08X}).", label, static_cast<unsigned long>(hr)));
            return true;
        }
        appendDefect(DescribeDiscoveryScopeDefect(L"MoveItems", recorder, {kExpectedMoveBytes, kExpectedMoveFiles}));
    }

    // Direct Delete of one recursive root, which used to report only open records forever.
    if (! seedSelection())
    {
        Fail(std::format(L"{}: could not seed the Delete fixture.", label));
        return true;
    }
    {
        DirectApiDiscoveryRecorder recorder{};
        FileSystemOptions options = makeOptions(recorder);
        const std::wstring target = treeRoot.wstring();
        const HRESULT hr          = state.fsLocal->DeleteItem(target.c_str(), flags, &options, nullptr, nullptr);
        if (FAILED(hr))
        {
            Fail(std::format(L"{}: direct DeleteItem failed (hr=0x{:08X}).", label, static_cast<unsigned long>(hr)));
            return true;
        }
        appendDefect(DescribeDiscoveryScopeDefect(L"DeleteItem", recorder, {kExpectedDeleteBytes, kExpectedDeleteFiles}));
        if (std::filesystem::exists(treeRoot))
        {
            Fail(std::format(L"{}: direct DeleteItem left the selected root in place.", label));
            return true;
        }
    }

    if (! scopeDefects.empty())
    {
        std::wstring joined;
        for (const std::wstring& defect : scopeDefects)
        {
            if (! joined.empty())
            {
                joined.append(L" | ");
        ...
        Fail(std::format(L"{}: {}", label, joined));
        return true;
    }

    Debug::Perf::Emit(L"FileOps.SelfTest.DiscoveryScope.LocalDirectApi",
                      L"calls=CopyItems,MoveItems,DeleteItem",
                      0u,
                      kExpectedCopyBytes,
                      kExpectedCopyFiles,
                      S_OK);
    NextStep(state, SelfTestState::Step::DiscoveryScope_DummyDirectApi);
    return false;
}
...
{
    using Task                  = FolderWindow::FileOperationState::Task;
    const auto releaseCopyPause = []() noexcept
    {
    ...
    // seeded Dummy provider (5 ms per access). Its first publication is held until the
    // Delete has actually entered its queue wait (or completed the explicit cancel).
    ...
    if (state.stepState == 0u)
    {
        ...
        // The configured Dummy instance survives same-process repeats. Clear this
        // case's destination so an old Copy cannot introduce an unrelated overwrite prompt.
        const HRESULT resetDestinationHr = state.fsDummy->DeleteItem(
            dummyDest.c_str(), static_cast<FileSystemFlags>(FILESYSTEM_FLAG_RECURSIVE | FILESYSTEM_FLAG_ALLOW_REPLACE_READONLY), nullptr, nullptr, nullptr);
        if (FAILED(resetDestinationHr) && resetDestinationHr != HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND) &&
            resetDestinationHr != HRESULT_FROM_WIN32(ERROR_PATH_NOT_FOUND))
        {
            fail(std::format(L"{}: could not reset the Dummy destination (hr=0x{:08X}).", label, static_cast<unsigned long>(resetDestinationHr)));
            return true;
        }
    ...
    if (state.stepState == 1u)
    {
        ...
        if (copyTask == nullptr || ! copyTask->HasEnteredOperation() || ! HasFileOpsLiveOutputPublishedPauseEnteredForSelfTest())
        {
            if (state.completedTasks.contains(state.taskA.value()))
            {
    ...
    if (state.stepState == 4u)
    {
        if (state.completedTasks.contains(state.taskA.value()))
        {
            fail(std::format(L"{}: the held Copy completed before the Delete decision was observed.", label));
            return true;
        }
        if (dontStart)
        {
        ...
        {
            Task* deleteTask = state.fileOps->FindTask(state.taskB.value());
            if (deleteTask == nullptr || ! deleteTask->IsOverlapQueued() || ! deleteTask->IsWaitingInQueue())
            {
                if (state.completedTasks.contains(state.taskB.value()))
                {
                    fail(std::format(L"{}: the Delete completed before its queued wait was observed.", label));
                    return true;
                }
                return false;
            }
        }
        AppendLog(std::format(L"{}: releasing held Copy after observing the Delete {}.", label, dontStart ? L"cancellation" : L"queue wait"));
        releaseCopyPause();
        state.stepState = 3u;
        return false;
    }
...
case SelfTestState::Step::Phase5_DiscoverySkipContinues:
{
    ...
    // Skip is a host mode switch on a traversal that is still producing, so the case drives the
    // Local provider: its walker runs ahead behind a bounded queue and observes the switch at its
    // next checkpoint. A provider that already holds its whole plan, such as Dummy, closes on the
    // exact total before its first byte moves and leaves nothing to skip.
    ...
    const std::filesystem::path destinationA     = state.tempRoot / L"discovery-skip-dst-a";
    const std::filesystem::path destinationB     = state.tempRoot / L"discovery-skip-dst-b";
    ...
    if (state.stepState == 0)
    {
        ...
        // Bounded producer queues plus a low transfer rate keep the single-pass traversal open long
        // enough to observe the switch, the same fixture the Parallel/Queue transition cases use.
        const std::string config =
            R"json({"concurrencyMode":"manual","copyMoveMaxConcurrency":1,"deleteMaxConcurrency":1,"deleteRecycleBinMaxConcurrency":1,"enumerationSoftMaxBufferMiB":512,"enumerationHardMaxBufferMiB":2048})json";
        ...
        {
            Fail(L"Failed to reset destinations for the discovery-Skip test.");
            return true;
        }
        ...
        if (! state.taskA.has_value())
        {
            Fail(L"Failed to start the active local copy task for discovery-Skip testing.");
            return true;
        }
    ...
    if (state.stepState == 1)
    {
        ...
        if (! state.taskB.has_value())
        {
            Fail(L"Failed to start the queued local copy task for discovery-Skip testing.");
            return true;
        }
...
case SelfTestState::Step::Phase6_PopupRateSmoothing:
{
    ...
    if (DebugResolveFileOperationsHostedButtonVariant(Kind::FooterQueueMode) != DxUi::ButtonVariant::Selector ||
        DebugResolveFileOperationsHostedButtonVariant(Kind::TaskSpeedLimit) != DxUi::ButtonVariant::Selector ||
        DebugResolveFileOperationsHostedButtonVariant(Kind::TaskDestination) != DxUi::ButtonVariant::DropDown ||
        DebugResolveFileOperationsHostedButtonVariant(Kind::FooterOptions) != DxUi::ButtonVariant::IconOnly ||
        DebugResolveFileOperationsHostedButtonVariant(Kind::CompletedGroupToggle) != DxUi::ButtonVariant::Disclosure ||
        DebugResolveFileOperationsHostedButtonVariant(Kind::TaskToggleCollapse) != DxUi::ButtonVariant::Disclosure ||
        DebugResolveFileOperationsHostedButtonVariant(Kind::TaskPause) != DxUi::ButtonVariant::Standard)
    {
...
case SelfTestState::Step::Phase6_LocalBandwidthThrottle:
{
    ...
    constexpr ULONGLONG kFirstObservationDelayMs   = 500ull;
    ...
    if (state.stepState == 1)
    {
        ...
        {
            const ULONGLONG operationStartTick = task->_operationStartTick.load(std::memory_order_acquire);
            // Keep pumping messages, but deliberately observe late. Duration starts at the
            // worker's published start, never at a delayed UI poll that clips real copy time.
            if (operationStartTick == 0 || nowTick < operationStartTick || (nowTick - operationStartTick) < kFirstObservationDelayMs)
            {
                return false;
            }
            state.localBandwidthRunStartTick = operationStartTick;
            AppendLog(std::format(L"Phase6_LocalBandwidthThrottle firstObservationDelay={}ms workerStart={} firstObservation={}",
                                  nowTick - operationStartTick,
                                  operationStartTick,
                                  nowTick));
        }
        if (task && state.localBandwidthRunStartTick != 0)
