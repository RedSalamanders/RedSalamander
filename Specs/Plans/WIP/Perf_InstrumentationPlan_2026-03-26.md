...
- [ ] Run folder-pane metric scenarios and archive the baseline report.
...
- folder-pane-specific perf scenario baseline that emits stable `render.*` and `icons.*` data outside FileOps/startup noise
...
These runs pass and archive perf JSONL correctly, but they mostly emit startup and `iconcache.*` metrics. They do not yet give a clean, repeated folder-pane baseline for the full `render.*` / `icons.*` set, so the top-level folder-pane baseline checklist item remains open on purpose.
...
- the scenario reaches the folder-pane codepaths more reliably than the older empty-state/watermark smokes and is good enough to keep as the provisional folder baseline case for enumeration-heavy regressions
- the Commands harness still does not archive `render.*`, `FolderView.DeferredInit`, or `icons.*` for this case, even after explicit render warmup from the selftest path, so the original render/icon baseline checklist item remains partially open rather than fully complete
- a stronger debug-only capture helper that directly invoked deferred init, `Render(...)`, icon queue processing, and a second render still did not surface `render.*` or `icons.*` in the archived JSONL for `2026-03-27_153032`
- a follow-up rerun (`2026-03-27_153447`) added a debug-only selftest assertion proving the warm-render helper call count increments inside the passing scenario, yet the helper's own `folder.selftest.render_warmup_us` emit still did not appear in the archived JSONL
- the JSONL sink was then hardened to serialize writes and allow shared-write opens before rerunning the scenario (`2026-03-27_154032`), but the folder render/icon rows were still absent
- a final fallback attempt exposed debug render/icon counters through the selftest and emitted `folder.selftest.*` counters directly from the Commands test path (`2026-03-27_154242`), and those rows were also absent from the archived JSONL
- that combination now makes the failure mode much clearer: the missing folder render/icon rows are no longer explained by a weak scenario or by the helper not running; this looks like a Commands-harness/capture-boundary limitation
