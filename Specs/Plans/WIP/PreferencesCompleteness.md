...
### 3. Viewers Pane
**Status:** 🟡 Partial (minor dead code issue)
...
**Broken:**
- ⚠️ **`SyncDxEditsFromState()` contains dead code: `if (true) return;`** (Preferences.Viewers.cpp line 536). The extension and viewer editor TextFields are NEVER synced from state during a refresh cycle. They are only populated via `UpdateEditorFromSelection()` on grid selection change. If an external event forces a refresh without changing selection, the edit fields go stale. Likely a debugging leftover or intentional workaround that was never cleaned up.
...
## Bug: `SaveSettingsFromDialog` Missing `showHiddenFiles`/`showSystemFiles` Merge
...
**Fix:** Add `folders->showHiddenFiles` and `folders->showSystemFiles` to the explicit merge block alongside `historyMax`.

---

## Bug: Viewers `SyncDxEditsFromState` Dead Code
...
**Fix:** Remove the `if (true) return;` guard, or replace with the intended conditional logic.
...
### P0 — Data Correctness
1. **Fix `SaveSettingsFromDialog` showHiddenFiles/showSystemFiles merge** — user changes lost on save to disk

### P1 — Code Quality
2. **Remove dead `if (true) return;` in Viewers `SyncDxEditsFromState()`** — stale editor fields on external refresh

### P2 — Feature Completeness (future)
3. **Editors pane implementation** — blocked on data model (connections vs. external editors)
4. **Mouse pane implementation** — blocked on action/shortcut model
5. **Global "Reset All to Defaults" button** — currently only per-pane resets exist
6. **`extensions.openWithFileSystemByExtension` UI** — no UI for filesystem plugin extension mapping

---

## Overall Assessment

**The Preferences dialog is in excellent shape.** 9 of 11 panes are fully wired and functional, covering 80+ settings with proper read/write/sync cycles. The 2 placeholder panes are explicitly spec'd as placeholders. The one real bug (showHiddenFiles save merge) is a narrow data-loss scenario that should be fixed promptly. The dead code in Viewers is low-priority cleanup.
