...
> **FILE OPERATIONS CONTRACT (2026-08-21).** Re-verify the cross-provider Move cluster against bound exact-source cleanup. Optional content verification does not authorize source deletion, and a mandatory destination reread is no longer the deletion proof.

> **HOLD.** Do not pick items from this file independently. FolderView perf closeout is `Operation_FolderView_WarpDrive_RemainingCloseout_2026-08-25.md` (I2). TW-2/TW-3/TW-D1 cleanup is the thumbnail follow-up (I1). Retired DxUi/WarpDrive continuation batons live in `Specs/Plans/Done/` and must not be resumed. Do not execute TW-5/TW-23/TW-24 via those batons.
...
[[Operation_FolderView_WarpDrive_RemainingCloseout_2026-08-25]] (remaining closeout; 2026-06-28 master frozen in Done), `Operation_IronLedger...` (FolderView data-safety from the audit), and
