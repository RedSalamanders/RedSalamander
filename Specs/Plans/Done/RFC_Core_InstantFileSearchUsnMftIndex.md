...
Closeout audit (`2026-04-25`): Done. The NTFS MFT/USN implementation, SQLite-backed service behavior, degraded no-wait fallback, and listed selftest contracts are now represented in `Common/LocalSearchIndexCore.cpp`, `Common/SearchServiceBroker.cpp`, `RedSalamander/FindFilesWindow.cpp`, `RedSalamander/SelfTest/Commands/Commands.SelfTest.Search.cpp`, `RedSalamander/SelfTest/CompareDirectories/CompareDirectoriesEngine.SelfTest.Cases.SearchAndIndex.cpp`, and the authoritative `Specs/Core/Core_Search.md`.
