> **NON-NORMATIVE RETIRED RECORD.** Moved from `Specs/Plans/WIP/` on 2026-08-25. **No remaining action on this file.** This is frozen competitive analysis, not an implementation queue. Remaining unresolved routing lives in `Specs/Plans/WIP/Product_WhimFilesRemainingDecisions_2026-08-25.md` (D1). Do not implement G1–G24 from this archive. Do not recreate root `plans/`.

## Archive status

- **State:** RETIRED
- **Archived:** 2026-08-25
- **Original path:** `Specs/Plans/WIP/Product_WhimFilesGapAnalysisAndImprovementPlan_2026-07-08.md`
- **Live owner:** Specs/Plans/WIP/Product_WhimFilesRemainingDecisions_2026-08-25.md (D1 DECISION routing only; G2 authority is settled under File Operations and implementation remains deferred)
- **No remaining action:** yes

> **Frozen history: do not resume.** Waves 1–5, gap specifications, and "Draft for review (no code changes yet)" below are archive text. **No remaining action on this file.** If a gap is promoted, create or extend an ACTIVE owner; do not resume this file.

---
# RedSalamander vs. the File-Manager Field — Feature Gap Analysis & Improvement Plan

> **FILE OPERATIONS RESOLUTION (2026-08-21).** `G2` remains deferred, but its authority is settled: Undo may act only on exact identities and Proven Engine artifacts from typed per-item results. It may never infer created or removed objects from pathname sets. The File Operations program does not promise a general recovery journal.


**Date:** 2026-07-08
**Status:** Frozen history (was Draft for review; no remaining action on this file)
...
| G2 | **Undo for file operations** (copy/move/rename/recycle) | None — app mutates files with no undo (historical spike archived to Done; not an owner) | **P0** |
...
**Why it matters:** RedSalamander mutates user files and offers no undo — a safety gap the project already recognizes. The 2026-06 design spike is archived at `Specs/Plans/Done/AdvisorPlan_013-undo-file-operations-spike_2026-06-11.md` and is **not** an implementation owner. WhimFiles ships undo for moves and trash. Given RedSalamander's documented history of data-loss reviews (see memory: Riptide/Fairstream/Floodgate cloud data-safety findings), a robust undo is high-value insurance, not just polish.
...
2. **Recording point.** A future Undo design may consume only the engine's typed per-item completion
   receipts: qualified endpoints, exact source/destination object or revision identities,
   `PublicationState`, `SourceDisposition`, and Proven Engine artifact tokens. A pathname set,
   conflict-resolution list, or success HRESULT is not an inverse journal and cannot authorize
   deletion, replacement, or restoration. The File Operations program itself creates no general
   Undo journal; G2 remains deferred until a separate design defines bounded receipt retention and
   exact revalidation.
...
- Consume the archived `Specs/Plans/Done/AdvisorPlan_013-undo-file-operations-spike_2026-06-11.md` spike only as historical input if a future ACTIVE owner is promoted; produce the design doc it referenced (`Specs/Plans/WIP/Core_FileOperationsUndo.md`, currently absent). `plans/013` is not a live owner.

**Acceptance criteria**
- Move → Undo restores originals to source paths and removes them from destination.
- Rename → Undo restores the original names.
- Copy → Undo may delete only the still-matching exact publication identified by a Proven Engine
  receipt; it never deletes by destination pathname and never touches a replacement or pre-existing
  file.
...
- The retired scratch note contained similar requests, but it was not product authority. Command-palette direction remains an explicit decision in this analysis until promoted through the current WIP index.
...
**Current state**
- RedSalamander does the *reverse* well ("Command Shell" / "Insert Current Directory in Terminal"), but has no documented shell helper to open a path *in RedSalamander* from the terminal, the way WhimFiles ships `wf` for zsh/bash.
...
4. **G2 Undo** (M–L) — data-safety insurance; a future ACTIVE owner may consume the archived 013 spike.
...
5. **G2 Undo** (M–L) — data-safety insurance; a future ACTIVE owner may consume the archived 013 spike.
...
- **Engine choice:** recommend **JScript via Windows `IActiveScript`** first — zero new runtime dependency, native, and directly familiar to the large Directory Opus scripting community; consider Lua/QuickJS later for a sandboxed modern option. A broader VFS-aware pane automation ABI is not approved by old scratch prose; that separate public-ABI choice is owned by the Atlas decision queue.
