...
### Active Follow-on From Segoe UI Variable Closeout

The `Specs/Plans/Done/UI_SegoeUIVariableAndGdiSurfaceRetirementPlan.md` closeout on 2026-04-23 split one remaining cluster out of the typography/GDI retirement scope: the broad `cmd_app_shortcuts_*` grouped-collapse, reorder, persisted-layout, and search/sort round-trip suite for `ShortcutsWindow`.

This cluster stays here as ongoing DxUi behavior work because:

- the visible typography audit is clean except for the documented `Common/DxUi/DxUi.Typography.h` helper allowlist,
- active product/test source is clean for `ThemedControls`, `NONCLIENTMETRICS`, `lfMenuFont`, `GetTextExtentPoint32W`, `GetTextMetricsW`, and `CreateMenuFontForDpi`,
- targeted migrated-surface Shortcuts coverage is already green for shell contract, live search, tooltip tracking, and long-run open/close stability.

Follow-on evidence anchors:

- broad shortcuts family red archive: `Specs/TestRuns/7d3a1247382a/Commands/2026-04-23_201922/`
- focused long-run scrolling rerun after the selftest resize-baseline fix: `Specs/TestRuns/7d3a1247382a/Commands/2026-04-23_202549/`
- related green shell/live-search anchor archives: `Specs/TestRuns/7d3a1247382a/Commands/2026-04-23_200627/` and `Specs/TestRuns/7d3a1247382a/Commands/2026-04-23_201922/`

### Live Remaining Work Checklist

Last refreshed: 2026-04-25.

The original framework-gap inventory above is historical. The current tree already contains the shared `DxUi` menu, menu bar, toolbar, radio buttons, progress bar, tab control, multi-section `StatusStrip`, tracking tooltip behavior, and shared scrollbar helpers. `ThemedControls.cpp` has been retired. Do not reopen those framework gaps from this roadmap unless a new regression proves the implementation no longer satisfies its authoritative spec.

Remaining closeout work for this plan:

- [x] Confirm current status from checked-in evidence and source: remaining live scope is the `ShortcutsWindow` grouped/grid behavior cluster, not missing core `DxUi` primitives.
- [x] Reproduce the focused red guard `cmd_app_shortcuts_long_run_scrolling_stays_bounded` on the current tree and capture the exact failure mode. Current same-machine archive: `Specs/TestRuns/4cb089111a23/Commands/2026-04-24_085349/`; failure: `Shortcuts window did not settle after narrow-width validation resize.`
- [x] Fix the `ShortcutsWindow` long-run scrolling stability regression so the narrow-width validation resize settles, visible row geometry and bounded visible work remain stable across wheel chunks, and unexpected host resize churn stays at zero. Focused guard is green on the current tree.
- [x] Close the focused activation/Escape/header-reorder regressions: `cmd_app_shortcuts_escape_closes_from_search_and_grid`, `cmd_app_shortcuts_grid_enter_activates_selected_command`, `cmd_app_shortcuts_grid_doubleClick_activates_selected_command`, and `cmd_app_shortcuts_header_drag_reorders_columns_without_sort` are green after the host Escape hook, pixel-accurate activation clicks, and DPI-scaled header drag helpers.
- [x] Fix the broad `cmd_app_shortcuts_` persistence cluster. Latest broad shortcuts archive is 30/30 green: `Specs/TestRuns/4cb089111a23/Commands/2026-04-24_112309/`.
- [x] Confirm `ShortcutsWindow` persisted view-state fields are mandatory settings and schema-backed: `functionBarCollapsed`, `folderViewCollapsed`, `sortColumnId`, `sortDescending`, and `gridLayout` are present in `Specs/SettingsStore.schema.json`, `Common/SettingsStore.h`, `Common/Common/SettingsStore.cpp`, and `Specs/Core/Core_SettingsStore.md`.
- [x] Preserve or extend deterministic test coverage before production-code changes when a missing guard is found. `Tests/DxUiTests/DxUiTests.Grid.cpp` now covers display-index header rectangles after column reordering, grouped offscreen programmatic selection, and long-run visible-row boundary clipping; `.\.build\x64\Debug\DxUiTests.exe` is green.
- [x] Align Shortcuts persistence selftests with the durable settings contract. Live selection-preservation checks remain for reorder/resize/sort/collapse mutations, while close/reopen assertions require restored persisted layout/collapse/sort plus a valid selected row, because selected row and search text are transient window state rather than persisted settings fields.
- [x] Archive correctness and performance evidence under `Specs/TestRuns/<MachineHash>/Commands/` for the focused guard and the broad shortcuts family. Closeout archive: `Specs/TestRuns/4cb089111a23/Commands/2026-04-24_112309/`; perf evidence: `Specs/TestRuns/4cb089111a23/Commands/2026-04-24_112309/perf/perf_metrics.jsonl`.
- [x] Update authoritative normative specs with the final `ShortcutsWindow` behavior and validation contract: `Specs/Core/Core_SettingsStore.md`, `Specs/UI/UI_DxUiSharedGrid.md`, and `Specs/Testing/Testing_TestCoverage.md`.
- [x] Fix and verify the focused Find/grid follow-on failures exposed after the Shortcuts cluster passed. Current focused Find prefix archive `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_062926/` is 55 passed / 0 failed after the header-reorder debug helper, final snapshot polling fix, and Find rerun selection restoration.
- [x] Re-run the patched Shortcuts activation ownership cases and `cmd_pane_focusAddressBar_tab_traversal` after the current code changes. Green archives: `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_062938/`, `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_062947/`, and `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_063910/`.
- [x] Re-run the full `Commands` suite with the crash-safe `Tools/Run-AllTests.ps1` JSON parser and verify it reports failures without crashing. Current broad fail-fast archive is 479 passed / 0 failed / 0 skipped: `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_091024/`; perf evidence: `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_091024/perf/perf_metrics.jsonl`.
- [x] Run the FileOps/selection coverage implicated by the broader sweep, then either fix any remaining failures or link the active FileOps plan that owns them. The speed-limit shell-stability guard now holds an observable per-item task and is green in `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_085614/`; the speed-limit prompt family is 4/4 green in `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_085628/`; the broader FileOps prefix is 21/21 green in `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_085639/`.
- [x] Re-run `.\.build\x64\Debug\DxUiTests.exe` and settings round-trip coverage for Shortcuts persisted state. `DxUiTests.exe` is green on the current binary; `settings_shortcuts_view_state_roundtrip` is green in both `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_063917/` and the full Commands archive `Specs/TestRuns/4cb089111a23/Commands/2026-04-25_091024/`.
- [x] Run `git diff --check` after final code/spec edits. The check passes on 2026-04-25 with only Git line-ending conversion warnings.
- [x] Final normative-spec pass: confirm `Specs/Core/Core_SettingsStore.md`, `Specs/UI/UI_DxUiSharedGrid.md`, `Specs/Testing/Testing_TestCoverage.md`, and any touched FileOps/Search specs reflect the final implementation. Updated specs now also cover Find rerun selection restoration, task-scoped FileOps custom speed-limit prompt validation, and the crash-safe `Tools/Run-AllTests.ps1` JSON-result contract.
- [x] Keep this roadmap in `Specs/Plans/Done/` only after the above checklist is green. The WIP copy has been removed and this done copy carries the final green checklist plus archive paths.
