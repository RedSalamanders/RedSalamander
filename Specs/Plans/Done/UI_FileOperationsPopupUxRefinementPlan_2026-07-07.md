...
**Status:** Complete — implemented, specified, performance-validated, and archived to `Specs/Plans/Done/` on 2026-07-22
**Date:** 2026-07-07
...
`Specs/UI/Images/FileOperationsPopup_Product_Conflict_2026-07-10.png`. The former product backlog
(B1 reorder, B4 group animation, B5 keyboard/UIA, C5 Keep Both engine support, and C7 shared DxUi
graph support) is now implemented and represented in the authoritative UI, FileSystem, shared-helper,
...
State meanings: **Done** = implemented and has targeted regression coverage; **In progress** = code
exists but verification/closeout or a sub-requirement is
...
| Done | B1 | Per-task queue control: "Start now" / reorder waiting tasks; Pause-all/Resume-all | Waiting tasks own engine queue-order keys; adjacent Move Up/Down controls, Start now, and footer Pause all/Resume all are implemented with focused queue-admission coverage. | B | M |
...
| Done | B4 | Auto-collapse & group completed tasks; compact vs expanded density | Completed cards auto-collapse on first finished snapshot; persisted Compact/Expanded density and the `Completed (N)` group are implemented with 180 ms cubic motion that snaps under reduced motion. | B | M |
| Done | B5 | Keyboard navigation + UIA/LiveRegion for the whole popup | Interactive actions, progress bars, and graphs are hosted in a same-HWND `DxUi::WindowHost`; keyboard traversal/action routing, stable automation IDs, UIA Invoke/RangeValue semantics, and transition notifications are covered. | B | L |
...
| Done | C5 | Conflict prompt enrichment: "Keep both", stacked source/destination compare | Stacked Source/Destination metadata and engine-backed top-level Keep Both are implemented; unique sibling names are provider-probed and work for same- and cross-filesystem copy/move. | C | M |
| Done | C6 | Motion polish: animated throughput transitions, progress easing, collapse/expand motion | Shared graph latest-value easing, hosted progress easing, queue-thumb/auto-resize motion, and completed group/card motion are implemented with deterministic reduced-motion behavior. | C | S |
| Done | C7 | Rainbow throughput graph mode + shared DxUi graph/gradient support | `DxUi::ThroughputGraph` owns samples, per-stream/rainbow/accent rendering, high-contrast fallback, latest-value easing, and reduced-motion snapping; File Operations consumes it. | C | M |
...
- B4 completed-group follow-up: the popup now inserts a localized `Completed (N)` group when at
  ...
  state, counts, toggle/clear visibility, and per-task hidden-by-group state. Completed-group and
  task-card expansion now use reduced-motion-aware 180 ms cubic motion.
...
**Final closeout gate**
- **Done:** current Release/performance evidence records hosted-control synchronization, queue reorder,
  and popup rendering under `Specs/TestRuns/7d3a1247382a/`.
- **Done with classified baseline exception:** durable behavior is merged into authoritative
  specifications and all task-relevant Debug/Release gates pass. The literal Full wrapper remains
  blocked by the pre-existing, standalone-reproducible
  `cmd_preferences_dialog_plugins_live_search_dx_interaction` UIA/D3D hang archived at
  `Specs/TestRuns/7d3a1247382a/Continuation/2026-07-21_1832_commands_plugins_uia_hang/`; this task
  reproduced the same unrelated boundary after all popup cases passed and did not expand scope into
...
Completed motion slice: group/card expand-collapse uses bounded 180 ms cubic easing and snaps to the
target state when reduced motion is enabled.
...
Completed: the popup retains its same top-level HWND and mirrors each interactive action into a
hosted `DxUi::Button`; progress and bandwidth history use hosted `DxUi::ProgressBar` and
`DxUi::ThroughputGraph` controls. `Tab`/`Shift+Tab`, Enter, card-local arrows, Space, Delete, and
Escape are routed through the host. Stable automation IDs, Invoke/RangeValue semantics,
`WM_GETOBJECT`, provider-count parity, focus-preserving tree rebuilds, and bounded accessibility
notifications are covered by real-window Commands tests.
...
- **C5 — Conflict prompt enrichment.** Completed: the inline conflict prompt captures
  ...
  row when available. A real **"Keep both"** action now selects a provider-neutral unique sibling
  destination and retries through same-filesystem and cross-filesystem copy/move paths; it is not a
  UI-only command.
...
- **C7 — Rainbow throughput graph + DxUi support.** When the app theme is Rainbow
  ...
  Completed DxUi improvement: the reusable `ThroughputGraph` renderer accepts samples, an optional
  limit line, `rainbowSeed`, `reducedMotion`,
  `highContrast`, and transition timing; it internally caches Direct2D solid/linear-gradient brushes and
...
1. **A1 shape:** a segmented Queue/Parallel control exposes both modes.
2. **A2 total bar home:** the single aggregate bar remains in the footer; the duplicate per-item
   under-graph bar was removed.
3. **B1 scope:** the engine owns explicit queue-order keys; the popup exposes adjacent Move Up/Down
   and Start now actions.
4. **A3 taskbar:** the owning main window receives aggregate Windows taskbar progress.
5. **C2 speed control shape:** one menu button shows the current limit and owns preset/custom choices.
6. **C3 persisted collapse state:** popup presentation state round-trips through `SettingsStore`.
7. **C7 DxUi graph primitive:** File Operations uses the shared `DxUi::ThroughputGraph` hosted by the
   popup's same-HWND `DxUi::WindowHost`.
...
## 7. Closeout — 2026-07-22

All A/B/C ledger items are complete. The lasting interaction, progress, motion, accessibility,
queue-order, Keep Both, lifecycle, localization, and performance contracts are merged into
`Specs/UI/UI_FileOperationsPopup.md`, `Specs/FileSystem/FileSystem_FileOperations.md`,
`Specs/Core/Core_SharedHelpers.md`, and `Specs/Testing/Testing_TestCoverage.md`.

Validation:

- Test-enabled x64 Release build passed; final relevant incremental log
  `.build/logs/msbuild-20260722_045147_043.log` has 0 warnings / 0 errors. The earlier complete
  Release build `.build/logs/msbuild-20260722_022949_338.log` had 0 errors and ten unrelated
  pre-existing `ViewerPETests.cpp` unused-symbol warnings.
- Release popup cohort passed 3 / 0 / 0 at
  `20260722T030755Z-58672-91cf0a0db98e469892dd3832d2a3250e`; Debug rerun passed 3 / 0 / 0 at
  `20260722T032524Z-50512-8f9a7715b3f14a66b0dc97567c55d39d`.
- Queue reorder repeated ten times in Release, passing 30 / 0 / 0 at
  `20260722T030441Z-13384-e9bd026e0cb04a799c010feabbf45e05`; the final timing-sensitive Debug
  pre-calculation check repeated five times and passed 15 / 0 / 0 at
  `20260722T034224Z-38644-427ede59a52c42bd96cc786ca4ce412e`.
- Complete Debug FileOps passed 111 / 0 with 20 expected remote-profile/7z-delegation skips at
  `20260722T063305Z-54040-05e9c0ab06804346945781c4508fcd16`; disk audit issues: 0.
- Debug and Release `DxUiTests.exe --suite=Control` passed. Final post-recovery full-solution Debug
  rebuild `.build/logs/msbuild-20260722_085706_283.log` has 0 warnings / 0 errors and leaves no
  artifact-contamination marker.
- Literal Full disposition: popup cases completed before the broad Commands process reached the
  known Preferences/Plugins UIA/D3D hang. The same case also hung in an isolated five-minute repro,
  matching the 2026-07-21 baseline archive cited above. It is unrelated to File Operations and is
  not a closeout blocker for this scoped plan.

Performance evidence:

- `Specs/TestRuns/7d3a1247382a/FileOps/2026-07-22_030448_fileops_popup_queue_reorder_release/`
  retains all ten `FileOps.Queue.ReorderUs` samples: 23 us minimum, 34 us median, 41 us p95,
  57 us maximum.
- `Specs/TestRuns/7d3a1247382a/Commands/2026-07-22_030758_fileops_popup_dxui_host_release/`
  retains hosted-sync distributions: 172 samples, 97 us minimum, 189 us median, 991 us p95,
  3,646 us maximum; rebuild median 734 us and update median 178 us. These are candidate-only
  measurements for new behavior, not improvement claims.
