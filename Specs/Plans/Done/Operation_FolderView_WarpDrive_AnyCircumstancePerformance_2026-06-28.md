> **NON-NORMATIVE RETIRED RECORD.** Moved from `Specs/Plans/WIP/` on 2026-08-25. **SIZE WAIVER retained on this Done archive only.** **No remaining action on this file.** Do not resume from the pause diary or historical boxes. Remaining closeout lives in `Specs/Plans/WIP/Operation_FolderView_WarpDrive_RemainingCloseout_2026-08-25.md` (I2). Thumbnail enrichment is not this archive: `Specs/Plans/WIP/FolderView_ThumbnailBackgroundEnrichmentFollowup_2026-07-04.md` (I1).

## Archive status

- **State:** RETIRED
- **Archived:** 2026-08-25
- **Original path:** `Specs/Plans/WIP/Operation_FolderView_WarpDrive_AnyCircumstancePerformance_2026-06-28.md`
- **Live owner:** Specs/Plans/WIP/Operation_FolderView_WarpDrive_RemainingCloseout_2026-08-25.md (I2 Task 10 Step 4 + Task 11 only)
- **No remaining action:** yes

> **Frozen history: do not resume.** Tasks 0–9 and Task 10 Steps 1–3 are historical. Task 10 Steps 2–3 were measured no-ops (gate not met — DXGI scroll-rects / virtualization V2 are not remaining product work). Every pause-diary "Resume by" sentence and every `[frozen]` box below is archive text. **No remaining action on this file.**

---
# FolderView Warp Drive Implementation Plan

> **FILE OPERATIONS COORDINATION (2026-08-21).** Preserve typed immutable transfer ingress in `FolderView.FileOps.cpp` and `FolderView.DragDrop.cpp`; sequence overlapping edits. `Win32_Inventory.md` `CHK-4` still applies.


> **Frozen historical instructions (do not execute):** this archive used checkbox (`- [frozen]`) syntax for tracking. Do not resume it.
...
- [frozen] **Task 10 — Dirty regions / scroll rects / virtualization.** Done when each sub-item is either implemented behind its passing gate *or* explicitly recorded in the log as "gate not met — not implemented," with the deciding metric cited.

### Phase R5 — Closeout

- [frozen] **Task 11 — Closeout & spec migration.** Done when durable contracts are merged into `Specs/UI/UI_FolderView.md` + `Specs/Testing/*`, `.\Tools\Run-AllTests.ps1 -Suite Full` passes, the full perf matrix (Debug + test-enabled Release) is archived and cited, and this plan is moved to `Specs/Plans/Done/`.

> The detailed `- [frozen]` boxes inside each Task section remain authoritative for the
...
Root plans `Specs/Plans/Done/AdvisorPlan_009-icon-pipeline-investigation_2026-06-11.md` and
`Specs/Plans/Done/AdvisorPlan_010-watcher-incremental-refresh-investigation_2026-06-11.md` are historical prompts
(archived from `plans/009` / `plans/010` on 2026-08-25),
...
- [frozen] **Step 1: Update authoritative contracts**
...
- [frozen] **Step 2: Update testing contracts**
...
- [frozen] **Step 3: Run final verification**
...
- [frozen] Confirmed UI-thread stalls are fixed or explicitly blocked with evidence.
...
- [frozen] Frame-producing perf cases have enough samples for declared p95/p99.
...
- [frozen] Dirty-region, scroll-rect, and virtualization work is implemented only if
  gated by fresh evidence.
- [x] Durable contracts are merged into `Specs/UI/` and `Specs/Testing/`.
- [frozen] Final full-suite result and perf archive paths are recorded.
