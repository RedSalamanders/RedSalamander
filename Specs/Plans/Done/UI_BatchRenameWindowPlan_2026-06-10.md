...
- **Status: Done.** The feature is fully implemented and shipped: `BatchRenameEngine`, `BatchRenameWindow`, `BatchRenameMenus`, and the execution engine are in the tree, with 69 registered `cmd_pane_batchRename_` selftests.
- The shipped implementation was then twice hardened by the two Done Truename plans (2026-06-16 remediation, 2026-06-19 follow-ups), with archived perf evidence: `batchrename.validation.us` 134238236 -> 85016; runs `Specs/TestRuns/4cb089111a23/Commands/2026-06-19_214440/`, `2026-06-19_214739/`, and `2026-06-19_214811/`.
- Durable behavior lives in `Specs/UI/UI_BatchRenameWindow.md` and `docs/BatchRename.md`; this plan is historical.
- Residual low-value polish is deliberately deferred, not lost — preserved as SHOULDs in `Specs/UI/UI_BatchRenameWindow.md:140` and in Phase 15 below: keyboard-semantics selftests (Enter/Ctrl+Enter/Escape/F1), high-contrast + DPI 100/150/200 tests, access-denied/read-only-row and non-local race selftests, the four named perf scenarios, a `Verify-NoSubclassManager.ps1` run, and the File Operations informational-task progress surface.
- Live batch-rename perf findings are owned by Operation_Keystone (KS-P1/KS-P4/KS-S2/KS-S4), not by this plan.
...
- [ ] Respect:
  ...
  - [x] provider cancellation (completed by Truename T-S3-4, archived run 2026-06-19_214811)
    ...
    - 2026-07-02 folder review: `ERROR_CANCELLED` now propagates from provider `ReadDirectoryInfo(...)` without local fallback, and the focused selftest passes (completed by Truename T-S3-4, archived run 2026-06-19_214811).
...
- [ ] Add command selftests:
  ...
  - [x] `cmd_pane_batchRename_target_collection_respects_provider_cancellation` (completed by Truename T-S3-4, archived run 2026-06-19_214811)
    ...
    - 2026-07-02 folder review: the selftest now passes.
...
- [x] Selftests cover engine, launch routing, UI snapshots, and execution. (completed by Truename T-S3-4, archived run 2026-06-19_214811)
- [x] Perf instrumentation and archived runs exist under `Specs/TestRuns/`. (completed by Truename T-S3-4, archived run 2026-06-19_214811)
