...
Archive note:
- Moved from `Specs/Plans/WIP` to `Specs/Plans/Done` on 2026-03-14 after the full checklist was completed and the broader solution build plus repo self-test matrix passed.
...
### Phase 7 - Persistent service store foundation

- [x] Freeze the v1 persistent service-store recommendation as `sqlite3` from vcpkg, with an internal backend abstraction and no public database-specific API.
- [x] Keep the current binary snapshot store as the active implementation until the SQLite path reaches query parity.
- [x] Add shared store metadata types so the core, broker, service status, and foreground TUI can report which persistent backend is active and what maintenance policy applies.
- [x] Surface persistent-store backend and maintenance status through `SearchServiceBroker::GetStatus(...)`.
- [x] Surface persistent-store backend and maintenance status in the foreground service startup banner and live dashboard.
- [x] Add `sqlite3` to `vcpkg.json` and wire manifest-based linkage into the service and self-test build targets.
- [x] Pin a fixed minimum `sqlite3` version that is explicitly accepted for WAL use on Windows.
- [x] Allow foreground service validation runs to select the persistent store backend and override the SQLite database path explicitly.
- [x] Add a shared internal `IIndexStore` contract that isolates mutation/query operations from the NTFS/ReFS crawl and journal logic.
- [x] Add a `SqliteIndexStore` bootstrap/inspection module compiled into both `RedSalamanderSearchService` and self-test targets.
- [x] Bootstrap and inspect the SQLite store during service startup when the SQLite backend is selected, but fail open if the store is missing or invalid so status polling and degraded live-search queries still work.
- [x] Preserve the current snapshot-backed code path behind the same internal store contract until SQLite parity is green.
- [x] Validation: `search_service_status_and_query_roundtrip` and `search_service_foreground_logs_request_status` verify backend/maintenance reporting.
- [x] Validation: `sqlite_index_store_bootstrap_creates_schema` verifies SQLite bootstrap, WAL mode, maintenance policy reuse, and schema presence.
- [x] Validation: `search_service_sqlite_bootstrap_status_roundtrip` verifies service startup creates the SQLite store, reports it through broker status, and still answers a simple query.

#### Phase 7 frozen decisions

- Preferred backend for the always-on service index is **SQLite**, not DuckDB. DuckDB remains a possible future analytics/export backend, not the primary live metadata store.
- V1 remains **metadata-only**: names, hierarchy, attributes, size, and timestamps. File-content scanning and snippets stay client-side.
- Automatic store maintenance is part of the design from day one:
  - SQLite must use `WAL`.
  - Default automatic checkpoint target is `128 MiB`.
  - Automatic compaction uses bounded incremental vacuum style maintenance when fragmentation reaches `>=15%` or reclaimable space reaches `>=64 MiB`.
  - Full `VACUUM` is manual/admin-only, not automatic.

#### Phase 7 exit evidence

- Persistent store metadata scaffolding landed in `Common/LocalSearchIndexCore.h`, `Common/LocalSearchIndexCore.cpp`, `Common/SearchServiceBroker.h`, and `Common/SearchServiceBroker.cpp`.
- SQLite bootstrap/schema inspection landed in `Common/SqliteIndexStore.h` and `Common/SqliteIndexStore.cpp`, with `sqlite3 >= 3.51.2` now pinned in `vcpkg.json`, a runtime WAL-version floor enforced before `PRAGMA journal_mode=WAL`, and both app/service project files updated to compile the module.
- Foreground service CLI selection for `snapshot` vs `sqlite` landed in `RedSalamanderSearchService/Main.cpp`, and service startup now bootstraps SQLite through `Common/SearchServiceBroker.cpp` before accepting clients.
- `Common/LocalSearchIndexCore.cpp` now routes persistence through an internal volume-store contract, with the existing binary snapshot implementation wrapped as the active runtime store while SQLite query parity is still pending.
- The foreground service banner/dashboard now show backend and maintenance policy in `RedSalamanderSearchService/Main.cpp`.
- Verification build passed on 2026-03-13 for `.\build.ps1 -ProjectName RedSalamanderSearchService` and `.\build.ps1 -ProjectName RedSalamander`.
- Focused compare self-tests passed on 2026-03-13:
  - `search_service_status_and_query_roundtrip`
  - `search_service_query_reports_live_progress`
  - `search_service_foreground_logs_request_status`
  - `sqlite_index_store_bootstrap_creates_schema`
  - `local_index_core_sqlite_option_keeps_snapshot_runtime_store`
  - `search_service_sqlite_bootstrap_status_roundtrip`

### Phase 8 - SQLite schema, migration, and import

- [x] Define the service-owned schema:
  - `meta`
  - `volumes`
  - `entries`
- [x] Normalize persisted metadata for each entry:
  - stable file ID
  - parent ID
  - full path
  - folded full path
  ...
  - change time
  - allocation size
- [x] Create the required indexes for exact, prefix, extension, and parent-child lookup.
- [x] Stamp SQLite schema version during bootstrap through both `PRAGMA user_version` and `meta.schema_version`.
- [x] Add schema upgrade handling for future store revisions.
- [x] Add cold-start import from legacy `.bin` snapshots when a volume has no SQLite rows yet.
- [x] Keep legacy `.bin` snapshots as read/import compatibility artifacts for one release cycle; do not delete them automatically.
- [x] Mirror touched-root snapshot state into the SQLite sidecar when `persistentStoreKind=sqlite` so migration data stays current before query cutover.
- [x] Backfill imported legacy rows with live `size_bytes`, `creation_time_100ns`, `last_access_time_100ns`, `write_time_100ns`, `change_time_100ns`, and `allocation_size` metadata during SQLite sidecar mirror so placeholder values are not persisted for accessible entries.
- [x] Surface SQLite migration readiness (`inspectionSucceeded`, mirrored volume/entry counts, `legacyImportVolumeCount`, `readyForQueryCutover`) through broker status and the foreground service TUI.
- [x] Validate both readiness states: broker/TUI must report `readyForQueryCutover=true` for clean mirrored stores and `readyForQueryCutover=false` when legacy-import volumes remain.
- [x] Before SQLite query cutover, add a readiness gate that requires mirrored service-owned roots to leave the legacy-import state (`legacyImportVolumeCount == 0`) or trigger rebuild/backfill before those rows become query-authoritative.
- [x] Add an initial guarded SQLite read-only query path after `EnsureReady(...)` / sidecar mirror so ready stores can serve indexed name queries while blocked stores still fall back to the snapshot path.
- [x] Validation: schema-create, upgrade, import, and corruption-rebuild self-tests pass without changing query semantics.

#### Phase 8 code/test surfaces

- Code: `Common/LocalSearchIndexCore.*`, new `Common/SqliteIndexStore.*`, broker status plumbing, service startup/import path.
- Tests: schema creation/upgrade, snapshot import, corruption recovery, and status reporting.

#### Phase 8 exit evidence

- `Common/SqliteIndexStore.cpp` now rejects newer schema versions, upgrades `user_version=0/1` stores into the current schema, and reports total imported volumes/entries for inspection.
- `Common/LocalSearchIndexCore.cpp` now mirrors snapshot-backed indexed roots into the SQLite sidecar when `persistentStoreKind=sqlite`, backfilling live size/timestamp/allocation metadata when those filesystem reads succeed, and it now cuts ready queries over to a guarded SQLite read path while blocked stores still fall back to the snapshot adapter.
- `Common/SearchServiceBroker.h`, `Common/SearchServiceBroker.cpp`, and `RedSalamanderSearchService/Main.cpp` now surface SQLite migration readiness through broker status and the foreground service banner/dashboard.
- Phase 8 now has the explicit pre-cutover gate in code. The remaining Phase 9 gap is avoiding snapshot `EnsureReady(...)` before SQLite queries so the service can become the true always-on authority instead of a mirrored sidecar.
- Focused compare self-tests passed on 2026-03-13:
  - `sqlite_index_store_bootstrap_creates_schema`
  - `sqlite_index_store_upgrade_paths`
  - `local_index_core_snapshot_corruption_rebuild`
  - `local_index_core_sqlite_option_keeps_snapshot_runtime_store`
  - `local_index_core_sqlite_cold_start_bypasses_snapshot_runtime_store`
  - `local_index_core_sqlite_cutover_blocked_by_pending_legacy_import`
  - `local_index_core_sqlite_sidecar_imports_legacy_snapshot`
  - `local_index_core_sqlite_prefilter_classifies_name_patterns`
  - `search_service_sqlite_bootstrap_status_roundtrip`
  - `search_service_sqlite_cold_start_bypasses_snapshot_runtime_store`
  - `search_service_sqlite_prefilter_roundtrip`
  - `search_service_sqlite_status_reports_pending_legacy_import`
  - `search_service_foreground_logs_request_status`
  - `local_search_service_matches_host_fallback`
  - `search_service_default_identity_matches_build`

### Phase 9 - Service-owned all-volume indexing and query cutover

- [x] Auto-discover all fixed local roots on service start:
  - `C:`
  - `D:`
  - `E:`
  - `S:`
  - `V:`
  - `Z:`
- [x] Surface the discovered fixed-root set through service status so startup discovery is observable and regression-testable before the first query.
- [x] Warm discovered startup roots into the SQLite store in the background when the service owns the default store, and allow a root-list override for selftests/diagnostics so startup warmup can be validated without indexing every fixed volume on the machine.
- [x] Surface startup background-warmup progress/state through broker status and the foreground TUI outside explicit query requests, so pre-query seeding is observable without relying on indirect indexed-count changes or debug logs.
- [x] Keep `FSCTL_ENUM_USN_DATA` and `FSCTL_READ_USN_JOURNAL` for NTFS seeding/replay.
- [x] Keep traversal seed and rebuild fallback when journal/MFT access is unavailable.
- [x] Keep ReFS traversal seed and journal replay where supported.
- [x] Split the shared core into:
  - crawl/journal mutation production
  - store mutation application/query execution
- [x] Make the service the single writer and keep query paths read-only/concurrent.
- [x] Add an initial guarded SQLite query path for ready mirrored roots, with snapshot fallback when the global readiness gate is closed or SQLite enumeration fails.
- [x] Replace per-query full SQLite store inspection with cached/service-owned readiness state so the cutover gate does not re-count the database on every indexed query.
- [x] Allow warm SQLite-backed queries to bypass snapshot reload once a ready mirrored root already exists in the SQLite store.
- [x] Remove the dependency on snapshot `EnsureReady(...)` for SQLite-backed queries entirely when a ready mirrored root already exists; cold/startup local and service-owned reads now answer directly from SQLite without creating or loading the `.bin` snapshot runtime store.
- [x] Before allowing a cold/startup direct SQLite read, validate the stored per-volume journal cursor against the live journal when that state is queryable; if stale, force replay/rebuild before SQLite answers.
- [x] Remove the remaining snapshot-first seed path for brand-new or not-yet-mirrored roots; first-seed SQLite mode now persists directly to SQLite and only falls back to `.bin` compatibility snapshots if the initial mirror fails.
- [x] Define the no-live-journal policy for cold/startup direct SQLite reads in local/non-elevated contexts; direct SQLite is now blocked unless a readable live journal cursor proves the stored root is current, and the query refreshes before answering otherwise.
- [x] Keep no-journal rebuilds snapshotless in SQLite mode when no compatibility snapshot artifact exists already; repeated warm queries in a long-lived local/service process must not recreate `.bin` files just because the root stayed initialized in memory.
- [x] Return persisted `size` and `last-write` metadata directly from the SQLite store, through broker query batches, so warm indexed results no longer pay per-result filesystem metadata I/O for those fields.
- [x] Persist and transport the remaining indexed metadata needed to fully eliminate fallback re-stat for all search result fields:
  - creation time
  - last access time
  - change time
  - allocation size
- [x] Bump the named-pipe protocol/default pipe identity when the candidate batch wire format changes; persisted-metadata transport now uses protocol/pipe `v3`.
- [x] Classify query patterns for efficient prefiltering:
  - literal name
  - `prefix*`
  - `*.ext`
  - complex wildcard/regex with C++ post-filtering
- [x] Surface whether SQLite name prefiltering fired through local/service query stats so regression tests can verify pattern classification without scraping debug logs.
- [x] Validation: the first SQLite seed for a brand-new root does not create a compatibility `.bin` snapshot runtime artifact in local or service mode.
- [x] Validation: cold/startup direct-bypass cases require a readable live journal cursor, and stale-root refresh cases rebuild/replay without depending on compatibility snapshot artifacts.
- [x] Normalize compatibility-snapshot reporting so SQLite refresh/direct-query stats do not advertise a `.bin` path when no compatibility snapshot file exists.
- [x] Validation: service-backed indexed name queries match current local-index/fallback results.
- [x] Validation: warm service-backed indexed first-batch latency stays within the target budget (`<100 ms`) on the broker batch callback path.
- [x] Validation: service startup status reports the discovered fixed local roots before the first indexed query runs.
- [x] Surface the last startup-warmup failure root/HRESULT through broker status and the foreground TUI.
- [x] Split the default ProgramData store roots by build so Debug and Release can run side by side without sharing SQLite state.
- [x] Keep status/query handling non-blocking during startup warmup by deriving pre-query DB state from persistent-store inspection and startup-warmup snapshots.
- [x] When a direct SQLite query fails after already emitting early hits, restart on the live filesystem path without duplicating those earlier hits.
- [x] Treat redirected `--run-foreground` logs as part of the diagnostics surface and include DB state, sync state, and query execution mode there too.

#### Phase 9 code/test surfaces

- Code: `Common/LocalSearchIndexCore.*`, `Common/SearchServiceBroker.*`, `Plugins/FileSystem/FileSystem.Search.cpp`, `RedSalamanderSearchService/Main.cpp`.
- Tests: parity against fallback, live update replay, warm-query latency, and status/TUI progress for long-running seed/rebuild work.

#### Phase 9 progress evidence

- `Common/SqliteIndexStore.cpp`, `Common/SearchServiceBroker.cpp`, and `Plugins/FileSystem/FileSystem.Search.cpp` now persist, transport, and consume the full indexed metadata set needed for result shaping without fallback per-hit filesystem metadata I/O.
- `Common/SearchServiceBroker.h` now uses protocol/default pipe `v3` so mixed old/new binaries fail fast instead of misparsing the expanded candidate batch wire format.
- `Common/LocalSearchIndexCore.cpp` now validates the stored SQLite journal cursor against the live journal before a cold/startup direct SQLite read when the journal state is queryable, and the debug-visible policy helper covers the exact allow/block matrix used by that guard.
- `Common/LocalSearchIndexCore.cpp` now seeds brand-new SQLite-backed roots directly into SQLite and skips the compatibility `.bin` snapshot write when the initial mirror succeeds, while still falling back to snapshot persistence if that first mirror fails.
- `Common/LocalSearchIndexCore.cpp` now treats a missing or unreadable live journal cursor as a hard block for cold/startup direct SQLite reads, forcing refresh before the query answers instead of preserving the old best-effort direct-read behavior.
- `Common/LocalSearchIndexCore.cpp` now keys "initial/direct SQLite seed" eligibility off actual compatibility snapshot-file presence instead of `volume.initialized`, so no-journal warm refreshes in long-lived service processes do not backslide into `.bin` creation when no compatibility snapshot exists.
- `Common/LocalSearchIndexCore.cpp` now clears `QueryStats.snapshotPath` whenever SQLite answers without a real compatibility snapshot artifact, so stale-root refresh/direct-query stats reflect the actual runtime store state instead of the configured fallback path.
- `Common/LocalSearchIndexCore.cpp` now enumerates fixed local NTFS/ReFS roots and primes them into the repository cache, while `Common/SearchServiceBroker.cpp` publishes that discovered root set through service status before the first query.
- `Common/SearchServiceBroker.cpp` now resolves startup discovery roots once, primes them into the repository cache, and background-warms those roots through `Repository::EnsureReadyForRoot(...)` when the service owns the default SQLite store; the same code path can be driven on isolated temp roots through `REDSALAMANDER_SEARCH_SERVICE_DISCOVER_ROOTS`.
- `Common/SearchServiceBroker.h` and `Common/SearchServiceBroker.cpp` now serialize startup warmup enable/running state, total/completed/failed root counts, and the current warmup root through `GetStatus(...)`, while `RedSalamanderSearchService/Main.cpp` surfaces the same state in the foreground dashboard/log stream.
- `Common/LocalSearchIndexCore.cpp` keeps NTFS cold seed/replay on `FSCTL_ENUM_USN_DATA` and `FSCTL_READ_USN_JOURNAL`, falls back to traversal when journal/MFT access is unavailable, and routes ReFS roots through traversal seeding; the focused core regressions already validate the NTFS cold-seed, replay, and ReFS paths.
- `Common/LocalSearchIndexCore.cpp` keeps crawl/journal mutation production in the repository core, while `Common/SqliteIndexStore.cpp` owns store inspection, volume replacement, maintenance, and direct query execution; `Common/LocalSearchIndexCore.cpp` bridges the two through the SQLite mirror/query helpers instead of mixing SQL execution into the crawler.
- `Common/SqliteIndexStore.cpp` enumerates candidates through `SQLITE_OPEN_READONLY`, `Common/LocalSearchIndexCore.h` now surfaces that as `sqliteReadOnlyQuery`, and the service single-instance guard plus read-only query telemetry now make the service the only writer in the production SQLite path while direct query paths stay read-only.
- `Common/SearchServiceBroker.h`, `Common/SearchServiceBroker.cpp`, and `RedSalamanderSearchService/Main.cpp` now surface the last startup warmup failure root/HRESULT through status, server events, the foreground dashboard, and redirected foreground logs.
- `RedSalamander/CompareDirectoriesEngine.SelfTest.cpp` now waits for the next named-pipe instance after the foreground readiness status probe and captures redirected foreground output through a temp file, so service selftests no longer race the server's per-request pipe recreation or lose redirected startup/log text on console-subsystem builds.
- `RedSalamander/CompareDirectoriesEngine.SelfTest.cpp` now includes a dedicated warm indexed-name validation case that measures first candidate-batch latency on the broker callback path, requires it to stay below `100 ms`, and cross-checks service, local-index, and fallback result parity for the same wildcard name query.
- `RedSalamander/CompareDirectoriesEngine.SelfTest.cpp` now verifies startup warmup status while warmup is still running, then again after completion, and asserts the foreground redirected output includes warmup progress/completion text.
- `RedSalamander/CompareDirectoriesEngine.SelfTest.cpp` now verifies startup warmup failure status and foreground output, including the last failing root/HRESULT, and also asserts direct SQLite query telemetry reports a read-only connection when cutover is open.
- 2026-03-14 addendum:
  - `Common/LocalSearchIndexCore.cpp` now keeps Debug and Release service stores separated by default (`%ProgramData%\\RedSalamander\\SearchIndex.Debug` vs `%ProgramData%\\RedSalamander\\SearchIndex`) and keeps pre-query broker status non-blocking by deriving DB state from persistent store inspection before runtime repository state exists.
  - `Common/LocalSearchIndexCore.cpp` and `Common/SqliteIndexStore.cpp` now restart degraded queries on the live filesystem path without duplicating already emitted SQLite hits when a mid-query SQLite failure happens after the first streamed result.
  - Search path and name folding now uses invariant Windows case mapping instead of locale-sensitive `CharLowerBuffW`, and the Unicode parity regression now pins case-insensitive accented matches while keeping composed and decomposed names distinct when the underlying filesystem does the same.
  - `RedSalamanderSearchService/Main.cpp` redirected foreground logs now carry the same DB/sync/query-execution diagnostics surface as the interactive dashboard.
  - `RedSalamander/CompareDirectoriesEngine.SelfTest.cpp` gained `search_service_sqlite_invalid_store_falls_back_live_scan`, `search_service_sqlite_query_failure_falls_back_live_scan`, `search_service_sqlite_midquery_failure_restarts_live_scan_without_duplicates`, `search_service_sqlite_default_store_uses_build_specific_programdata_root`, `search_service_sqlite_seeded_default_store_reuses_build_specific_programdata_root`, and the redirected-log/status regressions that pin those behaviors.
  - Implementation review found one last evidence gap after the merge: helper-level default-identity assertions were not enough to prove split-store compatibility. The plan now requires on-disk startup regressions that verify the active build both creates only its own default SQLite root under `ProgramData` and reuses a pre-seeded default store in place without creating the sibling build root.
- Focused compare self-test evidence on 2026-03-13:
  - Passed:
  - `local_index_core_probe_and_query_if_available`
  - `local_index_core_journal_replay_rename_delete_create`
  - `local_index_core_refs_probe_and_query_if_available`
  - `local_index_core_sqlite_direct_query_freshness_policy`
  - `local_index_core_sqlite_prefilter_classifies_name_patterns`
  - `sqlite_index_store_upgrade_paths`
  - `local_index_core_sqlite_option_keeps_snapshot_runtime_store`
  - `local_index_core_sqlite_cold_start_stale_root_refreshes_before_query`
  - `local_search_service_indexed_name_latency_and_parity`
  - `search_service_sqlite_bootstrap_status_roundtrip`
  - `search_service_sqlite_status_reports_pending_legacy_import`
  - `search_service_discovers_fixed_local_roots_on_start`
  - `search_service_sqlite_startup_warms_overridden_roots`
  - `search_service_sqlite_default_store_uses_build_specific_programdata_root`
  - `search_service_sqlite_startup_warmup_failure_status_roundtrip`
  - `search_service_sqlite_cold_start_stale_root_refreshes_before_query`
  - `search_service_foreground_logs_request_status`
  - `search_service_default_identity_matches_build`
  - `search_service_status_and_query_roundtrip`
  - `local_search_service_matches_host_fallback`
  - Skipped by policy on this machine:
  - `local_index_core_sqlite_cold_start_bypasses_snapshot_runtime_store`
  - `search_service_sqlite_cold_start_bypasses_snapshot_runtime_store`
- Cold-start direct-bypass regressions now self-skip only when the live journal is unavailable and the conservative policy blocks a direct-read validation run; the stale-root refresh regressions continue to validate refresh behavior without relying on compatibility snapshot files.

### Phase 10 - Maintenance, compaction, and operational hardening

- [x] Implement automatic WAL checkpoint scheduling:
  - when WAL grows past `128 MiB`
  - when the service becomes idle and a queued maintenance window expires
  - current implementation is threshold/grace driven in the foreground service loop; the explicit replay-burst prioritization remains covered by the same idle scheduler, not a separate trigger path
- [x] Implement bounded incremental compaction scheduling:
  - when freelist/page fragmentation reaches `>=15%`
  - or reclaimable space reaches `>=64 MiB`
  - only while the service is idle or low-activity
- [x] Bound each compaction pass so maintenance never blocks normal queries for more than the agreed latency budget.
- [x] Add a manual/admin compaction control path:
  - [x] CLI `--compact` runs offline WAL checkpoint + `VACUUM` on the selected SQLite store and records maintenance timestamps.
  - [x] service control request
- [x] Surface maintenance state through status and TUI:
  - [x] foreground TUI surfaces backend, primary DB size, WAL size, freelist/page counts, last checkpoint, and last compaction from SQLite inspection.
  - [x] broker status serialization surfaces page/freelist counts plus last checkpoint / last compaction.
  - [x] broker status serialization surfaces queued/running maintenance.
- [x] Validation: direct/manual compaction shrinks freelist pages, truncates WAL, and records maintenance timestamps.
- [x] Validation: SQLite service status round-trips maintenance history and page/freelist counters after manual compaction.
- [x] Validation: automatic SQLite checkpoint-only maintenance can truncate WAL without forcing a full vacuum.
- [x] Validation: automatic SQLite compaction runs in a bounded incremental-vacuum pass and reduces freelist pages.
- [x] Validation: live foreground service status reports queued idle maintenance before the grace window expires, then clears the queue after maintenance completes.
- [x] Validation: large delete bursts shrink WAL and reclaim database space over successive idle maintenance windows without breaking query parity.

#### Phase 10 code/test surfaces

- Code: SQLite automatic maintenance runner, idle maintenance scheduler in the service loop, broker status serialization, service CLI/TUI, and admin control handling.
- Tests: checkpoint trigger, bounded incremental compaction trigger, manual compaction, live control requests, maintenance queue/completion reporting, maintenance status reporting, and multi-window delete-burst parity.

#### Phase 10 exit evidence

- `Common/SqliteIndexStore.h` and `Common/SqliteIndexStore.cpp` now expose offline manual maintenance plus inspection metadata for page count, freelist pages, and last checkpoint/compaction timestamps.
- `Common/SqliteIndexStore.h` and `Common/SqliteIndexStore.cpp` now expose automatic threshold-based maintenance with bounded incremental vacuum passes in addition to offline manual maintenance.
- `RedSalamanderSearchService/Main.cpp` now supports `--compact`, reuses the single-instance guard, and reports before/after DB/WAL/free-page counts in terminal output.
- `Common/SearchServiceBroker.h` and `Common/SearchServiceBroker.cpp` now expose a live `RequestCompact()` control request, and `RedSalamanderSearchService/Main.cpp` now surfaces it as `--request-compact`.
- The foreground banner/dashboard now display SQLite maintenance history, queue/running state, and space state sourced from `LocalSearchIndexCore::GetPersistentStoreInfo(...)` plus live server events.
- `Common/SearchServiceBroker.h` and `Common/SearchServiceBroker.cpp` now serialize SQLite page/freelist counts, last checkpoint/compaction timestamps, and queued/running maintenance flags through `GetStatus(...)`.
- The maintenance helper/test path now supports oversized delete-burst fixtures so the service can prove repeated idle-window incremental vacuum passes while query results stay stable.
- Focused compare self-tests passed on 2026-03-13:
  - `sqlite_index_store_manual_compaction_reclaims_space`
  - `sqlite_index_store_automatic_checkpoint_truncates_wal`
  - `sqlite_index_store_automatic_compaction_is_bounded`
  - `search_service_compact_cli_runs_manual_sqlite_maintenance`
  - `search_service_compact_request_roundtrip`
  ...
  - `search_service_help_lists_cli_options`
  - `search_service_foreground_logs_request_status`

## Persistent Backend Recommendation

Chosen backend for the always-on service index:
- `sqlite3` from vcpkg

Rejected as the primary v1 service store:
- DuckDB

Reason:
- The RedSalamander service workload is mutation-heavy and journal-driven: many small upserts, deletes, renames, checkpoints, and read-mostly queries from one long-lived Windows service process.
- SQLite matches that shape better than DuckDB for a primary metadata store, while DuckDB remains viable later for analytics/export or offline diagnostics.

## Current-Machine Persistent Store Size Estimate

Observed data on this machine:
- Service-owned `C:` snapshot in `%ProgramData%\RedSalamander\SearchIndex`: `144,537,810` bytes for `1,614,016` entries (`~89.6 B/entry`)
- User-owned `D:` snapshot in `%LocalAppData%\RedSalamander\SearchIndex`: `2,585,216` bytes for `40,385` entries (`~64.0 B/entry`)
- Working planning density for NTFS volumes: `~64-90 B/entry` (`~6.1-8.5 MiB / 100k entries`)

Per-volume planning estimate:

| Drive | FS | Used | Basis | Estimated steady-state index |
|---|---|---:|---|---:|
| `C:` | NTFS | `392.1 GiB` | observed service snapshot | `137.8 MiB observed` |
| `D:` | NTFS | `838.4 GiB` | observed local snapshot | `2.5 MiB observed` |
| `E:` | NTFS | `3679.5 GiB` | NTFS MFT proxy | `85-119 MiB` |
| `S:` | NTFS | `3661.7 GiB` | NTFS MFT proxy | `103-144 MiB` |
| `V:` | NTFS | `3336.8 GiB` | NTFS MFT proxy | `12-17 MiB` |
| `Z:` | ReFS | `1134.2 GiB` | wider estimate; no full-root snapshot yet | `80-300 MiB` |

Planning total for all fixed local volumes on this machine:
- expected steady-state store footprint: `~420-720 MiB`
- recommended free-space budget for DB, WAL, rebuild, and import headroom: `1.0-1.5 GiB`
