...
The broader manual regex audit archived with the final 2026-04-23 evidence still lists expected allowlist/exception paths such as `Common/DxUi/DxUi.Typography.h`, the hidden text-input bridge diagnostic, Fluent icon font creation, and the NavigationView font-related perf scope. The strict menu/HFONT measurement search in the same archive is clean.
...
## Hidden Bridge Status

The hidden DxUi text-input bridge now uses the shared Segoe UI Variable contract through a custom non-visual `DxUiTextInputBridgeWindow` service window in `Common/DxUi/DxUi.TextInput.cpp`; it no longer depends on a hidden `EDIT` / RichEdit control. NavigationView edit mode now runs through a visible DxUi-backed text host instead of a native visible Win32 edit surface.
...
- `.\.build\x64\Debug\DxUiTests.exe --suite=TextInputBridge`
- archived focused bridge replacement rerun: `Specs/TestRuns/4cb089111a23/DxUi/2026-04-22_182650_textinputbridge_custom_service_green/`
