# Preferences Dialog Migration History (RedSalamander.exe)

This document is historical migration/planning context for Preferences.

The authoritative product contract now lives in `Specs/UI/UI_PreferencesDialog.md`.
Shared `DxUi` control, accessibility, visible-native-retirement, and migrated-window acceptance rules live in `Specs/UI/UI_DxUiSharedGrid.md`.
...
- Modeless, independent top-level dialog: keep the main app usable while Preferences is open without forcing Preferences to stay above the main window.
...
- Host: `IDD_PREFERENCES` is modeless, independent, resizable, and uses OK/Cancel/Apply semantics.
- Navigation: shared `DxUi::Tree` host with Plugins child nodes; selecting a plugin opens its settings subpage.
- Scroll host: `RedSalamanderPrefsPageHost` provides vertical scrolling, wheel routing, and reduced resize artifacts; its visible background/card surface is now rendered through a shared `DxUi::WindowHost`, and migrated page content now re-lands incrementally on one-host DX page surfaces while the remaining pages stay on the proven legacy path until their parity slices are ready.
...
Status:
- Addressed by the current implementation (active combo/input path plus the subsequent input-layout and dropdown hot tracking/commit fixes), plus:
...
- Earlier builds vertically centered some multiline-backed single-line edits through a retired `ThemedControls` helper. That helper is no longer the cross-dialog contract to preserve.
- The active themed combo/input paths are:
  - `DxUi::ComboBox` / `DxUi::TextField` for migrated controls.
  - `PrefsInput::CreateFramedEditBox(...)` / `PrefsInput::CreateFramedComboBox(...)` plus `ThemedInputFrames` for remaining Win32 controls.
  - High-contrast mode keeps system-controlled native borders/colors.
- Focus cues:
...
- Confirm the control is on the active `DxUi` path or the `PrefsInput`/`ThemedInputFrames` framed legacy path.
- Verify dropdown close/routing in the active combo implementation rather than the retired `ThemedControls` files.
...
- Tree-driven pane switches stay atomic: selecting a category or plugin child in the navigation tree must not show a new shell title/description over the old page body, and only one pane host may remain visible after the switch settles.
- Tree-driven pane switches must also force the active pane subtree to repaint before the batch completes, so returning to a one-host page does not flash a default white child background or leave nested DX toggles/buttons/inputs absent until hover or a later resize.
- Tree-driven pane switches between different page families (for example grid pages, note-only pages, and toggle-card pages) must also replace the active page-local UI Automation subtree itself; after switch settle, the old page’s edit/combo/toggle descendants must not remain visible under the new page.
- Tree-driven `Plugins` root/details/root navigation must also replace the page-local UI Automation subtree itself; returning from a plugin child to the Plugins root must clear details-mode edit/combo/value descendants and restore the list-mode subtree.
- Tree-driven category and Plugins-root navigation must also restore the exact shell description text for the active destination page; title-only updates are not acceptable if the description still belongs to the previous pane.
- Mixed-input one-host pages such as `Compare Directories` must survive a real category round-trip: after leaving and returning, the page must repaint, restore its section statics plus toggle/edit/combo UIA subtree, and keep the legacy compare body controls hidden.
- Mixed rich one-host pages such as `Themes` must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its input/button/grid/swatch UIA subtree, and keep the legacy themes body controls hidden.
- Mixed toggle/edit/combo one-host pages such as `Advanced` must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its page-local UIA subtree, and keep the legacy advanced body controls hidden.
- Mixed toggle/input/button one-host pages such as `Hot Paths` must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its page-local UIA subtree, and keep the legacy hot-path body controls hidden.
- Mixed toggle/edit/combo one-host pages such as `Panes` must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its page-local UIA subtree, and keep the legacy panes body controls hidden.
- Note-only one-host pages such as `Editors` and `Mouse` must also survive real category round-trips: after leaving and returning, the page must repaint, restore the note-only UIA subtree, and keep stale edit/combo/toggle descendants absent.
- Mixed edit/combo/grid one-host pages such as `Viewers` must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its page-local UIA subtree, and keep the legacy viewers body controls hidden.
- Mixed edit/combo/grid/button one-host pages such as `Keyboard` must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its page-local UIA subtree, and keep the legacy keyboard body controls hidden.
- Mixed search/list/button one-host pages such as `Plugins` list mode must also survive a real category round-trip: after leaving and returning, the page must repaint, restore its page-local UIA subtree, and keep the legacy plugins list-mode controls hidden.
...
- Windowing behavior: Preferences is modeless and uses the independent top-level tool-window contract from `Specs/UI/UI_TopLevelToolWindows.md`.
...
- Independent top-level window: clicking the main app and Preferences must freely switch focus between them.
- Layout:
  - Left: shared DX navigation host for categories + plugin subpages.
...
Implementation note:
- The control ID remains `IDC_PREFS_CATEGORY_LIST` for compatibility, but the dialog resource now uses a plain host surface attached to the shared `DxUi::Tree` path rather than a visible `SysTreeView32`.
...
- Retired shared `ThemedControls` drawing utilities are no longer part of the active contract; shared Preferences plumbing now lives in `PrefsUi`, `PrefsInput`, `PrefsPaneHost`, `ThemedInputFrames`, and migrated `DxUi` hosts.
...
- Pane modules now exist for all categories (some are still placeholders with note-only UI):
  ...
  - The visible page-host background/card surface is now rendered through a shared `DxUi::WindowHost`; pane root windows still paint matching background/cards in `WM_PAINT` / `WM_PRINTCLIENT` to satisfy standard controls that request parent background.
...
- Page-host paint: the visible background/card surface now uses a shared `DxUi::WindowHost` on `IDC_PREFS_PAGE_HOST`, while scroll/resize invalidation remains host-owned so uncovered regions and child invalidation still avoid stale pixels and scroll artifacts.
...
- ListView theming is owned by the active per-pane list/header handling and shared `PrefsUi` helpers, not the retired `ThemedControls` helpers.
...
- Boolean toggles in Preferences use the current DxUi/per-pane Settings-style switch path: state text + switch, accent track for the “On” state, knob, and low-contrast focus.
...
- ListView header styling is handled by active list/header code in the pane modules and shared Preferences helpers.
...
- Keyboard page DX rendering and shortcuts-grid behavior are implemented in `RedSalamander/Preferences.Keyboard.cpp`; the host now delegates the visible page surface to the pane-owned DX host while preserving the existing backing state and capture-mode contract.
...
- Viewers page DX rendering and mappings-grid behavior are implemented in `RedSalamander/Preferences.Viewers.cpp`; the live page now owns its visible inputs, mappings grid, hint text, and command buttons on a pane-owned DX host.
...
- Themes page DX rendering, colors-grid behavior, and swatch preview are implemented in `RedSalamander/Preferences.Themes.cpp`; the live page now owns its visible labels, inputs, colors grid, buttons, and swatch preview on a pane-owned DX host.
...
- Combobox polish:
  - Implemented: active combo/input paths apply `DarkMode_Explorer` / `Explorer` theming where native controls remain, while migrated controls use DxUi theming directly.
...
### Future improvements
The RedSalamander preferences system is a sophisticated Windows dialog with many category panes, JSON-backed storage, and modern card-based UI. The architecture is modular but suffers from a monolithic dialog file and repetitive control creation patterns. Key improvements focus on reducing code duplication and enhancing UX.
