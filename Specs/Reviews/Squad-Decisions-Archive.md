...
> [!IMPORTANT]
> **Historical snapshot — not a live work queue.** This is the preserved decision log of the retired
> `.squad/` multi-agent framework (removed 2026-08-26). Entries are date-bounded records; several are
> explicitly superseded inline. Folder routing was reconciled on 2026-07-17 at repository commit
> `f4e0c8c3bed8`; the completed disposition ledger is
> `Specs/Plans/Done/Operation_Observatory_WholeRepositoryCodeAuditAndRemediationPlan_2026-07-15.md`,
> while live work is indexed by `Specs/Plans/WIP/README.md`. Durable behavior described here is
> authoritative only where it has been merged into the owning `Specs/<Domain>/` specification.

## Active Decisions

### [SUPERSEDED 2026-05-01] ASan Triplet: Release-Only Build Strategy (2026-04-28)

**Agent:** Yoko (Core Dev) | **Status:** Superseded — see "ASan Triplet Removal" below

**Original context:** ASan triplets (`x64-windows-asan`, `arm64-windows-asan`) used `VCPKG_BUILD_TYPE release` to build only release binaries, avoiding OpenSSL LNK1158 linker recursion when both `-INCREMENTAL` and `/INCREMENTAL:NO` appeared on the link line.

**Original decision:** Accept release-only ASan triplet. "ASan Debug" configuration meant first-party code compiled in debug mode linking against ASan-instrumented release dependencies.

**Why superseded:** The whole ASan-instrumented vcpkg triplet machinery (overlay triplets, OpenSSL port patch, `VcpkgConfiguration=Release` override, `RSVcpkgBinSubdir` helper) was removed on 2026-05-01. ASan Debug now links against the standard `x64-windows` / `arm64-windows` triplets. MSVC STL container-annotation mismatches (LNK2038 on `annotate_vector` / `annotate_string` / `annotate_optional`) are suppressed via `_DISABLE_STL_ANNOTATION` in the ASan Debug configuration. Trade-off: ASan no longer detects STL container overflow, but heap/stack/global/use-after-free coverage is unchanged. Removed: `vcpkg-overlay-triplets/`, `vcpkg-overlay-ports/`, `vcpkg-configuration.json`, `-Asan` switch in `vcpkg-install.ps1`.
