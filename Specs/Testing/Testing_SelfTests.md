...
- `Tools/TestInventory.ps1` is the source-derived manifest for static Commands
  registrations, Compare registrations, FileOperations active phases, and each
  Tools Pester file. `Tests/README.md` overview, headings, narratives, and family
  tables must be linted against that manifest (or against each other for
  runner-generated listed-case totals); duplicated inventory numbers must not be
  updated independently without a drift guard.
