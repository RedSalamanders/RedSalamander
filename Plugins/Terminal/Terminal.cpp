...
HRESULT STDMETHODCALLTYPE Terminal::SetConfiguration(const char* configurationJsonUtf8) noexcept
{
    ...
    if (configurationJsonUtf8 != nullptr && configurationJsonUtf8[0] != '\0')
    {
        ...
        const auto readEnumString =
            [root](const char* key, std::initializer_list<std::wstring_view> allowed, std::wstring& target) noexcept -> HRESULT
        {
        ...
        {
            const Common::Json::MemberResult<int64_t> member =
                Common::Json::GetInt64Member(root, key, Common::Json::MemberRequirement::Optional);
            if (member.status == Common::Json::MemberStatus::MissingOptional)
            {
                return S_FALSE;
            }
        ...
        const Common::Json::MemberResult<double> fontSize =
            Common::Json::GetDoubleMember(root, "fontSizeDip", Common::Json::MemberRequirement::Optional);
    ...
    {
        RedSalamander::DxUi::Typography::InvalidateFontFamilyAvailability(_dwriteFactory.get());
