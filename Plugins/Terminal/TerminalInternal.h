...
[[nodiscard]] inline SCROLLINFO MakeTerminalScrollInfo(const GhosttyTerminalScrollbar& scrollbar) noexcept
{
    ...
    const int nativeMaximum       = static_cast<int>(std::min<uint64_t>(total - 1u, static_cast<uint64_t>((std::numeric_limits<int>::max)())));
    const UINT nativePage         = static_cast<UINT>(std::min<uint64_t>(visible, static_cast<uint64_t>(nativeMaximum) + 1u));
    ...
    info.nPos   = static_cast<int>(std::min<uint64_t>(std::min(scrollbar.offset, maximumOffset), static_cast<uint64_t>(nativeMaximumOffset)));
    return info;
}
...
[[nodiscard]] constexpr TerminalWheelScrollResult ResolveTerminalWheelScroll(int previousRemainder,
                                                                             int wheelDelta,
                                                                             UINT linesPerDetent,
                                                                             uint16_t viewportRows) noexcept
{
    ...
    const uint64_t rowsPerDetent =
        linesPerDetent == WHEEL_PAGESCROLL ? static_cast<uint64_t>(std::max<uint16_t>(viewportRows, 1u)) : static_cast<uint64_t>(linesPerDetent);
    ...
    return {
        .rows      = static_cast<intptr_t>(std::clamp<int64_t>(
            rowDelta, static_cast<int64_t>((std::numeric_limits<intptr_t>::min)()), static_cast<int64_t>((std::numeric_limits<intptr_t>::max)()))),
        .remainder = remainder,
    };
}
...
{
    encoded.clear();
    if (pipeName.empty() || pipeName.find_first_not_of(L"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.-") != std::wstring_view::npos)
    {
        return E_INVALIDARG;
    }
    ...
    if (CryptBinaryToStringW(reinterpret_cast<const BYTE*>(script.data()), byteCount, CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, nullptr, &characterCount) ==
            FALSE ||
        characterCount == 0u)
    {
    ...
    if (CryptBinaryToStringW(
            reinterpret_cast<const BYTE*>(script.data()), byteCount, CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, encoded.data(), &writableCount) == FALSE)
    {
        encoded.clear();
        return HRESULT_FROM_WIN32(GetLastError());
    }
...
inline HRESULT WaitForOverlappedIo(HANDLE pipe, OVERLAPPED& overlapped, std::stop_token stopToken, DWORD& bytes) noexcept
{
    for (;;)
    {
        ...
        {
            return GetOverlappedResult(pipe, &overlapped, &bytes, FALSE) != FALSE ? S_OK : HRESULT_FROM_WIN32(GetLastError());
        }
        if (wait == WAIT_FAILED)
        {
            return HRESULT_FROM_WIN32(GetLastError());
...
inline HRESULT DecodeIntegrationField(std::string_view encoded, std::wstring& value) noexcept
{
    ...
    DWORD byteCount = 0u;
    if (CryptStringToBinaryA(encoded.data(), static_cast<DWORD>(encoded.size()), CRYPT_STRING_BASE64, nullptr, &byteCount, nullptr, nullptr) == FALSE ||
    ...
    if (CryptStringToBinaryA(
            encoded.data(), static_cast<DWORD>(encoded.size()), CRYPT_STRING_BASE64, reinterpret_cast<BYTE*>(utf8.data()), &written, nullptr, nullptr) == FALSE)
    {
        return E_INVALIDARG;
    }
    utf8.resize(written);
...
inline HRESULT WriteIntegrationReply(
    HANDLE pipe, std::stop_token stopToken, bool versioned, std::wstring_view target, std::wstring_view promptReplacement) noexcept
{
    std::string utf8;
    if (versioned)
    {
...
inline HRESULT ParseIntegrationRequest(std::string_view record, IntegrationRequest& request) noexcept
{
    ...
    const auto scrubPackedHistory =
        wil::scope_exit([&packedHistory]() noexcept { SecureZeroMemory(packedHistory.data(), packedHistory.size() * sizeof(wchar_t)); });
...
{
    return ! value.empty() && value.size() <= 256u && value.front() != L'-' && value.find_first_of(L"\\/") == std::wstring_view::npos &&
           ! ContainsCommandControl(value);
}
...
{
    if (location.sizeBytes < sizeof(TerminalLogicalLocation) || ! IsSpanWellFormed(location.windowsPath) || ! IsSpanWellFormed(location.wslDistribution) ||
        ! IsSpanWellFormed(location.wslAbsolutePath) || ! IsSpanWellFormed(location.pluginShortId) || ! IsSpanWellFormed(location.pluginBackingWindowsPath))
    {
        return false;
    }
    ...
    switch (location.kind)
    {
        case TerminalLocationKind::WindowsLocal:
        {
            ...
            return noWsl && noPlugin && ! ContainsCommandControl(windowsPath) &&
                   (pathClass == Common::Paths::WindowsPathClass::DriveAbsolute || pathClass == Common::Paths::WindowsPathClass::ExtendedDriveAbsolute);
        }
        case TerminalLocationKind::WindowsUnc:
        {
            const Common::Paths::WindowsPathClass pathClass = Common::Paths::ClassifyWindowsPath(windowsPath);
            return noWsl && noPlugin && ! ContainsCommandControl(windowsPath) &&
                   (pathClass == Common::Paths::WindowsPathClass::Unc || pathClass == Common::Paths::WindowsPathClass::ExtendedUnc);
        }
        case TerminalLocationKind::Wsl: return noWindows && noPlugin && IsValidWslDistribution(distribution) && IsSupportedAbsoluteWslPath(wslPath);
        case TerminalLocationKind::PluginBacked:
            return noWindows && noWsl && ! pluginShortId.empty() && pluginShortId.size() <= 256u && ! ContainsCommandControl(pluginShortId) &&
                   (pluginBackingPath.empty() || IsSupportedAbsoluteWindowsPath(pluginBackingPath));
        case TerminalLocationKind::Unsupported: return noWindows && noWsl && noPlugin;
                                                            ...
                                                            std::wstring_view trustedCurrentDirectory)
{
    ...
    if (insertion.mode == TerminalPathInsertionMode::ContextualLeafOrFull && integrationTrusted && idleAtPrimaryPrompt && ! hasPendingUserInput)
    {
...
[[nodiscard]] inline bool AddLocalizedSchemaString(yyjson_mut_doc* document, yyjson_mut_val* object, const char* key, UINT resourceId) noexcept
{
    ...
    return utf8.has_value() && ! utf8->empty() && yyjson_mut_obj_add_strncpy(document, object, key, utf8->data(), utf8->size());
}

[[nodiscard]] inline yyjson_mut_val* AddSchemaField(
    yyjson_mut_doc* document, yyjson_mut_val* fields, const char* key, const char* type, UINT labelResourceId, UINT descriptionResourceId) noexcept
{
    yyjson_mut_val* field = yyjson_mut_obj(document);
    if (field == nullptr || ! yyjson_mut_arr_add_val(fields, field) || ! yyjson_mut_obj_add_strcpy(document, field, "key", key) ||
        ! yyjson_mut_obj_add_strcpy(document, field, "type", type) || ! AddLocalizedSchemaString(document, field, "label", labelResourceId) ||
...
[[nodiscard]] inline bool AddSchemaOption(yyjson_mut_doc* document, yyjson_mut_val* options, const char* value, UINT labelResourceId) noexcept
{
    yyjson_mut_val* option = yyjson_mut_obj(document);
    return option != nullptr && yyjson_mut_arr_add_val(options, option) && yyjson_mut_obj_add_strcpy(document, option, "value", value) &&
...
[[nodiscard]] inline std::string BuildTerminalConfigurationSchema() noexcept
{
    ...
    if (! yyjson_mut_obj_add_uint(document, root, "version", 1u) || ! AddLocalizedSchemaString(document, root, "title", IDS_TERMINAL_SCHEMA_TITLE) ||
        ! yyjson_mut_obj_add_val(document, root, "fields", fields))
    {
        return {};
    }

    yyjson_mut_val* field =
        AddSchemaField(document, fields, "defaultShell", "option", IDS_TERMINAL_SCHEMA_DEFAULT_SHELL_LABEL, IDS_TERMINAL_SCHEMA_DEFAULT_SHELL_DESCRIPTION);
    ...
    field = AddSchemaField(document, fields, "fontFamily", "text", IDS_TERMINAL_SCHEMA_FONT_FAMILY_LABEL, IDS_TERMINAL_SCHEMA_FONT_FAMILY_DESCRIPTION);
    if (field == nullptr || ! yyjson_mut_obj_add_strcpy(document, field, "default", "Cascadia Mono"))
    {
        return {};
    }

    const auto addValueField =
        [document,
         fields](const char* key, UINT labelResourceId, UINT descriptionResourceId, uint64_t defaultValue, uint64_t minimum, uint64_t maximum) noexcept
    {
        yyjson_mut_val* valueField = AddSchemaField(document, fields, key, "value", labelResourceId, descriptionResourceId);
        return valueField != nullptr && yyjson_mut_obj_add_uint(document, valueField, "default", defaultValue) &&
               yyjson_mut_obj_add_uint(document, valueField, "min", minimum) && yyjson_mut_obj_add_uint(document, valueField, "max", maximum);
    };
    ...
    const auto addBooleanOptionField = [document, fields](const char* key, UINT labelResourceId, UINT descriptionResourceId, const char* defaultValue) noexcept
    {
        yyjson_mut_val* optionField  = AddSchemaField(document, fields, key, "option", labelResourceId, descriptionResourceId);
        yyjson_mut_val* optionValues = optionField == nullptr ? nullptr : AddSchemaOptions(document, optionField);
        return optionField != nullptr && optionValues != nullptr && yyjson_mut_obj_add_strcpy(document, optionField, "default", defaultValue) &&
    ...
    if (! addBooleanOptionField("warnOnUnsafePaste", IDS_TERMINAL_SCHEMA_WARN_PASTE_LABEL, IDS_TERMINAL_SCHEMA_WARN_PASTE_DESCRIPTION, "1") ||
        ! addBooleanOptionField("followPathWhenIdle", IDS_TERMINAL_SCHEMA_FOLLOW_PATH_LABEL, IDS_TERMINAL_SCHEMA_FOLLOW_PATH_DESCRIPTION, "0"))
    {
        return {};
    }

    field   = AddSchemaField(document, fields, "hyperlinkPolicy", "option", IDS_TERMINAL_SCHEMA_HYPERLINK_LABEL, IDS_TERMINAL_SCHEMA_HYPERLINK_DESCRIPTION);
    ...
    field   = AddSchemaField(document, fields, "osc52Policy", "option", IDS_TERMINAL_SCHEMA_OSC52_LABEL, IDS_TERMINAL_SCHEMA_OSC52_DESCRIPTION);
    options = field == nullptr ? nullptr : AddSchemaOptions(document, field);
    if (field == nullptr || options == nullptr || ! yyjson_mut_obj_add_strcpy(document, field, "default", "ask") ||
        ...
        ! addValueField(
            "osc52MaxBytes", IDS_TERMINAL_SCHEMA_OSC52_BYTES_LABEL, IDS_TERMINAL_SCHEMA_OSC52_BYTES_DESCRIPTION, 262144u, 4096u, kMaximumOsc52Bytes))
    {
        return {};
    }
...
[[nodiscard]] inline std::wstring ReadPowerShellAppPath(HKEY root) noexcept
{
    ...
    DWORD bytes                      = 0u;
    if (RegGetValueW(root, kPwshAppPath, nullptr, RRF_RT_REG_SZ | RRF_NOEXPAND, nullptr, nullptr, &bytes) != ERROR_SUCCESS || bytes < sizeof(wchar_t) ||
...
[[nodiscard]] inline std::optional<std::wstring> ReadImeString(HWND window, LPARAM value) noexcept
{
    ...
    const auto releaseContext = wil::scope_exit([window, inputContext]() noexcept { static_cast<void>(ImmReleaseContext(window, inputContext)); });
...
{
    return (static_cast<uint32_t>(alpha) << 24u) | (static_cast<uint32_t>(color.r) << 16u) | (static_cast<uint32_t>(color.g) << 8u) |
...
[[nodiscard]] inline GhosttyKey GhosttyKeyFromVirtualKey(WPARAM virtualKey) noexcept
{
    switch (virtualKey)
    {
        ...
        default:
            ...
            {
                return static_cast<GhosttyKey>(static_cast<int>(GHOSTTY_KEY_F1) + static_cast<int>(virtualKey - VK_F1));
            }
...
[[nodiscard]] constexpr bool VirtualKeyProducesTranslatedCharacter(WPARAM virtualKey) noexcept
{
    if ((virtualKey >= static_cast<WPARAM>('0') && virtualKey <= static_cast<WPARAM>('9')) ||
        (virtualKey >= static_cast<WPARAM>('A') && virtualKey <= static_cast<WPARAM>('Z')) || (virtualKey >= VK_NUMPAD0 && virtualKey <= VK_DIVIDE) ||
        (virtualKey >= VK_OEM_1 && virtualKey <= VK_OEM_3) || (virtualKey >= VK_OEM_4 && virtualKey <= VK_OEM_8))
    {
        return true;
    }
...
[[nodiscard]] constexpr bool GhosttySizedFieldPresent(size_t recordSize, size_t fieldOffset, size_t fieldSize) noexcept
{
    return fieldOffset <= recordSize && fieldSize <= recordSize - fieldOffset;
}

[[nodiscard]] inline GhosttyClipboardWriteResult ValidateGhosttyClipboardWrite(const GhosttyClipboardWrite* write,
                                                                               size_t maximumBytes,
                                                                               GhosttyClipboardDataView& selectedView) noexcept
{
    selectedView                 = {};
    constexpr size_t minimumSize = offsetof(GhosttyClipboardWrite, contents_len) + sizeof(size_t);
    if (write == nullptr || write->size < minimumSize || write->location != GHOSTTY_CLIPBOARD_LOCATION_STANDARD ||
        write->contents_len > kMaximumClipboardRepresentations || (write->contents_len != 0u && write->contents == nullptr))
    {
    ...
    for (size_t index = 0u; index < write->contents_len; ++index)
    {
        ...
        if ((candidate.mime.ptr == nullptr && candidate.mime.len != 0u) || (candidate.data.ptr == nullptr && candidate.data.len != 0u))
        {
            return GHOSTTY_CLIPBOARD_WRITE_RESULT_INVALID_DATA;
        }
        const std::string_view mime(candidate.mime.ptr != nullptr ? reinterpret_cast<const char*>(candidate.mime.ptr) : "", candidate.mime.len);
    ...
    const std::string_view selectedText(selectedView.data != nullptr ? reinterpret_cast<const char*>(selectedView.data) : "", selectedView.length);
    if (selectedView.length > maximumBytes || (selectedView.data != nullptr && std::memchr(selectedView.data, 0, selectedView.length) != nullptr) ||
...
inline void CaptureRuntimeClipboard(GhosttyTerminal /*terminal*/, void* userData, const GhosttyClipboardWrite* write) noexcept
{
    ...
    if (capture != nullptr && write != nullptr && write->size >= offsetof(GhosttyClipboardWrite, contents_len) + sizeof(size_t) && write->contents != nullptr &&
        write->contents_len == 1u)
    {
    ...
    if (capture != nullptr && write != nullptr && GhosttySizedFieldPresent(write->size, offsetof(GhosttyClipboardWrite, reply), sizeof(write->reply)) &&
        write->reply != nullptr)
    {
...
inline void CaptureRuntimePty(GhosttyTerminal /*terminal*/, void* userData, const uint8_t* data, size_t length) noexcept
{
    auto* capture = static_cast<RuntimeClipboardCapture*>(userData);
