...
[[nodiscard]] std::string EncodeRepeatedByteBase64(size_t byteCount, uint8_t value)
{
    ...
    while (remaining >= 3u)
    {
        const uint32_t packed = (static_cast<uint32_t>(value) << 16u) | (static_cast<uint32_t>(value) << 8u) | static_cast<uint32_t>(value);
    ...
    if (remaining != 0u)
    {
        const uint32_t packed = (static_cast<uint32_t>(value) << 16u) | (remaining == 2u ? static_cast<uint32_t>(value) << 8u : 0u);
        encoded.push_back(alphabet[(packed >> 18u) & 0x3Fu]);
        encoded.push_back(alphabet[(packed >> 12u) & 0x3Fu]);
...
[[nodiscard]] VtUpgradeCorpus BuildVtUpgradeCorpus()
{
    ...
    corpus.inputByteCount = corpus.prelude.size() + corpus.kittyPartial.size() + corpus.kittyCompletion.size() + corpus.postlude.size();
    return corpus;
}

[[nodiscard]] bool ComputeSha256(std::span<const uint8_t> bytes, std::array<char, TerminalVtUpgradeTestContract::kSha256TextBytes>& digestText) noexcept
{
    ...
    if (! bytes.empty())
    {
        ...
        status = BCryptHashData(hash.get(), reinterpret_cast<PUCHAR>(const_cast<uint8_t*>(bytes.data())), static_cast<ULONG>(bytes.size()), 0u);
        if (! BCRYPT_SUCCESS(status))
        {
            return false;
        }
...
[[nodiscard]] bool ComputeSha256(std::string_view bytes, std::array<char, TerminalVtUpgradeTestContract::kSha256TextBytes>& digestText) noexcept
{
    return ComputeSha256(std::span(reinterpret_cast<const uint8_t*>(bytes.data()), bytes.size()), digestText);
}
...
void ObserveProcessMemory(TerminalVtUpgradeTestContract::Evidence& evidence) noexcept
{
    ...
    if (GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(&counters), sizeof(counters)) != FALSE)
    {
...
{
    const auto nanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - startedAt).count();
...
HRESULT Terminal::DebugRunVtUpgradeCorpus(TerminalVtUpgradeTestContract::Evidence* evidence) noexcept
{
    ...
    *evidence                    = {};
    ...
    wil::unique_hwnd renderWindow(CreateWindowExW(WS_EX_TOOLWINDOW, L"STATIC", L"", WS_POPUP, 0, 0, 960, 640, nullptr, nullptr, g_hInstance, nullptr));
    if (! renderWindow)
    {
    ...
    const auto placementCount = [this](size_t& count, size_t& allocatedBytes) noexcept
    {
        ...
        return _runtime.kittyGraphicsGet(graphics, GHOSTTY_KITTY_GRAPHICS_DATA_PLACEMENT_COUNT, &count) == GHOSTTY_SUCCESS &&
               _runtime.kittyGraphicsGet(graphics, GHOSTTY_KITTY_GRAPHICS_DATA_PLACEMENT_ALLOCATED_BYTES, &allocatedBytes) == GHOSTTY_SUCCESS;
    };
    ...
    const auto captureSnapshot = [this](std::string& canonical) noexcept
    {
        ...
        if (_runtime.renderStateGet(_renderState, GHOSTTY_RENDER_STATE_DATA_COLS, &columns) != GHOSTTY_SUCCESS ||
            ...
            _runtime.renderStateGet(_renderState, GHOSTTY_RENDER_STATE_DATA_CURSOR_VISUAL_STYLE, &cursorStyle) != GHOSTTY_SUCCESS ||
        ...
        while (rowIndex < rows && _runtime.renderRowIteratorNext(_renderRows))
        {
            ...
            while (columnIndex < columns && _runtime.renderRowCellsNext(_renderCells))
            {
                ...
                const bool rawReady = _runtime.renderRowCellsGet(_renderCells, GHOSTTY_RENDER_STATE_ROW_CELLS_DATA_RAW, &rawCell) == GHOSTTY_SUCCESS;
                ...
                if (! rawReady || _runtime.cellGet(rawCell, GHOSTTY_CELL_DATA_WIDE, &wide) != GHOSTTY_SUCCESS ||
                    _runtime.cellGet(rawCell, GHOSTTY_CELL_DATA_HAS_HYPERLINK, &hyperlink) != GHOSTTY_SUCCESS ||
                    _runtime.renderRowCellsGet(_renderCells, GHOSTTY_RENDER_STATE_ROW_CELLS_DATA_SELECTED, &selected) != GHOSTTY_SUCCESS ||
                    _runtime.renderRowCellsGet(_renderCells, GHOSTTY_RENDER_STATE_ROW_CELLS_DATA_HAS_STYLING, &hasStyling) != GHOSTTY_SUCCESS ||
                    (hasStyling && _runtime.renderRowCellsGet(_renderCells, GHOSTTY_RENDER_STATE_ROW_CELLS_DATA_STYLE, &style) != GHOSTTY_SUCCESS))
                {
                    return false;
                }
                ...
                canonical.append(std::format("cell={},{},w{},h{},s{},b{},i{},f{},k{},v{},x{},o{},u{},bg{}:{:02x}{:02x}{:02x},fg{}:{:02x}{:02x}{:02x},g{}:",
                                             rowIndex,
                                             columnIndex,
...

        {
            ...
            if (_runtime.terminalGet(_ghosttyTerminal, GHOSTTY_TERMINAL_DATA_KITTY_GRAPHICS, &graphics) != GHOSTTY_SUCCESS || graphics == nullptr ||
                _runtime.kittyGraphicsGet(graphics, GHOSTTY_KITTY_GRAPHICS_DATA_PLACEMENT_COUNT, &enginePlacementCount) != GHOSTTY_SUCCESS ||
                _runtime.kittyGraphicsGet(graphics, GHOSTTY_KITTY_GRAPHICS_DATA_PLACEMENT_ITERATOR, &_kittyPlacementIterator) != GHOSTTY_SUCCESS)
            {
                return false;
            }
            ...
            while (_runtime.kittyPlacementNext(_kittyPlacementIterator))
            {
                ...
                if (_runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_IMAGE_ID, &imageId) != GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_PLACEMENT_ID, &placementId) != GHOSTTY_SUCCESS ||
                    ...
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_X, &sourceX) != GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_Y, &sourceY) != GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_WIDTH, &sourceWidth) != GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_HEIGHT, &sourceHeight) !=
                        GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_COLUMNS, &placementColumns) != GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_ROWS, &placementRows) != GHOSTTY_SUCCESS ||
                    _runtime.kittyPlacementGet(_kittyPlacementIterator, GHOSTTY_KITTY_GRAPHICS_PLACEMENT_DATA_Z, &z) != GHOSTTY_SUCCESS)
                {
                    return false;
                }
                ...
                const std::array<GhosttyKittyGraphicsImageData, 5u> imageKeys{{GHOSTTY_KITTY_IMAGE_DATA_WIDTH,
                ...
                if (image == nullptr ||
                    _runtime.kittyGraphicsImageGetMulti(image, imageKeys.size(), imageKeys.data(), imageValues.data(), nullptr) != GHOSTTY_SUCCESS ||
    ...
    constexpr std::string_view firstStyledCell       = "\x1b"
                                                       "c\x1b[31;44;1mA\x1b[0m";
    constexpr std::string_view replacementStyledCell = "\r\x1b[32;45;3mB\x1b[0m";
    _runtime.terminalVtWrite(_ghosttyTerminal, reinterpret_cast<const uint8_t*>(firstStyledCell.data()), firstStyledCell.size());
    std::string firstStyledSnapshot;
    if (! captureSnapshot(firstStyledSnapshot))
    {
        return E_FAIL;
    }
    _runtime.terminalVtWrite(_ghosttyTerminal, reinterpret_cast<const uint8_t*>(replacementStyledCell.data()), replacementStyledCell.size());
    ...
    const auto applyCorpus =
        [this, &corpus, &placementCount](std::string_view postlude, uint64_t& vtWriteUs, size_t& finalPlacementCount, size_t& finalPlacementBytes) noexcept
    {
        ...
        const auto write            = [this, &elapsedNanoseconds](std::string_view bytes) noexcept
        {
            ...
            _runtime.terminalVtWrite(_ghosttyTerminal, reinterpret_cast<const uint8_t*>(bytes.data()), bytes.size());
            elapsedNanoseconds += static_cast<uint64_t>(
                std::max<int64_t>(0, std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - startedAt).count()));
        };
    ...
    for (uint32_t ordinal = 0u; ordinal < warmupCount + evidence->sampleCount; ++ordinal)
    {
        ...
        while (std::chrono::steady_clock::now() < kittyDeadline)
        {
            ...
            {
                if (imageKey.imageId == 77u && image.state == KittyImageState::Ready && image.pendingRequestId == 0u)
                {
                    kittyReady = true;
                    break;
                }
        ...
        evidence->kittyMemoryHighWaterBytes         = std::max<uint64_t>(evidence->kittyMemoryHighWaterBytes, kittyStats.memoryHighWaterBytes);
    ...
    if (! applyCorpus(corpus.postludeWithoutExpectedDelta, controlVtWriteUs, controlPlacementCount, controlPlacementBytes) || controlVtWriteUs == 0u ||
        controlPlacementCount != 1u || controlPlacementBytes == 0u)
    {
        return E_FAIL;
    }
    InvalidateRect(renderWindow.get(), nullptr, FALSE);
    ...
    return evidence->kittyPendingProbeCount == warmupCount + evidence->sampleCount &&
                   evidence->kittyCompletedProbeCount == warmupCount + evidence->sampleCount && evidence->kittyPlacementCount == 1u &&
                   evidence->kittyPlacementBytes != 0u && evidence->kittyPlacementBytes <= kMaximumKittyPlacementBytes && evidence->kittyCacheBytes == 4u &&
                   evidence->kittyPinnedBytes == 4u && evidence->peakPrivateBytes != 0u && evidence->peakWorkingSetBytes != 0u
               ? S_OK
               : E_FAIL;
}

extern "C" __declspec(dllexport) HRESULT __stdcall RedSalamanderTerminalDebugVtUpgradeCorpus(TerminalVtUpgradeTestContract::Evidence* evidence) noexcept
{
    Terminal terminal;
