...
// Define the ETW provider for ViewerText.dll
...
{
    static const std::wstring name        = LoadStringResource(g_hInstance, IDS_VIEWERTEXT_NAME);
    static const std::wstring description = LoadStringResource(g_hInstance, IDS_VIEWERTEXT_DESCRIPTION);
    static const PluginMetaData metaData  = {
        .id          = L"builtin/viewer-text",
        .shortId     = L"read",
        .name        = name.c_str(),
...
[[nodiscard]] const char* GetPluginSchema(std::wstring_view pluginId) noexcept
{
    ...
    return GetViewerTextStaticConfigurationSchema();
}
} // namespace
...
HRESULT CreatePluginInstance(REFIID riid, IHost* host, void** result)
{
    ...
    *result = nullptr;
    ...
    {
        auto* instance = new (std::nothrow) ViewerText();
        if (instance == nullptr)
        {
