...
HRESULT STDMETHODCALLTYPE FilesInformationMtp::QueryInterface(REFIID riid, void** ppvObject) noexcept
{
    if (! ppvObject)
    {
        return E_POINTER;
...
ULONG STDMETHODCALLTYPE FilesInformationMtp::AddRef() noexcept
{
    return _refCount.fetch_add(1, std::memory_order_relaxed) + 1;
}

ULONG STDMETHODCALLTYPE FilesInformationMtp::Release() noexcept
{
...
HRESULT STDMETHODCALLTYPE FilesInformationMtp::GetBuffer(FileInfo** ppFileInfo) noexcept
{
    return _packedBuffer.GetBuffer(ppFileInfo);
}

HRESULT STDMETHODCALLTYPE FilesInformationMtp::GetBufferSize(unsigned long* pSize) noexcept
{
    return _packedBuffer.GetBufferSize(pSize);
}

HRESULT STDMETHODCALLTYPE FilesInformationMtp::GetAllocatedSize(unsigned long* pSize) noexcept
{
    return _packedBuffer.GetAllocatedSize(pSize);
}

HRESULT STDMETHODCALLTYPE FilesInformationMtp::GetCount(unsigned long* pCount) noexcept
{
    return _packedBuffer.GetCount(pCount);
}

HRESULT STDMETHODCALLTYPE FilesInformationMtp::Get(unsigned long index, FileInfo** ppEntry) noexcept
{
    return _packedBuffer.Get(index, ppEntry);
}

HRESULT FilesInformationMtp::BuildFromEntries(std::vector<Entry> entries) noexcept
{
