...
HRESULT STDMETHODCALLTYPE FilesInformationGoogleDrive::QueryInterface(REFIID riid, void** ppvObject) noexcept
{
    if (! ppvObject)
    {
        return E_POINTER;
...
ULONG STDMETHODCALLTYPE FilesInformationGoogleDrive::AddRef() noexcept
{
    return _refCount.fetch_add(1, std::memory_order_relaxed) + 1;
}

ULONG STDMETHODCALLTYPE FilesInformationGoogleDrive::Release() noexcept
{
...
HRESULT STDMETHODCALLTYPE FilesInformationGoogleDrive::GetBuffer(FileInfo** ppFileInfo) noexcept
{
    if (! ppFileInfo)
    {
        return E_POINTER;
...
HRESULT STDMETHODCALLTYPE FilesInformationGoogleDrive::GetBufferSize(unsigned long* pSize) noexcept
{
    if (! pSize)
    {
        return E_POINTER;
...
HRESULT STDMETHODCALLTYPE FilesInformationGoogleDrive::GetAllocatedSize(unsigned long* pSize) noexcept
{
    if (! pSize)
    {
        return E_POINTER;
...
HRESULT STDMETHODCALLTYPE FilesInformationGoogleDrive::GetCount(unsigned long* pCount) noexcept
{
    if (! pCount)
    {
        return E_POINTER;
...
HRESULT STDMETHODCALLTYPE FilesInformationGoogleDrive::Get(unsigned long index, FileInfo** ppEntry) noexcept
{
    if (! ppEntry)
    {
        return E_POINTER;
...
size_t FilesInformationGoogleDrive::AlignUp(size_t value, size_t alignment) noexcept
{
    const size_t mask = alignment - 1u;
    return (value + mask) & ~mask;
}

size_t FilesInformationGoogleDrive::ComputeEntrySizeBytes(std::wstring_view name) noexcept
{
...
HRESULT FilesInformationGoogleDrive::BuildFromEntries(std::vector<Entry> entries) noexcept
{
    _buffer.clear();
    _count     = 0;
    _usedBytes = 0;
    ...
    for (const Entry& source : entries)
    {
...
HRESULT FilesInformationGoogleDrive::LocateEntry(unsigned long index, FileInfo** ppEntry) const noexcept
{
