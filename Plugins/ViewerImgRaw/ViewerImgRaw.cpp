...
LRESULT CALLBACK FileComboHostWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) noexcept
{
    return RedSalamander::ViewerFileComboHost::DispatchFileComboHostWndProc<ViewerImgRaw>(
...
void ViewerImgRaw::OnCreate(HWND hwnd)
{
    ...
    if (_hFileComboHost)
    {
        ...
        _fileComboControl->SetOnSelectionChanged([this](const size_t selection) noexcept
        {
...
bool ViewerImgRaw::CancelActiveOpen() noexcept
{
    if (! _isLoading)
    {
        return false;
    ...
    _statusMessage = LoadStringResource(g_hInstance, IDS_VIEWERRAW_STATUS_CANCELED);
...
void ViewerImgRaw::OnContextMenu(HWND hwnd, POINT screenPt) noexcept
{
    if (! _menuHandle)
    {
        _menuHandle.reset(Localization::LoadMenuResource(g_hInstance, IDR_VIEWERRAW_MENU));
    }
