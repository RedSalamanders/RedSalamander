# Private testing module; import through its reviewed callers.
...
function Get-RSTestRunPlanSurfaceInventory {
    ...
    $convertPlan = {
        ...
        return @($Entries | ForEach-Object {
                $entry = $_
                $requiredMetadata = @(
                    'Id', 'ImpactTags', 'InputSets', 'ArtifactIds', 'ArtifactReadIds',
                    'ArtifactWriteIds', 'ProducesArtifactIds', 'InvalidatesReceiptScopes',
                    'CachePolicy', 'EnvironmentInputs', 'CoverageContract', 'OutcomeContract',
                    'OrderContract', 'ResourceClasses', 'EstimatedDurationKey')
                [pscustomobject]@{
                    Id = $_.Id
                    Name = $_.Name
                    Kind = $_.Kind
                    ...
                    ImpactTags = @($_.ImpactTags)
                    ArtifactReadIds = @($_.ArtifactReadIds)
                    ArtifactWriteIds = @($_.ArtifactWriteIds)
                    ResourceClasses = @($_.ResourceClasses)
                    MetadataComplete = (@($requiredMetadata | Where-Object { $null -eq $entry.PSObject.Properties[$_] }).Count -eq 0)
                }
            })
    }
...
function Get-RSTestInventory {
    ...
    $commandsFamilyFiles = [ordered]@{
        ...
        ThemeOverlay = 'Commands.SelfTest.ThemeOverlay.cpp'
    }
    ...
    $jsonReady = [ordered]@{
        ...
        runPlan = [ordered]@{
            ci = @($Inventory.RunPlan.CI | ForEach-Object {
                    [ordered]@{
                        id = $_.Id; name = $_.Name; kind = $_.Kind; artifact = $_.Artifact
                        cachePolicy = $_.CachePolicy; impactTags = @($_.ImpactTags)
                        artifactReadIds = @($_.ArtifactReadIds); artifactWriteIds = @($_.ArtifactWriteIds)
                        resourceClasses = @($_.ResourceClasses); metadataComplete = [bool]$_.MetadataComplete
                    }
                })
...
Export-ModuleMember -Function @(
    'Get-RSSelectStringCount',
    'Get-RSFileOpsPhaseIntegrity',
    'Get-RSTestProjectNames',
    'Get-RSTestInventory',
    'ConvertTo-RSTestInventoryJson'
)
