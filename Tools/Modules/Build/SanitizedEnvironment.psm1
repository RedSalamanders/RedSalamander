# Private build module; the root SanitizedEnvironment.ps1 file is a historical compatibility loader.
...
function Start-RSContainedProcess {
    ...
    try {
        try {
            $delegationFactory = if ($DelegateArtifactOperation) {
                Get-Command New-RSArtifactOperationChildDelegation -ErrorAction SilentlyContinue
            } else {
                $null
            }
            if ($null -ne $delegationFactory) {
                # Invoke the exact discovered command. SanitizedEnvironment does
                # not import the optional lock module into its private session state.
...
function Close-RSContainedProcess {
    ...
    try {
        ...
        $delegationCompleter = if ($null -ne $delegation) {
            Get-Command Complete-RSArtifactOperationChildDelegation -ErrorAction SilentlyContinue
        } else {
            $null
        }

        if ($null -ne $delegationCompleter) {
            & $delegationCompleter -Delegation $delegation
        }
    }
}
...
Export-ModuleMember -Function @(
    'Get-RSNormalizedEnvironmentMap',
    'New-RSProcessStartInfo',
    'Start-RSContainedProcess',
    'Close-RSContainedProcess',
    'Invoke-RSProcess'
)
