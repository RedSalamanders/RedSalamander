...
struct Sha256
{
    [[nodiscard]] bool Update(std::span<const uint8_t> bytes) noexcept
    {
        if (_finished ||
            bytes.size() >
                std::numeric_limits<uint64_t>::max() - _totalBytes)
        {
            return false;
        }
        ...
        {
            const size_t copied =
                std::min(bytes.size(), _buffer.size() - _bufferLength);
            std::memcpy(
                _buffer.data() + _bufferLength,
                bytes.data(),
                copied);
    ...
    [[nodiscard]] bool Finish(std::array<uint8_t, 32>& digest) noexcept
    {
        ...
        {
            std::fill(
                _buffer.begin() + static_cast<ptrdiff_t>(_bufferLength),
                _buffer.end(),
                uint8_t{0});
            Transform(_buffer.data());
            _bufferLength = 0;
        ...
        {
            _buffer[63 - index] =
                static_cast<uint8_t>(bitLength >> (index * 8));
        }
        Transform(_buffer.data());
        for (size_t index = 0; index < _state.size(); ++index)
        {
            digest[index * 4] =
                static_cast<uint8_t>(_state[index] >> 24);
            digest[index * 4 + 1] =
                static_cast<uint8_t>(_state[index] >> 16);
            digest[index * 4 + 2] =
                static_cast<uint8_t>(_state[index] >> 8);
            digest[index * 4 + 3] =
                static_cast<uint8_t>(_state[index]);
        }
        _finished = true;
        return true;
    }
...
private:
    [[nodiscard]] static constexpr uint32_t RotateRight(
        uint32_t value,
...
{
    auto consume = [&](int& index, std::string_view option)
        -> std::string_view
    {
    ...
    for (int index = 1; index < argc; ++index)
    {
        ...
        else if (!(value = consume(index, "--expected-source-sha256")).empty())
        {
            arguments.expectedSourceSha256 = value;
                      ...
                      index,
                      "--expected-adapter-header-sha256")).empty())
        {
        ...
        else if (!(value = consume(index, "--expected-build-identity")).empty())
        {
            arguments.expectedBuildIdentity = value;
                      ...
                      index,
                      "--expected-fixture-manifest-sha256")).empty())
        {
            arguments.expectedFixtureManifestSha256 = value;
        }
        else if (!(value = consume(
                      index,
                      "--expected-adapter-abi")).empty())
        {
            uint64_t parsed = 0;
            if (!ParseUnsigned(value, parsed, 10) ||
        ...
        else if (!(value = consume(index, "--measurement-mode")).empty())
        {
            ...
            measurementMode = true;
                      ...
                      index,
                      "--added-release-package-bytes")).empty())
        {
            if (!ParseUnsigned(
                    value,
                    arguments.addedReleasePackageBytes,
                    10))
            {
                return false;
            }
            arguments.hasAddedReleasePackageBytes = true;
                      ...
                      index,
                      "--crossed-abi-elements")).empty())
        {
            if (!ParseUnsigned(
                    value,
                    arguments.crossedAbiElements,
                    10))
            {
                return false;
            }
            arguments.hasCrossedAbiElements = true;
                      ...
                      index,
                      "--non-system-runtime-dependencies")).empty())
        {
            if (!ParseUnsigned(
                    value,
                    arguments.nonSystemRuntimeDependencies,
                    10))
            {
                return false;
            }
    ...
    return !arguments.adapterDll.empty() &&
        ...
        !arguments.expectedFixtureManifestSha256.empty() &&
        adapterAbi &&
        capabilities &&
        measurementMode &&
        (!arguments.fullMeasurements ||
         (arguments.hasAddedReleasePackageBytes &&
          arguments.hasCrossedAbiElements &&
          arguments.hasNonSystemRuntimeDependencies)) &&
        arguments.expectedAdapterAbi ==
            RS_TERMINAL_GATE0_ADAPTER_ABI_V1 &&
        IsLowerHex(arguments.expectedSourceSha256, 64) &&
        IsLowerHex(arguments.expectedAdapterHeaderSha256, 64) &&
        IsLowerHex(arguments.expectedFixtureManifestSha256, 64);
}
...
[[nodiscard]] bool ExpandFixtureInput(
    const FixtureContract::Fixture& fixture,
    std::vector<uint8_t>& output)
{
    if (fixture.inputSizeBytes >
    ...
    for (const auto& step : fixture.inputPlan)
    {
        ...
        case FixtureContract::InputOp::RepeatByte:
            ...
            output.insert(
                output.end(),
                static_cast<size_t>(step.repeatCount),
                step.byteValue);
            break;
        default:
            return false;
    ...
    std::string sha256;
    return output.size() == fixture.inputSizeBytes &&
        Sha256Hex(output, sha256) &&
        sha256 == fixture.inputSha256;
}

[[nodiscard]] const FixtureContract::Check* FindCheck(
    const FixtureContract::Fixture& fixture,
    std::string_view id) noexcept
{
    const auto found = std::ranges::find(
        fixture.checks,
        id,
        &FixtureContract::Check::checkId);
    return found == fixture.checks.end() ? nullptr : &*found;
}

void AppendJcsString(
    std::string& output,
    std::string_view value)
{
    static constexpr std::string_view hex = "0123456789abcdef";
...
[[nodiscard]] bool AppendDecimalString(
    std::string& output,
    uint64_t value)
{
...
[[nodiscard]] std::string_view ScheduleAlgorithmName(
    FixtureContract::ScheduleAlgorithm algorithm) noexcept
{
    switch (algorithm)
    {
    ...
    case FixtureContract::ScheduleAlgorithm::TerminatorCuts:
        return "TerminatorCuts";
    }
    return {};
}

[[nodiscard]] std::string_view ExpectedScheduleId(
    FixtureContract::ScheduleAlgorithm algorithm) noexcept
{
    switch (algorithm)
    {
    ...
    case FixtureContract::ScheduleAlgorithm::Fibonacci:
        return "fibonacci";
    case FixtureContract::ScheduleAlgorithm::TerminatorCuts:
        return "terminator-split";
    }
    return {};
}

[[nodiscard]] bool ValidateScheduleIdentity(
    const FixtureContract::Fixture& fixture,
    const FixtureContract::ScheduleSpec& schedule)
{
    const std::string_view algorithm =
        ScheduleAlgorithmName(schedule.algorithm);
    if (algorithm.empty() ||
        schedule.scheduleId !=
            ExpectedScheduleId(schedule.algorithm) ||
        !IsLowerHex(schedule.scheduleSha256, 64) ||
        (schedule.algorithm !=
             FixtureContract::ScheduleAlgorithm::TerminatorCuts &&
         !schedule.cutOffsets.empty()))
    {
        return false;
    }
    uint64_t prior = 0;
    ...
    canonical.push_back('}');
    std::string sha256;
    return Sha256Hex(canonical, sha256) &&
        sha256 == schedule.scheduleSha256;
}

[[nodiscard]] bool ValidateFixtureIdentity(
    const FixtureContract::Fixture& fixture,
    std::string& contractSha256)
{
    std::vector<uint8_t> input;
    if (!ExpandFixtureInput(fixture, input) ||
        !IsLowerHex(fixture.contractSha256, 64))
    {
        return false;
    }
    std::string canonical;
    ...
    for (size_t index = 0; index < fixture.actions.size(); ++index)
    {
        ...
        if (!Sha256Hex(actionCanonical, actionSha256) ||
    ...
    for (size_t index = 0; index < fixture.checks.size(); ++index)
    {
        ...
        if (!IsLowerHex(check.expectedSha256, 64) ||
            !Sha256Hex(check.expectedValue, expectedSha256) ||
    ...
    canonical.append("]}");
    return Sha256Hex(canonical, contractSha256) &&
        contractSha256 == fixture.contractSha256;
}

[[nodiscard]] std::vector<uint8_t> BuildPerformancePayload();

[[nodiscard]] bool VerifyFrozenFixtureShape(
    const Arguments& arguments) noexcept
{
    if (FixtureContract::kFixtureManifestSha256 !=
            arguments.expectedFixtureManifestSha256 ||
        FixtureContract::GetFixtures().size() != 29 ||
        FixtureContract::kPerformancePayload.sizeBytes !=
            8U * 1024U * 1024U ||
        !IsLowerHex(FixtureContract::kPerformancePayload.sha256, 64))
    {
        return false;
    }
    std::string emptySha256;
    std::string abcSha256;
    if (!Sha256Hex(std::string_view{}, emptySha256) ||
        !Sha256Hex("abc", abcSha256) ||
        emptySha256 !=
    ...
    for (const auto& fixture : FixtureContract::GetFixtures())
    {
        if (fixture.fixtureId.empty() ||
            (!priorFixture.empty() &&
             priorFixture.compare(fixture.fixtureId) >= 0) ||
            fixture.actions.empty() ||
            fixture.actions.front().actionId !=
                "feed-current-schedule" ||
            fixture.actions.front().argumentsJcs !=
                "{\"input\":\"inputPlan\",\"schedule\":\"current\"}" ||
            fixture.checks.size() != 2 ||
            fixture.checks[0].checkId != "resource-trace" ||
            fixture.checks[1].checkId != "state" ||
            FindCheck(fixture, "resource-trace") == nullptr ||
        ...
        for (size_t index = 0; index < fixture.actions.size(); ++index)
        {
            ...
            if (action.actionId.empty() ||
                action.argumentsJcs.empty() ||
                !IsLowerHex(action.actionSha256, 64))
            {
                return false;
            }
        ...
        for (const auto& schedule : fixture.schedules)
        {
            if (schedule.scheduleId.empty() ||
                (!priorSchedule.empty() &&
                 priorSchedule.compare(schedule.scheduleId) >= 0) ||
                !IsLowerHex(schedule.scheduleSha256, 64))
            {
                return false;
            }
        ...
        if (fixture.schedules.empty() ||
            scheduleCount > kMaximumFixtureRows)
        {
            return false;
        }
        std::string contractSha256;
        if (!ValidateFixtureIdentity(
                fixture,
                contractSha256))
        {
            return false;
    ...
    const std::vector<uint8_t> performancePayload =
        BuildPerformancePayload();
    return scheduleCount == 43 &&
        FixtureContract::kPerformancePayload.fixtureIds.size() ==
            performanceIds.size() &&
        std::ranges::equal(
            FixtureContract::kPerformancePayload.fixtureIds,
            performanceIds) &&
        performancePayload.size() ==
            FixtureContract::kPerformancePayload.sizeBytes &&
        Sha256Hex(manifestCanonical, manifestSha256) &&
        manifestSha256 ==
            FixtureContract::kFixtureManifestSha256;
}
...
[[nodiscard]] bool LoadAndVerifyAdapter(
    ...
    LoadedAdapter& loaded)
{
    ...
    loaded.module.reset(LoadLibraryExW(
        dllPath.c_str(),
        nullptr,
    ...
    static_assert(
        sizeof(address) == sizeof(RsTerminalGate0QueryAdapterFn));
    const auto query =
    ...
    if (query(
            ...
            RS_TERMINAL_GATE0_ADAPTER_ABI_V1,
            &wrongVersion) != RS_TERMINAL_GATE0_UNSUPPORTED_ABI)
    {
        return false;
    }
    ...
    if (query(
            ...
            &wrongAbi) != RS_TERMINAL_GATE0_UNSUPPORTED_ABI)
    {
        return false;
    }
    ...
    if (query(
            RS_TERMINAL_GATE0_QUERY_SYMBOL_VERSION,
            arguments.expectedAdapterAbi,
            &loaded.api) != RS_TERMINAL_GATE0_SUCCESS)
    {
        return false;
    }
    const auto& identity = loaded.api.identity;
    if (loaded.api.size != sizeof(loaded.api) ||
        identity.size != sizeof(identity) ||
        identity.adapter_abi != arguments.expectedAdapterAbi ||
        !ValidAdapterString(identity.candidate_id) ||
        !ValidAdapterString(identity.upstream_pin) ||
        !ValidAdapterString(identity.source_sha256) ||
        !ValidAdapterString(identity.adapter_header_sha256) ||
        !ValidAdapterString(identity.build_identity) ||
        ...
        identity.architecture != kExpectedArchitecture ||
        identity.calling_convention !=
            RS_TERMINAL_GATE0_CALL_CDECL ||
        identity.capabilities != arguments.expectedCapabilities ||
        (identity.capabilities & kRequiredCapabilities) !=
            kRequiredCapabilities ||
        identity.pointer_size != sizeof(void*) ||
        loaded.api.session_create == nullptr ||
        loaded.api.session_write == nullptr ||
        loaded.api.session_action == nullptr ||
        loaded.api.session_state_jcs == nullptr ||
        loaded.api.session_destroy == nullptr)
    {
        return false;
    }
    return true;
...
MakeFrozenResourceLimits() noexcept
{
    return {
        ...
        .pending_effect_process_bytes =
            kMaximumSessions * 2U * 1024U * 1024U,
        .pending_effect_session_descriptors = 256,
        .pending_effect_process_descriptors =
            static_cast<uint32_t>(kMaximumSessions) * 256,
...
{
public:
    explicit ProcessResourceLedger(
        const RsTerminalGate0ResourceLimitsV1& limits) noexcept
        : _limits(limits)
    {
    }
        ...
        uint64_t& generation) noexcept
    {
        ...
        if (slot >= _sessions.size() ||
            _sessions[slot].active ||
            _activeSessions >= _limits.maximum_sessions)
        {
            return false;
        }
        ...
        uint32_t descriptors) noexcept
    {
        ...
        if (!ValidSession(slot, generation) ||
            ...
            (kind != RS_TERMINAL_GATE0_RESOURCE_PENDING_EFFECT &&
             descriptors != 0))
        {
            ++_rejectedCalls;
            return false;
        ...
        if (bytes > sessionLimit - session.currentBytes[index] ||
            bytes > processLimit - _processCurrentBytes[index])
        {
            ++_rejectedCalls;
            return false;
        }
        if (kind == RS_TERMINAL_GATE0_RESOURCE_PENDING_EFFECT)
        {
            if (descriptors >
                    _limits.pending_effect_session_descriptors -
                        session.pendingDescriptors ||
                descriptors >
                    _limits.pending_effect_process_descriptors -
                        _processPendingDescriptors)
            {
                ++_rejectedCalls;
                return false;
            }
            ...
            session.peakPendingDescriptors = std::max(
                session.peakPendingDescriptors,
                session.pendingDescriptors);
        }
        session.currentBytes[index] += bytes;
        _processCurrentBytes[index] += bytes;
        session.peakBytes[index] = std::max(
            session.peakBytes[index],
            session.currentBytes[index]);
        _processPeakBytes[index] = std::max(
            _processPeakBytes[index],
            _processCurrentBytes[index]);
        ++_reserveCalls;
        return true;
    }
        ...
        uint32_t descriptors) noexcept
    {
        ...
        Session& session = _sessions[slot];
        if (bytes > session.currentBytes[index] ||
            bytes > _processCurrentBytes[index] ||
            descriptors > session.pendingDescriptors ||
    ...
    [[nodiscard]] bool AllReleased() const noexcept
    {
        ...
        return _activeSessions == 0 &&
            _processPendingDescriptors == 0 &&
            std::ranges::all_of(
                _processCurrentBytes,
                [](uint64_t value)
                {
                    return value == 0;
                });
    ...
    [[nodiscard]] bool PeaksWithinLimits() const noexcept
    {
        ...
        {
            const auto kind =
                static_cast<RsTerminalGate0ResourceKind>(index);
    ...
    [[nodiscard]] uint64_t CurrentCpuBytes() const noexcept
    {
        ...
        for (const auto kind : cpuKinds)
        {
            const uint64_t current =
                _processCurrentBytes[static_cast<size_t>(kind)];
            if (current >
...
private:
    struct Session
    {
        std::array<
            uint64_t,
            RS_TERMINAL_GATE0_RESOURCE_COUNT> currentBytes{};
        std::array<
            uint64_t,
            RS_TERMINAL_GATE0_RESOURCE_COUNT> peakBytes{};
    ...
    [[nodiscard]] bool ValidSession(
        size_t slot,
        uint64_t generation) const noexcept
    {
        return slot < _sessions.size() &&
            _sessions[slot].active &&
            _sessions[slot].generation == generation;
    }

    [[nodiscard]] uint64_t SessionByteLimit(
        RsTerminalGate0ResourceKind kind) const noexcept
    {
        ...
        case RS_TERMINAL_GATE0_RESOURCE_COUNT:
            break;
        }
        return 0;
    }

    [[nodiscard]] uint64_t ProcessByteLimit(
        RsTerminalGate0ResourceKind kind) const noexcept
    {
        switch (kind)
        {
        case RS_TERMINAL_GATE0_RESOURCE_TERMINAL_TEXT_CPU:
            return _limits.terminal_text_process_bytes;
        case RS_TERMINAL_GATE0_RESOURCE_IMAGE_CPU:
            return _limits.image_cpu_process_bytes;
        case RS_TERMINAL_GATE0_RESOURCE_IMAGE_GPU:
            return _limits.image_gpu_process_bytes;
        case RS_TERMINAL_GATE0_RESOURCE_PENDING_EFFECT:
            return _limits.pending_effect_process_bytes;
        case RS_TERMINAL_GATE0_RESOURCE_COUNT:
            break;
        }
        return 0;
    ...
    std::array<
        uint64_t,
        RS_TERMINAL_GATE0_RESOURCE_COUNT> _processCurrentBytes{};
    std::array<
        uint64_t,
        RS_TERMINAL_GATE0_RESOURCE_COUNT> _processPeakBytes{};
...
bool RS_TERMINAL_GATE0_CALL ReserveResource(
    void* userdata,
    RsTerminalGate0ResourceKind kind,
    uint64_t bytes,
    uint32_t descriptors) noexcept
{
    auto& context =
        *static_cast<ResourceCallbackContext*>(userdata);
    ...
    FeedAudit& audit) noexcept
{
    if (begin > end || end > payload.size() ||
        adapter.session_write(
            session,
            payload.data() + begin,
            end - begin) != RS_TERMINAL_GATE0_SUCCESS)
    {
        return false;
    }
    ...
    FeedAudit& audit) noexcept
{
    ...
    return audit.calls != 0 &&
        audit.bytes == payload.size();
}

[[nodiscard]] bool IsLimitFixture(
    std::string_view fixtureId) noexcept
{
...
[[nodiscard]] bool IsOrderingAction(
    std::string_view actionId) noexcept
{
    return actionId == "encode-paste-enabled" ||
        actionId == "encode-focus-gained-lost" ||
        actionId == "encode-mouse-matrix" ||
        actionId == "admit-external-events";
}

[[nodiscard]] bool IsKnownNonFeedAction(
    std::string_view actionId) noexcept
{
...
[[nodiscard]] bool ValidateActionObservation(
    std::string_view actionId,
    const RsTerminalGate0ActionObservationV1& observation,
    bool& limitEvidence) noexcept
{
    ...
    if (observation.size != sizeof(observation) ||
        (observation.flags & ~knownFlags) != 0 ||
        (observation.flags &
         (RS_TERMINAL_GATE0_ACTION_OBSERVED |
          RS_TERMINAL_GATE0_ACTION_CHECKED_ARITHMETIC)) !=
            (RS_TERMINAL_GATE0_ACTION_OBSERVED |
    ...
    if (IsOrderingAction(actionId) &&
        (observation.flags &
         RS_TERMINAL_GATE0_ACTION_ORDER_PRESERVED) == 0)
    {
        return false;
    }
    if (actionId == "teardown-sequence" &&
        (observation.flags &
         RS_TERMINAL_GATE0_ACTION_QUIET_POINT_REACHED) == 0)
    {
        return false;
    }
    return true;
...
[[nodiscard]] bool ResourceTraceJcs(
    bool allReservationsReleased,
    bool checkedArithmetic,
    bool limitFixture,
    bool peakWithinFrozenLimits,
    bool rejectionBeforeAllocation,
    std::string& output)
{
    output.clear();
    ...
    FixtureRow& row)
{
    ...
    row.expectedResourceTraceSha256 =
        expectedResource->expectedSha256;
    ...
    callbackContext.callbackError.store(
        false,
        std::memory_order_relaxed);
    callbackContext.accepting.store(
        true,
    ...
    if (adapter.session_create(&config, &session) ==
    ...
    if (createSucceeded)
    {
        for (size_t index = 0; index < fixture.actions.size(); ++index)
        {
            ...
            const RsTerminalGate0ActionV1 action = {
                ...
                .arguments_jcs =
                    AdapterString(frozenAction.argumentsJcs),
            };
        ...
        stateRead = ReadStateJcs(
            adapter,
            session,
            actualState);
        if (stateRead)
        {
            stateHashValid = Sha256Hex(
                actualState,
                row.actualStateSha256);
            stateExact =
                actualState == expectedState->expectedValue &&
                stateHashValid &&
                row.actualStateSha256 ==
                    expectedState->expectedSha256;
        }
        if (!stateHashValid)
        {
            (void)Sha256Hex(
    ...
    callbackContext.accepting.store(
        false,
        std::memory_order_release);
    const bool sessionReleased =
        ledger.Close(0, generation);
    const bool callbackClean =
        !callbackContext.callbackError.load(
    ...
    const bool allReleased =
        ...
        callbackClean &&
        ledger.AllReleased();
    const bool checkedArithmetic =
        actionsPassed &&
        ledger.CheckedArithmetic() &&
        callbackClean;
    const bool limitFixture =
        IsLimitFixture(fixture.fixtureId);
    const bool rejectionBeforeAllocation =
        limitFixture &&
        stateExact &&
        (limitEvidence || fixture.actions.size() == 1);
    std::string actualResource;
    const bool resourceBuilt = ResourceTraceJcs(
        allReleased,
        checkedArithmetic,
        limitFixture,
        ledger.PeaksWithinLimits(),
        rejectionBeforeAllocation,
        actualResource);
    const bool resourceHashValid =
        resourceBuilt &&
        Sha256Hex(
            actualResource,
    ...
    const bool resourceExact =
        resourceHashValid &&
        actualResource == expectedResource->expectedValue &&
        row.actualResourceTraceSha256 ==
            expectedResource->expectedSha256;
    row.passed =
        createSucceeded &&
...
[[nodiscard]] bool RunFixtureMatrix(
    const RsTerminalGate0AdapterV1& adapter,
    const RsTerminalGate0ResourceLimitsV1& limits,
    ProcessResourceLedger& ledger,
    ...
    std::vector<FixtureRow>& rows)
{
    ...
    for (const auto& fixture : FixtureContract::GetFixtures())
    {
        ...
        for (const auto& schedule : fixture.schedules)
        {
            ...
            FixtureRow row;
            const bool rowPassed = RunFixtureRow(
                adapter,
                limits,
                ledger,
                callbackContexts[contextIndex],
                fixture,
                schedule,
                payload,
                row);
    ...
    {
        if (callbackContexts[index].callbackError.load(
                std::memory_order_acquire))
        {
...
[[nodiscard]] bool RunEightSessionStorm(
    const RsTerminalGate0AdapterV1& adapter,
    const RsTerminalGate0ResourceLimitsV1& limits,
    ProcessResourceLedger& ledger,
    std::span<ResourceCallbackContext> callbackContexts) noexcept
{
    ...
    for (size_t index = 0; index < count; ++index)
    {
        ...
        context.callbackError.store(
            false,
            std::memory_order_relaxed);
        context.accepting.store(
            true,
            std::memory_order_release);
        callbacks[index] = {
            .size = sizeof(RsTerminalGate0ResourceCallbacksV1),
        ...
        if (adapter.session_create(
                &configs[index],
                &sessions[index]) != RS_TERMINAL_GATE0_SUCCESS ||
            sessions[index] == nullptr)
        {
            passed = false;
            break;
        }
        ...
        if (adapter.session_write(
                sessions[index],
                storm.data(),
                storm.size()) != RS_TERMINAL_GATE0_SUCCESS)
        {
            passed = false;
            break;
        }
    ...
    for (size_t reverse = count; reverse != 0; --reverse)
    {
        ...
        if (sessions[index] != nullptr &&
            adapter.session_destroy(sessions[index]) !=
                RS_TERMINAL_GATE0_SUCCESS)
        {
            passed = false;
        }
        callbackContexts[index].accepting.store(
            false,
            std::memory_order_release);
        if (opened[index] &&
            !ledger.Close(index, generations[index]))
        {
            passed = false;
        }
    }
...
[[nodiscard]] const FixtureContract::Fixture* FindFixture(
    std::string_view fixtureId) noexcept
{
    ...
    const auto found = std::ranges::find(
        fixtures,
        fixtureId,
        &FixtureContract::Fixture::fixtureId);
...
[[nodiscard]] std::vector<uint8_t> BuildPerformancePayload()
{
    const auto& specification =
        FixtureContract::kPerformancePayload;
    std::array<std::vector<uint8_t>, 5> fixturePayloads;
    if (specification.fixtureIds.size() !=
        fixturePayloads.size())
    {
        return {};
    }
    ...
    {
        const auto* fixture =
            FindFixture(specification.fixtureIds[index]);
        if (fixture == nullptr ||
            !ExpandFixtureInput(
                *fixture,
    ...
    uint64_t counter = 0;
    while (payload.size() <
        static_cast<size_t>(specification.sizeBytes))
    {
        const auto& fixture =
            fixturePayloads[
                static_cast<size_t>(
                    counter % fixturePayloads.size())];
        std::array<char, 16> counterBytes{};
        std::array<char, 16> convertedBytes{};
        const auto converted = std::to_chars(
            convertedBytes.data(),
            convertedBytes.data() + convertedBytes.size(),
            counter,
            16);
        if (converted.ec != std::errc{})
        {
        ...
        const size_t digits = static_cast<size_t>(
            converted.ptr - convertedBytes.data());
        std::fill(
            counterBytes.begin(),
            counterBytes.end() -
                static_cast<ptrdiff_t>(digits),
            '0');
        std::copy(
            convertedBytes.begin(),
            convertedBytes.begin() +
                static_cast<ptrdiff_t>(digits),
            counterBytes.end() -
                static_cast<ptrdiff_t>(digits));
        ...
        payload.insert(
            payload.end(),
            reinterpret_cast<const uint8_t*>(counterBytes.data()),
            reinterpret_cast<const uint8_t*>(
                counterBytes.data() + counterBytes.size()));
        payload.push_back('\n');
        ++counter;
    }
    std::string sha256;
    if (payload.size() != specification.sizeBytes ||
        !Sha256Hex(payload, sha256) ||
...
[[nodiscard]] bool CreatePerformanceSession(
    const RsTerminalGate0AdapterV1& adapter,
    const RsTerminalGate0ResourceLimitsV1& limits,
    ProcessResourceLedger& ledger,
    ...
    PerformanceSession& session) noexcept
{
    ...
    if (adapter.session_create(
            &config,
            &session.session) != RS_TERMINAL_GATE0_SUCCESS ||
...
[[nodiscard]] bool DestroyPerformanceSession(
    const RsTerminalGate0AdapterV1& adapter,
    ProcessResourceLedger& ledger,
    PerformanceSession& session) noexcept
{
    ...
    const bool destroyed =
        session.session != nullptr &&
        adapter.session_destroy(session.session) ==
            RS_TERMINAL_GATE0_SUCCESS;
    session.context->accepting.store(
        false,
        std::memory_order_release);
    const bool closed =
        ledger.Close(session.slot, session.generation);
    const bool callbackClean =
        !session.context->callbackError.load(
            std::memory_order_acquire);
    session.session = nullptr;
    return destroyed &&
        closed &&
...
[[nodiscard]] bool RunBenchmarkAction(
    const RsTerminalGate0AdapterV1& adapter,
    RsTerminalGate0Session session,
    std::string_view actionId,
    std::string_view argumentsJcs) noexcept
{
    ...
    constexpr uint64_t required =
        RS_TERMINAL_GATE0_ACTION_OBSERVED |
        RS_TERMINAL_GATE0_ACTION_CHECKED_ARITHMETIC;
    return adapter.session_action(
               session,
               &action,
               &observation) == RS_TERMINAL_GATE0_SUCCESS &&
        observation.size == sizeof(observation) &&
...
[[nodiscard]] uint64_t QpcDeltaToMicroseconds(
    uint64_t delta,
    uint64_t frequency) noexcept
{
    ...
    if (whole >
        std::numeric_limits<uint64_t>::max() / 1'000'000U ||
        remainder >
        std::numeric_limits<uint64_t>::max() / 1'000'000U)
    {
        return std::numeric_limits<uint64_t>::max();
    ...
    const bool greaterThanHalf =
        fractionalRemainder > frequency - fractionalRemainder;
    const bool exactHalf =
        fractionalRemainder == frequency - fractionalRemainder;
    if (greaterThanHalf ||
        (exactHalf && (fractional & 1U) != 0))
    {
        ++fractional;
    }
    ...
    Cleanup&& cleanup)
{
    ...
    const size_t iterations =
        static_cast<size_t>(warmups) + samples;
    for (size_t iteration = 0; iteration < iterations; ++iteration)
    {
        ...
        {
            const uint64_t converted = QpcDeltaToMicroseconds(
                static_cast<uint64_t>(
                    end.QuadPart - begin.QuadPart),
                frequency);
            if (converted ==
...
[[nodiscard]] bool RunFullMeasurements(
    const Arguments& arguments,
    const RsTerminalGate0AdapterV1& adapter,
    const RsTerminalGate0ResourceLimitsV1& limits,
    ProcessResourceLedger& ledger,
    ...
    uint64_t& qpcFrequency)
{
    ...
    const std::vector<uint8_t> parsePayload =
        BuildPerformancePayload();
    ...
    for (uint16_t row = 0; row < kRows; ++row)
    {
        ...
        *cursor++ = '[';
        const auto converted = std::to_chars(
            cursor,
            cursorPosition.data() + cursorPosition.size() - 3,
        ...
        *cursor++ = 'H';
        appendBytes(
            ...
            std::string_view(
                cursorPosition.data(),
                static_cast<size_t>(
                    cursor - cursorPosition.data())));
        for (uint16_t column = 0; column < kColumns; ++column)
        {
            snapshotPayload.push_back(static_cast<uint8_t>(
                'a' + ((row + column) % 26)));
        }
    }
    appendBytes(
        ...
        "/wAA/w==\x1b\\");
    constexpr std::string_view wrappedLine =
    ...
    auto initializeMeasurement = [&measurements](
                                     size_t index,
                                     const char* id,
                                     const char* unit)
        -> Measurement&
    ...
    Measurement& create = initializeMeasurement(
        0,
        "create-destroy-us",
        "microseconds");
    if (!CollectMeasurement(
            ...
            [&]()
            {
                ...
                const bool created = CreatePerformanceSession(
                        adapter,
                        limits,
                        ledger,
                        *selected,
                        0,
                        "performance-create-destroy",
                        "performance",
                        active);
    ...
    Measurement& parse = initializeMeasurement(
        1,
        "parse-8mib-us",
        "microseconds");
    if (!CollectMeasurement(
            ...
            [&]()
            {
                ...
                while (offset < parsePayload.size())
                {
                    const size_t chunk = std::min(
                        chunks[scheduleIndex++ % chunks.size()],
                        parsePayload.size() - offset);
                    if (adapter.session_write(
                            active.session,
                            parsePayload.data() + offset,
                            chunk) != RS_TERMINAL_GATE0_SUCCESS)
                    {
                        return false;
                    }
                    offset += chunk;
            ...
            [&]()
            {
                return DestroyPerformanceSession(
                    adapter,
                    ledger,
                    active);
            }))
    ...
    Measurement& full = initializeMeasurement(
        2,
        "full-snapshot-us",
        "microseconds");
    if (!CollectMeasurement(
    ...
    Measurement& dirty = initializeMeasurement(
        3,
        "dirty-snapshot-us",
        "microseconds");
    if (!CollectMeasurement(
            ...
            [&]()
            {
                ...
                constexpr std::string_view mutation =
                    "\x1b[1;1HA\x1b[2;1HB\x1b[3;3H";
                return adapter.session_write(
                    active.session,
                    reinterpret_cast<const uint8_t*>(
                        mutation.data()),
                    mutation.size()) ==
    ...
    Measurement& resize = initializeMeasurement(
        4,
        "resize-reflow-us",
        "microseconds");
    if (!CollectMeasurement(
            ...
            [&]()
            {
                return RunBenchmarkAction(
                    adapter,
                    active.session,
                    "benchmark-resize-reflow",
                    "{\"dimensions\":\"80x50,120x40\"}");
            },
            cleanupActive))
    {
        return false;
    ...
    Measurement& kitty = initializeMeasurement(
        5,
        "kitty-1024-rgba-us",
        "microseconds");
    if (!CollectMeasurement(
            ...
            [&]()
            {
                return RunBenchmarkAction(
                    ...
                    "{\"format\":\"rgba\",\"height\":\"1024\",\"width\":\"1024\"}");
            },
            cleanupActive))
    {
        return false;
    ...
    Measurement& storm = initializeMeasurement(
        6,
    ...
    auto stormScenario = [&](uint64_t* retainedCpuBytes)
    {
        ...
        for (size_t offset = 0;
             scenarioPassed && offset < sessionPayloadBytes;
             offset += chunkBytes)
        {
            const size_t length =
                std::min(chunkBytes, sessionPayloadBytes - offset);
            for (const auto& session : stormSessions)
            {
                if (session.session == nullptr ||
                    adapter.session_write(
                        session.session,
                        parsePayload.data() + offset,
                        length) != RS_TERMINAL_GATE0_SUCCESS)
                {
                    scenarioPassed = false;
                    break;
                }
        ...
        for (size_t reverse = stormSessions.size();
             reverse != 0;
             --reverse)
        {
            if (stormSessions[reverse - 1].session != nullptr &&
                !DestroyPerformanceSession(
                    adapter,
                    ledger,
                    stormSessions[reverse - 1]))
            {
                scenarioPassed = false;
            }
        }
    ...
    Measurement& retained = initializeMeasurement(
        7,
...
struct HarnessResult
{
    ...
    void AddCheck(
        ...
        const char* failCategory) noexcept
    {
        if (checkCount < checks.size())
        {
            checks[checkCount++] = {
                ...
                .passed = value,
                .resultCategory =
                    value ? passCategory : failCategory,
            };
        }
        passed = passed && value;
    }
...
void EmitResult(
    ...
    bool adapterIdentityAvailable) noexcept
{
    std::fputs(
        "{\"formatId\":\"red-salamander-terminal-engine-gate0-harness\","
        "\"version\":1,\"status\":\"",
        stdout);
    ...
    PrintJsonString(
        arguments.fullMeasurements ? "full" : "functional");
    ...
    for (size_t index = 0; index < result.rows.size(); ++index)
    {
        ...
        std::fprintf(
            stdout,
            ",\"inputSizeBytes\":\"%llu\",\"expectedStateSha256\":",
            static_cast<unsigned long long>(row.inputSizeBytes));
    ...
    std::fprintf(
        stdout,
        "],\"qpcFrequency\":\"%llu\",\"measurements\":[",
        static_cast<unsigned long long>(result.qpcFrequency));
    for (size_t index = 0; index < result.measurementCount; ++index)
    {
        ...
        std::fprintf(
            stdout,
            ",\"warmupCount\":%u,\"samples\":[",
            measurement.warmupCount);
        for (size_t sample = 0;
             sample < measurement.sampleCount;
             ++sample)
        {
            if (sample != 0)
            {
...
int main(int argc, char** argv)
{
    ...
    {
        std::fputs(
            "Invalid arguments for TerminalEngineGate0Harness.\n",
            stderr);
        return 2;
    }

    HarnessResult result;
    const bool fixtureContract =
        VerifyFrozenFixtureShape(arguments);
    result.AddCheck(
        "frozen-fixture-contract",
        fixtureContract,
        "fixture-contract-matched",
    ...
    LoadedAdapter loaded;
    const bool adapterIdentity =
        LoadAndVerifyAdapter(arguments, loaded);
    result.AddCheck(
        "adapter-runtime-identity",
        adapterIdentity,
        "adapter-identity-matched",
    ...
    std::array<ResourceCallbackContext, 768>
        callbackContexts{};
    const bool fixtureExecution = RunFixtureMatrix(
        loaded.api,
        limits,
        ledger,
        std::span(
            callbackContexts.data(),
            kMaximumFixtureRows),
        result.rows);
    result.AddCheck(
        "fixture-matrix",
        fixtureExecution,
        "every-fixture-schedule-action-matched",
        "fixture-schedule-action-mismatch");

    const bool sessionStorm = RunEightSessionStorm(
        loaded.api,
        limits,
        ledger,
        std::span(
            callbackContexts.data() + kMaximumFixtureRows,
            8));
    result.AddCheck(
        "shared-process-ledger",
        sessionStorm,
        "eight-session-ledger-balanced",
        "eight-session-ledger-failed");
    if (arguments.fullMeasurements)
    {
        const bool measurements = RunFullMeasurements(
            arguments,
            loaded.api,
            limits,
            ledger,
            std::span(
                callbackContexts.data() +
                    kMaximumFixtureRows + 8,
                callbackContexts.size() -
                    kMaximumFixtureRows - 8),
            result.measurements,
            result.qpcFrequency);
        result.measurementCount =
            measurements ? result.measurements.size() : 0;
        result.AddCheck(
            "full-measurements",
            measurements,
            "all-eleven-measurements-collected",
            "neutral-performance-runner-failed");
    }

    bool callbacksQuiet = true;
    for (const auto& context : callbackContexts)
    {
        callbacksQuiet = callbacksQuiet &&
            !context.callbackError.load(
                std::memory_order_acquire);
    }
    result.AddCheck(
        "callback-quiet-point",
        callbacksQuiet,
        "callbacks-quiet-before-unload",
        "callback-after-session-destroy");

    HMODULE module = loaded.module.release();
    const bool unloaded =
        module != nullptr && FreeLibrary(module) != FALSE;
    result.AddCheck(
        "adapter-unload",
        unloaded,
        "adapter-unloaded-after-quiet-point",
        "adapter-unload-failed");
