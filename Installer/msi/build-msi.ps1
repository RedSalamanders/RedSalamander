...
.PARAMETER OutputDirectory
    Output directory for the MSI. It must resolve to .build\AppPackages or one
    of its descendants; the default is the AppPackages root.
.PARAMETER BuildNumber
    Required positive build number identifying the already compiled Release profile.
.EXAMPLE
    .\Installer\msi\build-msi.ps1 -Configuration Release -Platform x64 -BuildNumber 184
.OUTPUTS
    None. Writes the versioned MSI to OutputDirectory.
.NOTES
...
if ($BuildNumber -le 0 -or $BuildNumber -gt 65535) {
    throw 'Standalone MSI packaging requires -BuildNumber in the range 1..65535. Use build.ps1 -Msi to propagate the compiled profile version automatically.'
}
...
try {
    $artifactProfileLock = Enter-RSArtifactOperationLock `
        ...
        -Operation "standalone MSI input $Configuration|$Platform" `
    ...
    $packagingLock = Enter-RSArtifactOperationLock `
        -RepoRoot $solutionDir `
        -Operation "standalone MSI packaging $Configuration|$Platform" `
...
$robocopyExitCode = Invoke-RSProcess `
    -FilePath $robocopyExe `
...
$shippingRobocopyArguments = @($releaseDir, $stageDir) + $shippingExes + @(
    '/R:1',
    '/W:1',
    '/NFL',
    '/NDL',
...
$shippingRobocopyExitCode = Invoke-RSProcess `
    -FilePath $robocopyExe `
    -Arguments $shippingRobocopyArguments `
    -WorkingDirectory $solutionDir
if ($shippingRobocopyExitCode -ge 8) {
    throw "robocopy (shipping exes) failed with exit code $shippingRobocopyExitCode"
}
foreach ($exe in $shippingExes) {
