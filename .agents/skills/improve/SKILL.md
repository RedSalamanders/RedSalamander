...
Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), write plans for the top 3–5 by leverage and record that default in the survey index (`advisor-plans/README.md` in this repository; do not recreate root `plans/`).
...
**RedSalamander:** root `plans/` was archived to `Specs/Plans/Done/` on 2026-08-25 and deleted. Do not recreate it. Write new advisor plans under `advisor-plans/` (docs-only surveys) or indexed `Specs/Plans/WIP/` (product work).
